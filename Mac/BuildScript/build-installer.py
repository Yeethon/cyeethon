        ...
        result.extend(
            [
                dict(
                    ...
                    url=(
                        "ftp://ftp.tcl.tk/pub/tcl//tcl8_6/tcl%s-src.tar.gz"
                        % (tcl_tk_ver,)
                    ),
                    ...
                    install=(
                        "make TCL_LIBRARY=%(TCL_LIBRARY)s && make install TCL_LIBRARY=%(TCL_LIBRARY)s DESTDIR=%(DESTDIR)s"
                        % {
                ...
                dict(
                    ...
                    url=(
                        "ftp://ftp.tcl.tk/pub/tcl//tcl8_6/tk%s-src.tar.gz"
                        % (tcl_tk_ver,)
                    ),
                    ...
                    useLDFlags=False,
                    install=(
                        "make TCL_LIBRARY=%(TCL_LIBRARY)s TK_LIBRARY=%(TK_LIBRARY)s && make install TCL_LIBRARY=%(TCL_LIBRARY)s TK_LIBRARY=%(TK_LIBRARY)s DESTDIR=%(DESTDIR)s"
                        % {
                            "DESTDIR": shellQuote(os.path.join(WORKDIR, "libraries")),
                            "TCL_LIBRARY": shellQuote(
                                (
    ...
    result.extend(
        [
            ...
            dict(
                ...
                extra_cflags=(
                    "-Os -DSQLITE_ENABLE_FTS5 -DSQLITE_ENABLE_FTS4 -DSQLITE_ENABLE_FTS3_PARENTHESIS -DSQLITE_ENABLE_JSON1 -DSQLITE_ENABLE_RTREE -DSQLITE_TCL=0 %s"
                    % ("", "-DSQLITE_WITHOUT_ZONEMALLOC ")[LT_10_5]
                ),
                configure_pre=[
                    "--enable-threadsafe",
                    "--enable-shared=no",
        ...
        result.extend(
            [
                dict(
                    ...
                    configure=None,
                    install=(
                        'make install CC=%s CXX=%s, PREFIX=%s/usr/local/ CFLAGS="-arch %s"'
                        % (
                            CC,
                            CXX,
                            shellQuote(os.path.join(WORKDIR, "libraries")),
        ...
        result.extend(
            [
                dict(
                    ...
                    configure_pre=["--includedir=/usr/local/include/db4"],
                )
            ]
        )
    return result
...
def pkg_recipes():
    ...
    result = [
        dict(
            ...
            readme="                This package installs Python.framework, that is the python\n                interpreter and the standard library.\n            ",
        ...
        dict(
            ...
            readme="                This package installs IDLE (an interactive Python IDE),\n                Python Launcher and Build Applet (create application bundles\n                from python scripts).\n\n                It also installs a number of examples and demos.\n                ",
            required=False,
            selected="selected",
        ),
        dict(
            ...
            readme="                This package installs the unix tools in /usr/local/bin for\n                compatibility with older releases of Python. This package\n                is not necessary to use Python.\n                ",
            required=False,
            selected="selected",
        ),
        dict(
            ...
            source="/pydocs",
            readme="                This package installs the python documentation at a location\n                that is useable for pydoc and IDLE.\n                ",
        ...
        dict(
            ...
            readme="                This package installs (or upgrades from an earlier version)\n                pip, a tool for installing and managing Python packages.\n                ",
    ...
    if not internalTk():
        ...
        for framework in ["Tcl", "Tk"]:
            ...
            if not os.path.exists(libfw):
                fatal(
                    (
                        "Please install a link to a current %s %s as %s so the user can override the system framework."
                        % (framework, frameworks[framework], libfw)
                    )
                )
            if os.path.exists(usrfw):
                fatal(
                    ("Please rename %s to avoid possible dynamic load issues." % usrfw)
                )
...
def build_universal_openssl(basedir, archList):
    ...
    def build_openssl_arch(archbase, arch):
        ...
        runCommand(
            " ".join(((["perl", "Configure"] + arch_opts[arch]) + configure_opts))
        )
    ...
    srcdir = os.getcwd()
    universalbase = os.path.join(
        srcdir, "..", (os.path.basename(srcdir) + "-universal")
    )
    ...
    for (lib_unversioned, lib_versioned) in [
        ...
        runCommand(
            (
                ...
                + " ".join(
                    (
                        shellQuote(os.path.join(fw, "lib", lib_versioned))
    ...
    if configure is not None:
        ...
        if recipe.get("useLDFlags", 1):
            configure_args.extend(
                [
                    (
                        "CFLAGS=%s-mmacosx-version-min=%s -arch %s -I%s/usr/local/include"
                        % (
...
def buildPython():
    ...
    runCommand(
        (
            "%s -C --enable-framework --enable-universalsdk=/ --with-universal-archs=%s %s %s %s %s %s LDFLAGS='-g -L%s/libraries/usr/local/lib' CFLAGS='-g -I%s/libraries/usr/local/include' 2>&1"
            % (
        ...
        for fn in filenames:
            ...
            if fn in EXPECTED_SHARED_LIBS:
                ...
                for sl in EXPECTED_SHARED_LIBS[fn]:
                    if ("\t%s " % sl) not in data:
                        print(
                            ("Expected shared lib %s was not linked with %s" % (sl, p))
                        )
...
def buildDMG():
    ...
    imagepath = os.path.join(
        outdir, ("python-%s-macosx%s" % (getFullVersion(), build_system_version))
    )
    ...
    runCommand(
        (
            ...
            % (
                shellQuote((imagepath + ".tmp.dmg")),
                shellQuote(os.path.join(WORKDIR, "mnt")),
            )
        )
    )
    shutil.copy(
...
def setIcon(filePath, icnsPath):
    ...
    if (not os.path.exists(toolPath)) or (
        os.stat(toolPath).st_mtime < os.stat((dirPath + "/seticon.m")).st_mtime
    ):
