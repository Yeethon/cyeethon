...
class Stats:
    ...
    def init(self, arg):
        ...
        try:
            ...
            print(
                ("Invalid timing data %s" % (self.files[(-1)] if self.files else "")),
                file=self.stream,
            )
            raise
    ...
    def load_stats(self, arg):
        ...
        if not self.stats:
            raise TypeError(
                (
                    "Cannot create or construct a %r object from %r"
                    % (self.__class__, arg)
                )
            )
        return
    ...
    def sort_stats(self, *field):
        ...
        if (len(field) == 1) and isinstance(field[0], int):
            field = [
                {(-1): "stdname", 0: "calls", 1: "time", 2: "cumulative"}[field[0]]
            ]
        elif len(field) >= 2:
            for arg in field[1:]:
    ...
    def print_call_heading(self, name_size, column_title):
        ...
        if subheader:
            print(
                ((" " * name_size) + "    ncalls  tottime  cumtime"), file=self.stream
            )

    def print_call_line(self, name_size, source, call_dict, arrow="->"):
        print(
            (func_std_string(source).ljust(name_size) + arrow),
            end=" ",
            file=self.stream,
        )
        if not call_dict:
