...
for key in SCHEME_KEYS:
    ...
    if key == "headers":
        sys_key = "include"
    INSTALL_SCHEMES["unix_prefix"][key] = sysconfig._INSTALL_SCHEMES["posix_prefix"][
        sys_key
    ]
    INSTALL_SCHEMES["unix_home"][key] = sysconfig._INSTALL_SCHEMES["posix_home"][
        sys_key
    ]
...
class install(Command):
    ...
    def run(self):
        ...
        if (
            ...
            log.debug(
                "modules installed to '%s', which is not in Python's module search path (sys.path) -- you'll have to change the search path yourself",
                self.install_lib,
            )
