...
class build_ext(Command):
    ...
    user_options = [
        ...
        (
            ...
            "p",
            (
                "platform name to cross-compile for, if supported (default: %s)"
                % get_platform()
            ),
        ),
        (
    ...
    def build_extension(self, ext):
        ...
        if (sources is None) or (not isinstance(sources, (list, tuple))):
            raise DistutilsSetupError(
                (
                    "in 'ext_modules' option (extension '%s'), 'sources' must be present and must be a list of source filenames"
                    % ext.name
                )
            )
    ...
    def find_swig(self):
        ...
        if os.name == "posix":
            ...
            raise DistutilsPlatformError(
                (
                    "I don't know how to find (much less run) SWIG on platform '%s'"
                    % os.name
                )
            )
