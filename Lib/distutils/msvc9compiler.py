    ...
    def initialize(self, plat_name=None):
        ...
        if (
            ...
            if (plat_name == get_platform()) or (plat_name == "win32"):
                ...
                plat_spec = (PLAT_TO_VCVARS[get_platform()] + "_") + PLAT_TO_VCVARS[
                    plat_name
                ]
    ...
    def _remove_visual_c_ref(self, manifest_file):
        try:
            ...
            try:
                ...
                manifest_f.close()
            pattern = re.compile(
                "<assemblyIdentity.*?name=(\"|')Microsoft\\.VC\\d{2}\\.CRT(\"|').*?(/>|</assemblyIdentity>)",
                re.DOTALL,
            )
            ...
            manifest_buf = re.sub(pattern, "", manifest_buf)
            pattern = re.compile(
                "<assemblyIdentity.*?name=(?:\"|')(.+?)(?:\"|').*?(?:/>|</assemblyIdentity>)",
                re.DOTALL,
            )
