...
class BuildExtTestCase(TempdirManager, LoggingSilencer, unittest.TestCase):
    ...
    def test_unicode_module_names(self):
        ...
        cmd = self.build_ext(dist)
        cmd.ensure_finalized()
        self.assertRegex(cmd.get_ext_filename(modules[0].name), "foo(_d)?\\..*")
        self.assertRegex(cmd.get_ext_filename(modules[1].name), "föö(_d)?\\..*")
    ...
    def _try_compile_deployment_target(self, operator, target):
        ...
        deptarget_ext = Extension(
            "deptarget", [deptarget_c], extra_compile_args=[("-DTARGET=%s" % (target,))]
        )
