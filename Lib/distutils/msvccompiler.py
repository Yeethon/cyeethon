...
class MSVCCompiler(CCompiler):
    ...
    _mc_extensions = [".mc"]
    src_extensions = (
        (_c_extensions + _cpp_extensions) + _rc_extensions
    ) + _mc_extensions
    res_extension = ".res"
    obj_extension = ".obj"
    ...
    def get_msvc_paths(self, path, platform="x86"):
        ...
        if self.__version >= 7:
            ...
            key = (
                "%s\\6.0\\Build System\\Components\\Platforms\\Win32 (%s)\\Directories"
        ...
        if self.__version == 6:
            for base in HKEYS:
                if read_values(base, ("%s\\6.0" % self.__root)) is not None:
                    self.warn(
                        "It seems you have Visual Studio 6 installed, but the expected registry settings are not present.\nYou must at least run the Visual Studio GUI once so that these entries are created."
                    )
                    break
        return []
