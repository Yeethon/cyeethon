    ...
    for name in pickle.__all__:
        ...
        if (not isinstance(picklecode, bytes)) or (len(picklecode) != 1):
            if verbose:
                print(
                    (
                        "skipping %r: value %r doesn't look like a pickle code"
                        % (name, picklecode)
                    )
                )
            continue
        ...
        if picklecode in copy:
            ...
            if d.name != name:
                raise ValueError(
                    (
                        "for pickle code %r, pickle.py uses name %r but we're using name %r"
            ...
            raise ValueError(
                (
                    "pickle.py appears to have a pickle opcode with name %r and code %r, but we don't"
                    % (name, picklecode)
                )
            )
    if copy:
...
def dis(pickle, out=None, memo=None, indentlevel=4, annotate=0):
    ...
    for (opcode, arg, pos) in genops(pickle):
        ...
        if len(stack) < numtopop:
            raise ValueError(
                (
                    "tries to pop %d items from stack with only %d items"
                    % (numtopop, len(stack))
                )
            )
        if numtopop:
