        ...
        for token in self:
            if not hasattr(token, "_pp"):
                (
                    yield (
                        indent
                        + "    !! invalid element in token list: {!r}".format(token)
                    )
                )
            else:
...
class LocalPart(TokenList):
    ...
    def local_part(self):
        ...
        for tok in self[0] + [DOT]:
            ...
            if (
                last_is_tl
                and (tok.token_type == "dot")
                and (last[(-1)].token_type == "cfws")
            ):
...
_non_token_end_matcher = re.compile(
    "[^{}]+".format(re.escape("".join(TOKEN_ENDS)))
).match
_non_attribute_end_matcher = re.compile(
    "[^{}]+".format(re.escape("".join(ATTRIBUTE_ENDS)))
).match
_non_extended_attribute_end_matcher = re.compile(
    "[^{}]+".format(re.escape("".join(EXTENDED_ATTRIBUTE_ENDS)))
).match
...
def get_obs_local_part(value):
    ...
    if (obs_local_part[0].token_type == "dot") or (
        (obs_local_part[0].token_type == "cfws")
    ...
    if (obs_local_part[(-1)].token_type == "dot") or (
        (obs_local_part[(-1)].token_type == "cfws")
...
def get_no_fold_literal(value):
    ...
    if value[0] != "[":
        raise errors.HeaderParseError(
            "expected '[' at the start of no-fold-literal but found '{}'".format(value)
        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise errors.HeaderParseError(
                    "expected dot-atom-text, no-fold-literal or obs-id-right but found '{}'".format(
                        value
                    )
                )
    msg_id.append(token)
...
def get_parameter(value):
    ...
    if not value:
        ...
        if value[0] != "'":
            raise errors.HeaderParseError(
                "Expected RFC2231 char/lang encoding delimiter, but found {!r}".format(
                    value
                )
            )
        ...
        if value and (value[0] != "'"):
            ...
            if (not value) or (value[0] != "'"):
                raise errors.HeaderParseError(
                    "Expected RFC2231 char/lang encoding delimiter, but found {}".format(
                        value
                    )
                )
        appendto.append(ValueTerminal("'", "RFC2231-delimiter"))
...
def parse_content_type_header(value):
    ...
    if value[0] != ";":
        ctype.defects.append(
            errors.InvalidHeaderDefect(
                "Only parameters are valid after content type, but found {!r}".format(
                    value
                )
            )
        )
...
def parse_content_disposition_header(value):
    ...
    if value[0] != ";":
        disp_header.defects.append(
            errors.InvalidHeaderDefect(
                "Only parameters are valid after content disposition, but found {!r}".format(
                    value
                )
            )
        )
