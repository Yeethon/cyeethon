...
def main():
    ...
    grp.add_argument(
        ...
        "--count",
        action="store_true",
        help="Count the number of times each line is executed and write the counts to <module>.cover for each module executed, in the module's directory. See also --coverdir, --file, --no-report below.",
    )
    grp.add_argument(
        "-t",
        "--trace",
    ...
    grp.add_argument(
        ...
        "--listfuncs",
        action="store_true",
        help="Keep track of which functions are executed at least once and write the results to sys.stdout after the program exits. Cannot be specified alongside --trace or --count.",
    )
    grp.add_argument(
        "-T",
        "--trackcalls",
    ...
    _grp.add_argument(
        ...
        "--report",
        action="store_true",
        help="Generate a report from a counts file; does not execute any code. --file must specify the results file to read, which must have been created in a previous run with --count --file=FILE",
    )
    _grp.add_argument(
        "-R",
        "--no-report",
    ...
    grp.add_argument(
        "-C",
        "--coverdir",
        help="Directory where the report files go. The coverage report for <package>.<module> will be written to file <dir>/<package>/<module>.cover",
    )
    grp.add_argument(
        "-m",
        "--missing",
    ...
    grp.add_argument(
        ...
        action="append",
        default=[],
        help="Ignore the given module(s) and its submodules (if it is a package). Accepts comma separated list of module names.",
    )
    grp.add_argument(
        "--ignore-dir",
        action="append",
        default=[],
        help="Ignore files in the given directory (multiple directories can be joined by os.pathsep).",
    )
    parser.add_argument(
