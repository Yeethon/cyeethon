            ...
            try:
                ...
                if not getattr(self.root, "recentfiles_message", False):
                    ...
                    messagebox.showwarning(
                        title="IDLE Warning",
                        message=f"""Cannot save Recent Files list to disk.
  {err}
Select OK to continue.""",
    ...
    def smart_backspace_event(self, event):
        ...
        ncharsdeleted = 0
        while 1:
            if chars == self.prompt_last_line:
                break
            chars = chars[:(-1)]
...
keynames = {"bracketleft": "[", "bracketright": "]", "slash": "/"}
...
def get_accelerator(keydefs, eventname):
    ...
    if (not keylist) or (
        macosx.isCocoaTk()
        and (
            eventname in {"<<open-module>>", "<<goto-line>>", "<<change-indentwidth>>"}
        )
    ):
        return ""
    s = keylist[0]
