...
def get_widget_padding(widget):
    ...
    padx = sum(
        map(
            widget.tk.getint, [info["padx"], widget.cget("padx"), widget.cget("border")]
        )
    )
    pady = sum(
        map(
            widget.tk.getint, [info["pady"], widget.cget("pady"), widget.cget("border")]
        )
    )
    return (padx, pady)
...
class LineNumbers(BaseSideBar):
    ...
    def bind_events(self):
        ...
        def drag_update_selection_and_insert_mark(y_coord):
            ...
            self.text.mark_set(
                "insert", f"{(lineno if (lineno == a) else (lineno + 1))}.0"
            )
    ...
    def update_sidebar_text(self, end):
        ...
        if end > self.prev_end:
            new_text = "\n".join(
                itertools.chain([""], map(str, range((self.prev_end + 1), (end + 1))))
            )
...
def _linenumbers_drag_scrolling(parent):
