...
def _signature_from_function(
    cls, func, skip_bound_arg=True, globalns=None, localns=None
):
    ...
    def _bind(self, args, kwargs, *, partial=False):
        ...
        for param in itertools.chain(parameters_ex, parameters):
            ...
            try:
                ...
                if param.kind == _POSITIONAL_ONLY:
                    raise TypeError(
                        "{arg!r} parameter is positional only, but was passed as a keyword".format(
                            arg=param.name
                        )
                    )
...
def signature(obj, *, follow_wrapped=True, globalns=None, localns=None):
    ...
    return Signature.from_callable(
        obj, follow_wrapped=follow_wrapped, globalns=globalns, localns=localns
    )
