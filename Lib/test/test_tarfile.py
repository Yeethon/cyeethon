...
class ListTest(ReadTest, unittest.TestCase):
    ...
    def test_list_verbose(self):
        ...
        self.assertRegex(
            out,
            (
                b"\\?rw-r--r-- tarfile/tarfile\\s+7011 \\d{4}-\\d\\d-\\d\\d\\s+\\d\\d:\\d\\d:\\d\\d ustar/\\w+type ?\\r?\\n"
                * 2
            ),
        )
        ...
        self.assertIn(
            (
                (
                    ((b"gnu" + (b"/123" * 125)) + b"/longlink link to gnu")
                    + (b"/123" * 125)
                )
                + b"/longname"
            ),
        ...
        self.assertIn(
            (
                (
                    ((b"pax" + (b"/123" * 125)) + b"/longlink link to pax")
                    + (b"/123" * 125)
                )
                + b"/longname"
            ),
...
class MiscReadTestBase(CommonReadTest):
    ...
    def test_check_members(self):
        for tarinfo in self.tar:
            self.assertEqual(
                int(tarinfo.mtime), 1041808783, ("wrong mtime for %s" % tarinfo.name)
            )
        ...
        try:
            ...
            for tarinfo in directories:
                ...
                if sys.platform != "win32":
                    self.assertEqual(
                        (tarinfo.mode & 511), (os.stat(path).st_mode & 511)
                    )
    ...
    def test_extractall_pathlike_name(self):
        ...
        with os_helper.temp_dir(DIR), tarfile.open(
            tarname, encoding="iso8859-1"
        ) as tar:
            directories = [t for t in tar if t.isdir()]
            tar.extractall(DIR, directories)
            for tarinfo in directories:
    ...
    def test_stream_padding(self):
        ...
        if self.decompressor:
            ...
            with self.open(tmpname) as fobj:
                data = fobj.read()
        self.assertEqual(
            data.count(b"\x00"), tarfile.RECORDSIZE, "incorrect zero padding"
        )

    @unittest.skipUnless(
        ((sys.platform != "win32") and hasattr(os, "umask")),
...
class GNUWriteTest(unittest.TestCase):
    ...
    def test_longnamelink_1025(self):
        self._test(
            (("longnam/" * 127) + "longname_"), (("longlnk/" * 127) + "longlink_")
        )
...
class UstarUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def test_unicode_longname1(self):
        ...
        self._test_ustar_name(
            ((("0123456789" * 15) + "0123ÿ/") + ("0123456789" * 10)), ValueError
        )

    def test_unicode_longname2(self):
        self._test_ustar_name(
            ((("0123456789" * 15) + "01ÿ/2") + ("0123456789" * 10)), ValueError
        )
        self._test_ustar_name(
            ((("0123456789" * 15) + "01ÿÿ/") + ("0123456789" * 10)), ValueError
        )

    def test_unicode_longname3(self):
        self._test_ustar_name(
            ((("0123456789" * 15) + "01ÿÿ/2") + ("0123456789" * 10)), ValueError
        )
        self._test_ustar_name(
            (((("0123456789" * 15) + "01234/") + ("0123456789" * 9)) + "01234567ÿ")
        )
        self._test_ustar_name(
            (((("0123456789" * 15) + "01234/") + ("0123456789" * 9)) + "012345678ÿ"),
            ValueError,
        )

    def test_unicode_longname4(self):
        self._test_ustar_name(
            (((("0123456789" * 15) + "01234/") + ("0123456789" * 9)) + "012345ÿÿ")
        )
        self._test_ustar_name(
            (((("0123456789" * 15) + "01234/") + ("0123456789" * 9)) + "0123456ÿÿ"),
            ValueError,
        )
...
class MiscTest(unittest.TestCase):
    def test_char_fields(self):
        self.assertEqual(
            tarfile.stn("foo", 8, "ascii", "strict"), b"foo\x00\x00\x00\x00\x00"
        )
        self.assertEqual(tarfile.stn("foobar", 3, "ascii", "strict"), b"foo")
        self.assertEqual(
            tarfile.nts(b"foo\x00\x00\x00\x00\x00", "ascii", "strict"), "foo"
        )
    ...
    def test_read_number_fields(self):
        ...
        self.assertEqual(
            tarfile.nti(b"\xff\x00\x00\x00\x00\x00\x00\x00"), (-72057594037927936)
        )
    ...
    def test_write_number_fields(self):
        ...
        self.assertEqual(
            tarfile.itn(2097152, format=tarfile.GNU_FORMAT),
            b"\x80\x00\x00\x00\x00 \x00\x00",
        )
        self.assertEqual(
            tarfile.itn(4294967295, format=tarfile.GNU_FORMAT),
        ...
        self.assertEqual(
            tarfile.itn((-72057594037927936), format=tarfile.GNU_FORMAT),
