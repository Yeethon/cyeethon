...
class BaseTaskTests():
    ...
    def test_other_loop_future(self):
        ...
        async def run(fut):
            (await fut)
        try:
            with self.assertRaisesRegex(RuntimeError, 'Task .* got Future .* attached'):
    ...
    def test_ensure_future_error_msg(self):
        ...
        with self.assertRaisesRegex(ValueError, 'The future belongs to a different loop than the one specified as the loop argument'):
    ...
    def test_get_stack(self):
        ...
        async def bar():
            ...
            file.seek(0)
            tb = file.read()
            self.assertRegex(tb, 'foo\\(\\) running')
    ...
    def test_task_repr(self):
        ...
        self.assertRegex(notmuch.__qualname__, '\\w+.test_task_repr.<locals>.notmuch')
        ...
        self.assertEqual(repr(t), ("<Task pending name='TestTask' %s cb=[<Dummy>()]>" % coro))
        t.cancel()
        self.assertEqual(repr(t), ("<Task cancelling name='TestTask' %s cb=[<Dummy>()]>" % coro))
        ...
        self.loop.run_until_complete(t)
        coro = format_coroutine(coro_qualname, 'done', src, t._source_traceback)
        self.assertEqual(repr(t), ("<Task finished name='TestTask' %s result='abc'>" % coro))
    ...
    def test_task_repr_coro_decorator(self):
        ...
        with self.assertWarns(DeprecationWarning):
            ...
            def notmuch():
                return 123
        self.assertEqual(notmuch.__name__, 'notmuch')
        self.assertRegex(notmuch.__qualname__, '\\w+.test_task_repr_coro_decorator\\.<locals>\\.notmuch')
        self.assertEqual(notmuch.__module__, __name__)
        gen = notmuch()
        coro_name = 'notmuch'
        ...
        if coroutines._DEBUG:
            if coroutines._DEBUG:
                ...
                coro = ('%s() running, defined at %s:%s, created at %s:%s' % (coro_qualname, filename, lineno, frame[0], frame[1]))
            else:
                code = gen.gi_code
                coro = ('%s() running at %s:%s' % (coro_qualname, code.co_filename, code.co_firstlineno))
        ...
        coro = format_coroutine(coro_qualname, 'running', src, t._source_traceback, generator=(not coroutines._DEBUG))
        self.assertEqual(repr(t), ("<Task pending name='TestTask' %s cb=[<Dummy>()]>" % coro))
        self.loop.run_until_complete(t)
    ...
    def test_task_repr_partial_corowrapper(self):
        ...
        expected = '<coroutine object \\w+\\.test_task_repr_partial_corowrapper\\.<locals>\\.func at'
    ...
    def test_cancel_with_message_then_future_result(self):
        cases = [((), ()), ((None,), ()), (('my message',), ('my message',)), ((5,), (5,))]
        for (cancel_args, expected_args) in cases:
            with self.subTest(cancel_args=cancel_args):
                ...
                self.assertEqual(actual, (asyncio.CancelledError, expected_args, 2))
    ...
    def test_cancel_with_message_before_starting_task(self):
        ...
        self.assertEqual(exc.args, ())
        actual = get_innermost_context(exc)
        self.assertEqual(actual, (asyncio.CancelledError, ('my message',), 2))
    ...
    def test_cancel_task_catching(self):
        ...
        t = self.new_task(self.loop, task())
        test_utils.run_briefly(self.loop)
        self.assertIs(t._fut_waiter, fut1)
        fut1.set_result(None)
        test_utils.run_briefly(self.loop)
        self.assertIs(t._fut_waiter, fut2)
        t.cancel()
    ...
    def test_cancel_task_ignoring(self):
        ...
        self.assertTrue(fut2.cancelled())
        test_utils.run_briefly(self.loop)
        self.assertIs(t._fut_waiter, fut3)
        fut3.set_result(42)
        res = self.loop.run_until_complete(t)
        self.assertEqual(res, 42)
    ...
    def test_cancel_current_task(self):
        ...
        async def task():
            t.cancel()
            self.assertTrue(t._must_cancel)
        ...
        self.assertTrue(t.done())
        self.assertTrue(t.cancelled())
        self.assertFalse(t._must_cancel)
        self.assertFalse(t.cancel())
...
>>>{text}<<<''')
    ...
    def test_wait_duplicate_coroutines(self):
        with self.assertWarns(DeprecationWarning):
            ...
            def coro(s):
                return s
        c = coro('test')
        task = self.new_task(self.loop, asyncio.wait([c, c, coro('spam')]))
        with self.assertWarns(DeprecationWarning):
    ...
    def test_wait_first_completed(self):
        ...
        task = self.new_task(loop, asyncio.wait([b, a], return_when=asyncio.FIRST_COMPLETED))
    ...
    def test_wait_really_done(self):
        ...
        task = self.new_task(self.loop, asyncio.wait([b, a], return_when=asyncio.FIRST_COMPLETED))
        (done, pending) = self.loop.run_until_complete(task)
    ...
    def test_wait_first_exception(self):
        ...
        task = self.new_task(loop, asyncio.wait([b, a], return_when=asyncio.FIRST_EXCEPTION))
        (done, pending) = loop.run_until_complete(task)
        self.assertEqual({b}, done)
        self.assertEqual({a}, pending)
        self.assertAlmostEqual(0, loop.time())
    ...
    def test_as_completed_invalid_args(self):
        fut = self.new_future(self.loop)
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.as_completed(fut))
        coro = coroutine_function()
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.as_completed(coro))
        coro.close()
    ...
    def test_log_destroyed_pending_task(self):
        ...
        mock_handler.assert_called_with(self.loop, {'message': 'Task was destroyed but it is pending!', 'task': mock.ANY, 'source_traceback': source_traceback})
    ...
    def test_coroutine_never_yielded(self, m_log):
        ...
        regex = ('^<CoroWrapper %s\\(?\\)? .* at %s:%s, .*> was never yielded from\\nCoroutine object created at \\(most recent call last, truncated to \\d+ last lines\\):\\n.*\\n  File "%s", line %s, in test_coroutine_never_yielded\\n    coro_noop\\(\\)$' % (re.escape(coro_noop.__qualname__), re.escape(func_filename), func_lineno, re.escape(tb_filename), tb_lineno))
    ...
    def test_task_source_traceback(self):
        ...
        self.assertEqual(task._source_traceback[(- 2)][:3], (__file__, lineno, 'test_task_source_traceback'))
        self.loop.run_until_complete(task)
        ...
        for (cancel_args, expected_args) in cases:
            with self.subTest(cancel_args=cancel_args):
                ...
                async def test():
                    ...
                    while True:
                        time += 0.05
                        (await asyncio.gather(asyncio.sleep(0.05), return_exceptions=True))
                        if (time > 1):
                            return
                ...
                try:
                    ...
                    self.assertEqual((exc_type, exc_args), (asyncio.CancelledError, expected_args))
...
class SetMethodsTest():
    ...
    def test_set_result_causes_invalid_state(self):
        ...
        with self.assertRaisesRegex(asyncio.InvalidStateError, 'step\\(\\): already done'):
            raise exc
        coro.close()
...
@unittest.skipUnless((hasattr(futures, '_CFuture') and hasattr(tasks, '_CTask')), 'requires the C _asyncio module')
...
@unittest.skipUnless(hasattr(tasks, '_c_register_task'), 'requires the C _asyncio module')
...
class GatherTestsBase():
    ...
    def test_env_var_debug(self):
        ...
        (sts, stdout, stderr) = assert_python_ok('-E', '-c', code, PYTHONASYNCIODEBUG='1', PYTHONDEVMODE='')
        self.assertEqual(stdout.rstrip(), b'False')
...
class WaitTests(test_utils.TestCase):
    ...
    def test_coro_is_deprecated_in_wait(self):
        ...
        with self.assertWarns(DeprecationWarning):
            self.loop.run_until_complete(asyncio.wait([task, coroutine_function()]))
...
class CompatibilityTests(test_utils.TestCase):
    ...
    def test_debug_mode_interop(self):
        ...
        assert_python_ok('-Wignore::DeprecationWarning', '-c', code, PYTHONASYNCIODEBUG='1')
