...
class StreamTests(test_utils.TestCase):
    ...
    def test_open_unix_connection_no_loop_ssl(self):
        with test_utils.run_test_unix_server(use_ssl=True) as httpd:
            conn_fut = asyncio.open_unix_connection(
                httpd.address, ssl=test_utils.dummy_ssl_context(), server_hostname=""
            )
            self._basetest_open_connection_no_loop_ssl(conn_fut)
    ...
    def test_readuntil_eof(self):
        ...
        with self.assertRaisesRegex(
            asyncio.IncompleteReadError, "undefined expected bytes"
        ) as cm:
    ...
    def test_readexactly_eof(self):
        ...
        self.assertEqual(
            str(cm.exception), "18 bytes read on a total of 36 expected bytes"
        )
        self.assertEqual(b"", stream._buffer)
            ...
            def start_callback(self):
                ...
                self.server = self.loop.run_until_complete(
                    asyncio.start_server(
                        self.handle_client_callback, host=addr[0], port=addr[1]
                    )
                )
                return addr
    ...
    def test_start_unix_server(self):
        class MyServer:
            ...
            def start(self):
                self.server = self.loop.run_until_complete(
                    asyncio.start_unix_server(self.handle_client, path=self.path)
                )
            ...
            def start_callback(self):
                start = asyncio.start_unix_server(
                    self.handle_client_callback, path=self.path
                )
        ...
        with test_utils.unix_socket_path() as path:
            ...
            server.start()
            msg = self.loop.run_until_complete(self.loop.create_task(client(path)))
            server.stop()
            self.assertEqual(msg, b"hello world!\n")
        with test_utils.unix_socket_path() as path:
            server = MyServer(self.loop, path)
    ...
    def test_read_all_from_pipe_reader(self):
        ...
        try:
            ...
            create = asyncio.create_subprocess_exec(*args, pass_fds={wfd})
    ...
    def test___repr__waiter(self):
        ...
        self.assertRegex(repr(stream), "<StreamReader waiter=<Future pending[\\S ]*>>")
