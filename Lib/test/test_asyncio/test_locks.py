...
STR_RGX_REPR = "^<(?P<class>.*?) object at (?P<address>.*?)\\[(?P<extras>(set|unset|locked|unlocked)(, value:\\d)?(, waiters:\\d+)?)\\]>\\Z"
    ...
    def test_lock_doesnt_accept_loop_parameter(self):
        ...
        for cls in primitives_cls:
            with self.assertRaisesRegex(
                TypeError,
                f"As of 3.10, the \*loop\* parameter was removed from {cls.__name__}\(\) since it is no longer necessary",
            ):
                cls(loop=self.loop)
...
class SemaphoreTests(test_utils.TestCase):
    ...
    def test_semaphore(self):
        ...
        with self.assertRaisesRegex(TypeError, "'Semaphore' object is not iterable"):
            self.loop.run_until_complete(acquire_lock())
