...
class SubprocessMixin:
    ...
    def test_start_new_session(self):
        proc = self.loop.run_until_complete(
            asyncio.create_subprocess_shell("exit 8", start_new_session=True)
        )
        exitcode = self.loop.run_until_complete(proc.wait())
    ...
    def test_send_signal(self):
        ...
        try:
            ...
            proc = self.loop.run_until_complete(
                asyncio.create_subprocess_exec(*args, stdout=subprocess.PIPE)
            )
...
if sys.platform != "win32":
    ...
    @unittest.skipUnless(
        has_pidfd_support(), "operating system does not support pidfds"
    )
