...
class ProactorSocketTransportTests(test_utils.TestCase):
    ...
    def socket_transport(self, waiter=None):
        transport = _ProactorSocketTransport(
            self.loop, self.sock, self.protocol, waiter=waiter
        )
    ...
    def test_ctor(self):
        ...
        self.proactor.recv_into.assert_called_with(
            self.sock, bytearray(self.buffer_size)
        )
    ...
    def test_loop_writing(self):
        ...
        self.loop._proactor.send.return_value.add_done_callback.assert_called_with(
            tr._loop_writing
        )
...
class ProactorDatagramTransportTests(test_utils.TestCase):
    ...
    def datagram_transport(self, address=None):
        ...
        transport = _ProactorDatagramTransport(
    ...
    def test_sendto_exception(self, m_log):
        ...
        transport._fatal_error.assert_called_with(
            err, "Fatal write error on datagram transport"
        )
...
class BaseProactorEventLoopTests(test_utils.TestCase):
    ...
    def test_datagram_loop_writing(self):
        ...
        tr._loop_writing()
        self.loop._proactor.sendto.assert_called_with(
            self.sock, b"data", addr=("127.0.0.1", 12068)
        )
        self.loop._proactor.sendto.return_value.add_done_callback.assert_called_with(
            tr._loop_writing
        )
        close_transport(tr)
