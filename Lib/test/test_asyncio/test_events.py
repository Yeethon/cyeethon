...
class EventLoopTestsMixin:
    ...
    def test_run_until_complete_nesting(self):
        ...
        with self.assertWarnsRegex(RuntimeWarning, "coroutine \\S+ was never awaited"):
    ...
    def test_ssl_connect_accepted_socket(self):
        if (
            ...
            and isinstance(self.loop, proactor_events.BaseProactorEventLoop)
        ):
...
class HandleTests(test_utils.TestCase):
    ...
    def test_handle_repr(self):
        ...
        regex = "^<Handle noop\\(1, 2\\)\\(3\\) at %s:%s>$" % (
            re.escape(filename),
            lineno,
        )
        ...
        regex = "^<Handle noop\\(x=1\\)\\(2, 3\\) at %s:%s>$" % (
            re.escape(filename),
            lineno,
        )
        self.assertRegex(repr(h), regex)
        if sys.version_info >= (3, 4):
            ...
            h = asyncio.Handle(cb, (), self.loop)
            cb_regex = "<function HandleTests.test_handle_repr .*>"
            cb_regex = "functools.partialmethod\\(%s, , \\)\\(\\)" % cb_regex
            regex = "^<Handle %s at %s:%s>$" % (cb_regex, re.escape(filename), lineno)
            self.assertRegex(repr(h), regex)
...
class TimerTests(unittest.TestCase):
    ...
    def test_timer(self):
        ...
        self.assertRaises(
            AssertionError, asyncio.TimerHandle, None, callback, args, self.loop
        )
    ...
    def test_timer_repr_debug(self):
        ...
        self.assertEqual(
            repr(h),
            (
                "<TimerHandle when=123 noop() at %s:%s created at %s:%s>"
                % (filename, lineno, create_filename, create_lineno)
            ),
        )
        h.cancel()
        self.assertEqual(
            repr(h),
            (
                "<TimerHandle cancelled when=123 noop() at %s:%s created at %s:%s>"
                % (filename, lineno, create_filename, create_lineno)
            ),
        )
