...
class BaseFutureTests:
    ...
    def test_yield_from_twice(self):
        ...
        self.assertEqual(next(g), f)
        f.set_result(42)
        self.assertEqual(next(g), ("B", 42))
        self.assertEqual(next(g), ("C", 42))
    ...
    def test_future_repr(self):
        ...
        self.assertRegex(
            ...
            (("<" + self.cls.__name__) + (" pending cb=\\[%s\\]>" % fake_repr)),
        )
        ...
        self.assertRegex(
            ...
            (
                ("<" + self.cls.__name__)
                + (" pending cb=\\[%s, %s\\]>" % (first_repr, last_repr))
            ),
        )
        ...
        cb_regex = "%s, <8 more>, %s" % (first_repr, last_repr)
        self.assertRegex(
            repr(f_many_callbacks),
            (("<" + self.cls.__name__) + (" pending cb=\\[%s\\]>" % cb_regex)),
        )
...
"""
            ...
            regex = "^Future/Task exception was never retrieved\\n"
