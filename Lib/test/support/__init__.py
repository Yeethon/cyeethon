...
def _is_full_match_test(pattern):
    return ("." in pattern) and (not re.search("[?*\\[\\]]", pattern))
...
def suppress_msvcrt_asserts(verbose=False):
    ...
    msvcrt.SetErrorMode(
        (
            (
                (msvcrt.SEM_FAILCRITICALERRORS | msvcrt.SEM_NOALIGNMENTFAULTEXCEPT)
...
def wait_process(pid, *, exitcode, timeout=None):
    ...
    if exitcode2 != exitcode:
        raise AssertionError(
            f"process {pid} exited with code {exitcode2}, but exit code {exitcode} is expected"
        )
    if pid2 != pid:
