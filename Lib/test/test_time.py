...
class TimeTestCase(unittest.TestCase):
    ...
    @unittest.skipUnless(
        hasattr(time, "pthread_getcpuclockid"), "need time.pthread_getcpuclockid()"
    )
    @unittest.skipUnless(hasattr(time, "clock_gettime"), "need time.clock_gettime()")
    ...
    def test_conversions(self):
        self.assertEqual(time.ctime(self.t), time.asctime(time.localtime(self.t)))
    ...
    def test_asctime(self):
        ...
        self.assertRaises(
            OverflowError, time.asctime, (((TIME_MAXYEAR + 1),) + ((0,) * 8))
        )
        self.assertRaises(
            OverflowError, time.asctime, (((TIME_MINYEAR - 1),) + ((0,) * 8))
        )
...
class TestPytime(unittest.TestCase):
    ...
    def test_localtime_timezone(self):
        ...
        if lt.tm_gmtoff is None:
            ...
            self.assertEqual(
                lt.tm_gmtoff, (-[time.timezone, time.altzone][lt.tm_isdst])
            )
...
class CPyTimeTestCase:
    ...
    def _check_rounding(
        self, pytime_converter, expected_func, use_float, unit_to_sec, value_filter=None
    ):
    ...
    def check_int_rounding(
        self, pytime_converter, expected_func, unit_to_sec=1, value_filter=None
    ):
        self._check_rounding(
            pytime_converter, expected_func, False, unit_to_sec, value_filter
        )

    def check_float_rounding(
        self, pytime_converter, expected_func, unit_to_sec=1, value_filter=None
    ):
        self._check_rounding(
            pytime_converter, expected_func, True, unit_to_sec, value_filter
        )
...
class TestCPyTime(CPyTimeTestCase, unittest.TestCase):
    ...
    def test_FromSeconds(self):
        ...
        self.check_int_rounding(
            (lambda secs, rnd: PyTime_FromSeconds(secs)),
    ...
    def test_FromSecondsObject(self):
        ...
        self.check_int_rounding(
            PyTime_FromSecondsObject, (lambda secs: (secs * SEC_TO_NS))
        )
        self.check_float_rounding(
            PyTime_FromSecondsObject, (lambda ns: self.decimal_round((ns * SEC_TO_NS)))
        )
        for (time_rnd, _) in ROUNDING_MODES:
    ...
    def test_AsSecondsDouble(self):
        ...
        self.check_int_rounding(
            (lambda ns, rnd: PyTime_AsSecondsDouble(ns)), float_converter, NS_TO_SEC
        )
        for (time_rnd, _) in ROUNDING_MODES:
            with self.assertRaises(TypeError):
    ...
    def test_AsTimeval(self):
        ...
        self.check_int_rounding(
            PyTime_AsTimeval, timeval_converter, NS_TO_SEC, value_filter=seconds_filter
        )

    @unittest.skipUnless(
        hasattr(_testcapi, "PyTime_AsTimespec"), "need _testcapi.PyTime_AsTimespec"
    )
    def test_AsTimespec(self):
        ...
        self.check_int_rounding(
            (lambda ns, rnd: PyTime_AsTimespec(ns)),
            timespec_converter,
            NS_TO_SEC,
    ...
    def test_AsMilliseconds(self):
        ...
        self.check_int_rounding(
            PyTime_AsMilliseconds, self.create_decimal_converter(MS_TO_NS), NS_TO_SEC
        )
    ...
    def test_AsMicroseconds(self):
        ...
        self.check_int_rounding(
            PyTime_AsMicroseconds, self.create_decimal_converter(US_TO_NS), NS_TO_SEC
        )
...
class TestOldPyTime(CPyTimeTestCase, unittest.TestCase):
    ...
    def test_object_to_time_t(self):
        ...
        self.check_float_rounding(
            pytime_object_to_time_t, self.decimal_round, value_filter=self.time_t_filter
        )
