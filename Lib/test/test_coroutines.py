...
class CoroutineTest(unittest.TestCase):
    ...
    def test_await_9(self):
        ...
        async def foo():
            ...
            return (
                (((await bar()) + (await wrap()())) + (await db["b"]()()()))
                + ((await bar()) * 1000)
            ) + (await DB.b()())
    ...
    def test_with_1(self):
        class Manager:
            ...
            async def __aenter__(self):
                (
                    await AsyncYieldFrom(
                        [("enter-1-" + self.name), ("enter-2-" + self.name)]
                    )
                )
                return self
            ...
            async def __aexit__(self, *args):
                (
                    await AsyncYieldFrom(
                        [("exit-1-" + self.name), ("exit-2-" + self.name)]
                    )
                )
                if self.name == "B":
                    return True
    ...
    def test_for_3(self):
        ...
        with self.assertRaisesRegex(TypeError, "that does not implement __anext__"):
            run_async(foo())
