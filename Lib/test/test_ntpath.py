...
class TestNtpath(NtpathTestCase):
    ...
    def test_realpath_broken_symlinks(self):
        ...
        with os_helper.change_cwd(ABSTFN):
            ...
            self.assertPathEqual(
                ntpath.realpath("broken\\foo"), (ABSTFN + "\\missing\\foo")
            )
            self.assertPathEqual(ntpath.realpath("broken1"), (ABSTFN + "\\broken\\bar"))
            self.assertPathEqual(
                ntpath.realpath("broken1\\baz"), (ABSTFN + "\\broken\\bar\\baz")
            )
            self.assertPathEqual(
                ntpath.realpath("broken2"), (ABSTFN + "\\self\\self\\missing")
            )
            self.assertPathEqual(
                ntpath.realpath("broken3"),
                (ABSTFN + "\\subdir\\parent\\subdir\\parent\\missing"),
            )
            self.assertPathEqual(ntpath.realpath("broken4"), (ABSTFN + "\\missing"))
            self.assertPathEqual(ntpath.realpath("broken5"), (ABSTFN + "\\missing"))
            self.assertPathEqual(
                ntpath.realpath(b"broken"), os.fsencode((ABSTFN + "\\missing"))
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken\\foo"),
                os.fsencode((ABSTFN + "\\missing\\foo")),
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken1"), os.fsencode((ABSTFN + "\\broken\\bar"))
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken1\\baz"),
                os.fsencode((ABSTFN + "\\broken\\bar\\baz")),
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken2"),
                os.fsencode((ABSTFN + "\\self\\self\\missing")),
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken3"),
                os.fsencode((ABSTFN + "\\subdir\\parent\\subdir\\parent\\missing")),
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken4"), os.fsencode((ABSTFN + "\\missing"))
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken5"), os.fsencode((ABSTFN + "\\missing"))
            )
    ...
    def test_realpath_symlink_loops(self):
        ...
        self.assertPathEqual(
            ntpath.realpath((ABSTFN + "1\\..")), ntpath.dirname(ABSTFN)
        )
        self.assertPathEqual(
    ...
    def test_realpath_symlink_prefix(self):
        ...
        self.assertPathEqual(
            ntpath.realpath((("\\\\?\\" + ABSTFN) + "3link")),
            (("\\\\?\\" + ABSTFN) + "3"),
        )
        self.assertPathEqual(
            ntpath.realpath((("\\\\?\\" + ABSTFN) + "3.link")),
            (("\\\\?\\" + ABSTFN) + "3."),
        )
