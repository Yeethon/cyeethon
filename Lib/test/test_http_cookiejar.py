...
class DateTimeTests(unittest.TestCase):
    ...
    def test_time2netscape(self):
        ...
        self.assertEqual(
            time2netscape((base + (2 * day))), "Sun, 21-Apr-2002 14:36:40 GMT"
        )
        self.assertEqual(
            time2netscape((base + (3 * day))), "Mon, 22-Apr-2002 14:36:40 GMT"
        )
        az = time2netscape()
    ...
    def test_iso2time(self):
        ...
        self.assertEqual(
            parse_date("1994-02-03 07:15:29 -0700"), (1994, 2, 3, 14, 15, 29)
        )
        self.assertEqual(
            parse_date("1994-02-03 19:45:29 +0530"), (1994, 2, 3, 14, 15, 29)
        )
...
class FileCookieJarTests(unittest.TestCase):
    ...
    def test_bad_magic(self):
        ...
        for cookiejar_class in (LWPCookieJar, MozillaCookieJar):
            ...
            try:
                c.load(
                    filename="for this test to work, a file with this filename should not exist"
                )
...
class CookieTests(unittest.TestCase):
    ...
    def test_missing_value(self):
        ...
        self.assertEqual(
            repr(c),
            re.sub(
                ("path_specified=%s" % True), ("path_specified=%s" % False), old_str
            ),
        )
        self.assertEqual(
    ...
    def test_rfc2109_handling(self):
        for (rfc2109_as_netscape, rfc2965, version) in [
            ...
            policy = DefaultCookiePolicy(
                rfc2109_as_netscape=rfc2109_as_netscape, rfc2965=rfc2965
            )
            c = CookieJar(policy)
    ...
    def test_expires(self):
        ...
        interact_netscape(
            c, "http://www.acme.com/", ('spam="bar"; expires=%s' % future)
        )
        ...
        interact_netscape(
            c, "http://www.acme.com/", ('eggs="bar"; expires=%s' % future)
        )
    ...
    def test_request_host(self):
        req = urllib.request.Request(
            "http://1.1.1.1/", headers={"Host": "www.acme.com:80"}
        )
    ...
    def test_wrong_domain(self):
        c = CookieJar()
        interact_2965(
            c, "http://www.nasty.com/", 'foo=bar; domain=friendly.org; Version="1"'
        )
    ...
    def test_strict_domain(self):
        ...
        interact_netscape(
            cj, "http://example.co.uk/", "okey=dokey; Domain=.example.co.uk"
        )
    ...
    def test_two_component_domain_rfc2965(self):
        ...
        interact_2965(
            c, "http://foo.net/foo", 'spam=eggs; domain=foo.net; path=/foo; Version="1"'
        )
        self.assertEqual(len(c), 1)
        self.assertEqual(interact_2965(c, "http://foo.net/foo"), "$Version=1; foo=bar")
        interact_2965(
            c, "http://www.foo.net/foo/", 'spam=eggs; domain=foo.net; Version="1"'
        )
    ...
    def test_port_mirror(self):
        ...
        h = interact_2965(c, url)
        self.assertRegex(
            h, "\\$Port([^=]|$)", "port with no value not returned with no value"
        )
        c = CookieJar(pol)
        url = "http://foo.bar.com/"
        ...
        h = interact_2965(c, url)
        self.assertIn(
            '$Port="80"', h, "port with single value not returned with single value"
        )
        c = CookieJar(pol)
        url = "http://foo.bar.com/"
    ...
    def test_no_return_comment(self):
        ...
        h = interact_2965(c, url)
        self.assertNotIn(
            "Comment", h, "Comment or CommentURL cookie-attributes returned to server"
        )
...
class LWPCookieTests(unittest.TestCase):
    ...
    def test_rejection(self):
        ...
        cookie = interact_2965(
            c, "http://www.sol.no/<oo/", 'foo8=bar; version=1; path="/%3coo"'
        )
    ...
    def test_intranet_domains_2965(self):
        ...
        cookie = interact_2965(
            c, "http://example/", 'foo2=bar; domain=".local"; Version=1'
        )
