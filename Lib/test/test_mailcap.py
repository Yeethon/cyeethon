...
MAILCAPDICT = {
    ...
    "image/x-xwindowdump": [{"view": "display %s", "lineno": 9}],
    "audio/*": [{"view": "/usr/local/bin/showaudio %t", "lineno": 7}],
    "video/*": [{"view": "animate %s", "lineno": 12}],
    "application/frame": [
        {"print": '"cat %s | lp"', "view": "showframe %s", "lineno": 0}
    ],
    "image/rgb": [{"view": "display %s", "lineno": 8}],
}
...
class FindmatchTest(unittest.TestCase):
    def test_findmatch(self):
        ...
        cases = [
            ...
            (
                [c, "audio/basic", "edit"],
                {},
                ("audiocompose /dev/null", audio_basic_entry),
            ),
            (
    ...
    @unittest.skipIf(
        (sys.platform == "vxworks"), "'test' command is not supported on VxWorks"
    )
    def test_test(self):
        caps = {
            "test/pass": [{"test": "test 1 -eq 1"}],
