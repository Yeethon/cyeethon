...
class PosixTester(unittest.TestCase):
    ...
    def test_stat_dir_fd(self):
        ...
        try:
            ...
            self.assertRaises(
                OverflowError, posix.stat, os_helper.TESTFN, dir_fd=(10 ** 20)
            )
        finally:
            posix.close(f)
    ...
    def test_utime_dir_fd(self):
        ...
        try:
            ...
            self.assertRaises(
                TypeError, posix.utime, os_helper.TESTFN, (None, None), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, os_helper.TESTFN, (now, None), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, os_helper.TESTFN, (None, now), dir_fd=f
            )
            self.assertRaises(
                TypeError, posix.utime, os_helper.TESTFN, (now, "x"), dir_fd=f
            )
            posix.utime(os_helper.TESTFN, (int(now), int(now)), dir_fd=f)
            posix.utime(os_helper.TESTFN, (now, now), dir_fd=f)
            posix.utime(
                os_helper.TESTFN,
                (int(now), int(((now - int(now)) * 1000000000.0))),
                dir_fd=f,
            )
            posix.utime(
                os_helper.TESTFN,
                dir_fd=f,
                times=(int(now), int(((now - int(now)) * 1000000000.0))),
            )
    ...
    @unittest.skipUnless(
        (os.link in os.supports_dir_fd), "test needs dir_fd support in os.link()"
    )
    ...
    def test_mkdir_dir_fd(self):
        ...
        try:
            ...
            posix.stat((os_helper.TESTFN + "dir"))
        finally:
            posix.close(f)
    ...
    def test_readlink_dir_fd(self):
        ...
        try:
            self.assertEqual(
                posix.readlink((os_helper.TESTFN + "link")),
    ...
    def test_rename_dir_fd(self):
        ...
        try:
            posix.rename(
                (os_helper.TESTFN + "ren"), os_helper.TESTFN, src_dir_fd=f, dst_dir_fd=f
            )
        except:
            posix.rename((os_helper.TESTFN + "ren"), os_helper.TESTFN)
            raise
        else:
            posix.stat(os_helper.TESTFN)
        finally:
            posix.close(f)
    ...
    def test_symlink_dir_fd(self):
        ...
        try:
            ...
            self.assertEqual(
                posix.readlink((os_helper.TESTFN + "link")), os_helper.TESTFN
            )
        finally:
            posix.close(f)
            os_helper.unlink((os_helper.TESTFN + "link"))
    ...
    def test_unlink_dir_fd(self):
        ...
        posix.stat((os_helper.TESTFN + "del"))
        try:
    ...
    @unittest.skipUnless(
        (os.mkfifo in os.supports_dir_fd), "test needs dir_fd support in os.mkfifo()"
    )
    def test_mkfifo_dir_fd(self):
        ...
        try:
            try:
                posix.mkfifo(os_helper.TESTFN, (stat.S_IRUSR | stat.S_IWUSR), dir_fd=f)
            except PermissionError as e:
                self.skipTest(("posix.mkfifo(): %s" % e))
            self.assertTrue(stat.S_ISFIFO(posix.stat(os_helper.TESTFN).st_mode))
        finally:
    ...
    def test_get_and_set_scheduler_and_param(self):
        possible_schedulers = [
            sched
            for (name, sched) in posix.__dict__.items()
    ...
    def test_bad_file_actions(self):
        ...
        with self.assertRaises(ValueError):
            self.spawn_func(
                ...
                os.environ,
                file_actions=[
                    (os.POSIX_SPAWN_OPEN, 3, (__file__ + "\x00"), os.O_RDONLY, 0)
                ],
            )
