...
MEMORY_SANITIZER = ("-fsanitize=memory" in _cflags) or (
        ...
        for [test, abilities] in tests:
            with self.subTest(test), test() as obj:
                ...
                if sys.platform.startswith("win") and (
                    test in (pipe_reader, pipe_writer)
                ):
                    continue
...
class TextIOWrapperTest(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertRegex(
            repr(t), ("<(%s\\.)?TextIOWrapper encoding='utf-8'>" % modname)
        )
        raw.name = "dummy"
        self.assertRegex(
            repr(t), ("<(%s\\.)?TextIOWrapper name='dummy' encoding='utf-8'>" % modname)
        )
        t.mode = "r"
        self.assertRegex(
            repr(t),
...
class MiscIOTest(unittest.TestCase):
    ...
    def test_io_after_close(self):
        for kwargs in [
            ...
            self.assertRaises(
                ValueError, f.write, (b"" if ("b" in kwargs["mode"]) else "")
            )
...
class CMiscIOTest(MiscIOTest):
    ...
    def check_daemon_threads_shutdown_deadlock(self, stream_name):
        ...
        err = res.err.decode()
        if res.rc != 0:
            pattern = "Fatal Python error: _enter_buffered_busy: could not acquire lock for <(_io\\.)?BufferedWriter name='<{stream_name}>'> at interpreter shutdown, possibly due to daemon threads".format_map(
                locals()
            )
