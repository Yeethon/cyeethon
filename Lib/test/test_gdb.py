...
def get_gdb_version():
    try:
        ...
        if proc.returncode:
            raise Exception(
                f"Command {' '.join(cmd)!r} failed with exit code {proc.returncode}: stdout={version!r} stderr={stderr!r}"
            )
    except OSError:
...
def cet_protection():
    ...
    return ("-mcet" in flags) and any(
        (
            (
                flag.startswith("-fcf-protection")
...
class DebuggerTests(unittest.TestCase):
    ...
    def get_stack_trace(
        ...
        if "PC not saved" in err:
            raise unittest.SkipTest(
                "gdb cannot walk the frame object because the Program Counter is not present"
            )
        for pattern in (
    ...
    def get_gdb_repr(self, source, cmds_after_breakpoint=None, import_site=False):
        ...
        m = re.search(
            "#0\\s+builtin_id\\s+\\(self\\=.*,\\s+v=\\s*(.*?)?\\)\\s+at\\s+\\S*[A-Za-z]+/[A-Za-z0-9_-]+\\.c",
            gdb_output,
            re.DOTALL,
        )
        if not m:
...
class PrettyPrintTests(DebuggerTests):
    ...
    def test_dicts(self):
        ...
        self.assertGdbRepr(
            {"foo": "bar", "douglas": 42}, "{'foo': 'bar', 'douglas': 42}"
        )
    ...
    def test_strings(self):
        ...
        if err or (not encoding):
            raise RuntimeError(
                f"unable to determine the preferred encoding of embedded Python in GDB: {err}"
            )
    ...
    def test_selfreferential_old_style_instance(self):
        ...
        self.assertTrue(
            re.match(
                "<Foo\\(an_attr=<\\.\\.\\.>\\) at remote 0x-?[0-9a-f]+>", gdb_repr
            ),
            ("Unexpected gdb representation: %r\n%s" % (gdb_repr, gdb_output)),
        )
...
class PyListTests(DebuggerTests):
    ...
    def test_basic_command(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-list"]
        )
        self.assertListing(
            "   5    \n   6    def bar(a, b, c):\n   7        baz(a, b, c)\n   8    \n   9    def baz(*args):\n >10        id(42)\n  11    \n  12    foo(1, 2, 3)\n",
            bt,
    ...
    def test_one_abs_arg(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-list 9"]
        )
        self.assertListing(
            "   9    def baz(*args):\n >10        id(42)\n  11    \n  12    foo(1, 2, 3)\n",
            bt,
    ...
    def test_two_abs_args(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-list 1,3"]
        )
        self.assertListing(
...
class StackNavigationTests(DebuggerTests):
    ...
    def test_pyup_command(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-up", "py-up"]
        )
    ...
    def test_down_at_bottom(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-down"]
        )
    ...
    def test_up_at_top(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=(["py-up"] * 5)
        )
...
class PyBtTests(DebuggerTests):
    ...
    def test_bt(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-bt"]
        )
        self.assertMultilineMatches(
            bt,
    ...
    def test_bt_full(self):
        ...
        bt = self.get_stack_trace(
            script=self.get_sample_script(), cmds_after_breakpoint=["py-bt-full"]
        )
        self.assertMultilineMatches(
            bt,
    ...
    def test_gc(self):
        ...
        gdb_output = self.get_stack_trace(
            cmd, cmds_after_breakpoint=["break update_refs", "continue", "py-bt"]
        )
        self.assertIn("Garbage-collecting", gdb_output)
        gdb_output = self.get_stack_trace(
            cmd, cmds_after_breakpoint=["break update_refs", "continue", "py-bt-full"]
        )
        self.assertIn("Garbage-collecting", gdb_output)
        ...
        for (func_name, args, expected_frame) in (
            ...
            for obj in (
                ...
                with self.subTest(f"{obj}.{func_name}"):
                    ...
                    gdb_output = self.get_stack_trace(
                        cmd, breakpoint=func_name, cmds_after_breakpoint=["bt", "py-bt"]
                    )
                    self.assertIn(f"<built-in method {func_name}", gdb_output)
                    gdb_output = self.get_stack_trace(
                        cmd, breakpoint=func_name, cmds_after_breakpoint=["py-bt-full"]
                    )
                    self.assertIn(
                        f"#{expected_frame} <built-in method {func_name}", gdb_output
                    )
    ...
    def test_wrapper_call(self):
        ...
        self.assertRegex(
            gdb_output, "<method-wrapper u?'__init__' of MyList object at "
        )
