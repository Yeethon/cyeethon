...
class TestMaildir(TestMailbox, unittest.TestCase):
    ...
    def test_get_folder(self):
        ...
        self.assertEqual(
            folder1.get_string(folder1.keys()[0]), (self._template % "bar")
        )
...
class _TestMboxMMDFMessage:
    ...
    def _check_from(self, msg, sender=None):
        ...
        self.assertIsNotNone(
            re.match(
                (sender + " \\w{3} \\w{3} [\\d ]\\d [\\d ]\\d:\\d{2}:\\d{2} \\d{4}"),
                msg.get_from(),
            )
        )
    ...
    def test_maildir_to_mboxmmdf(self):
        ...
        for class_ in (mailbox.mboxMessage, mailbox.MMDFMessage):
            ...
            for (setting, result) in pairs:
                ...
                self.assertEqual(
                    msg.get_from(),
                    ("MAILER-DAEMON %s" % time.asctime(time.gmtime(0.0))),
                )
            msg_maildir.set_subdir("cur")
...
class TestProxyFile(TestProxyFileBase, unittest.TestCase):
    ...
    def test_readline(self):
        self._file.write(
            bytes(
                ("foo%sbar%sfred%sbob" % (os.linesep, os.linesep, os.linesep)), "ascii"
            )
        )
...
class TestPartialFile(TestProxyFileBase, unittest.TestCase):
    ...
    def test_seek_and_tell(self):
        self._file.write(
            bytes(("(((foo%sbar%s$$$" % (os.linesep, os.linesep)), "ascii")
        )
    ...
    def test_close(self):
        self._file.write(bytes(("&foo%sbar%s^" % (os.linesep, os.linesep)), "ascii"))
        self._test_close(
            mailbox._PartialFile(self._file, 1, (6 + (3 * len(os.linesep))))
        )
