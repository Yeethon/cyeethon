...
class UnicodeTest(
    ...
    def test_maketrans_translate(self):
        ...
        self.checkequalnofix(
            "iiic", "abababc", "translate", {ord("a"): None, ord("b"): ord("i")}
        )
        self.checkequalnofix(
            "iiix",
            "abababc",
        ...
        self.checkequalnofix(
            "c", "abababc", "translate", {ord("a"): None, ord("b"): ""}
        )
        ...
        self.assertEqual(
            "axb".translate(str.maketrans({"a": None, "b": "123"})), "x123"
        )
    ...
    def test_split(self):
        ...
        for (left, right) in ("ba", "āĀ", "𐌁𐌀"):
            ...
            for delim in ("c", "Ă", "𐌂"):
                ...
                self.checkequal(
                    [left, right], ((left + (delim * 2)) + right), "split", (delim * 2)
                )
    ...
    def test_rsplit(self):
        ...
        for (left, right) in ("ba", "āĀ", "𐌁𐌀"):
            ...
            for delim in ("c", "Ă", "𐌂"):
                ...
                self.checkequal(
                    [left, right], ((left + delim) + right), "rsplit", delim
                )
                self.checkequal([(left + right)], (left + right), "rsplit", (delim * 2))
                self.checkequal(
                    [left, right], ((left + (delim * 2)) + right), "rsplit", (delim * 2)
                )
    ...
    def test_partition(self):
        ...
        for (left, right) in ("ba", "āĀ", "𐌁𐌀"):
            ...
            for delim in ("c", "Ă", "𐌂"):
                self.checkequal(
                    ((left + right), "", ""), (left + right), "partition", delim
                )
                self.checkequal(
                    (left, delim, right), ((left + delim) + right), "partition", delim
                )
                self.checkequal(
                    ((left + right), "", ""), (left + right), "partition", (delim * 2)
                )
                self.checkequal(
                    (left, (delim * 2), right),
                    ((left + (delim * 2)) + right),
    ...
    def test_rpartition(self):
        ...
        for (left, right) in ("ba", "āĀ", "𐌁𐌀"):
            ...
            for delim in ("c", "Ă", "𐌂"):
                self.checkequal(
                    ("", "", (left + right)), (left + right), "rpartition", delim
                )
                self.checkequal(
                    (left, delim, right), ((left + delim) + right), "rpartition", delim
                )
                self.checkequal(
                    ("", "", (left + right)), (left + right), "rpartition", (delim * 2)
                )
                self.checkequal(
                    (left, (delim * 2), right),
                    ((left + (delim * 2)) + right),
    ...
    @unittest.skipIf(
        (sys.maxsize > (2 ** 32)), "needs too much memory on a 64-bit platform"
    )
    ...
    def test_replace(self):
        ...
        for (left, right) in ("ba", "āĀ", "𐌁𐌀"):
            ...
            for delim in ("c", "Ă", "𐌂"):
                for repl in ("d", "ă", "𐌃"):
                    self.checkequal(
                        (left + right), (left + right), "replace", delim, repl
                    )
                    self.checkequal(
                        ((left + repl) + right),
                        ((left + delim) + right),
                    ...
                    self.checkequal(
                        (left + right), (left + right), "replace", (delim * 2), repl
                    )
                    self.checkequal(
                        ((left + repl) + right),
                        ((left + (delim * 2)) + right),
    ...
    def test_isspace_invariant(self):
        for codepoint in range((sys.maxunicode + 1)):
            ...
            self.assertEqual(
                char.isspace(),
                ((bidirectional in ("WS", "B", "S")) or (category == "Zs")),
            )
    ...
    def test_format(self):
        ...
        self.assertEqual(
            "{0:\x00<12}".format((3 + 2j)), "(3+2j)\x00\x00\x00\x00\x00\x00"
        )
        self.assertEqual(
            "{0:\x01<12}".format((3 + 2j)), "(3+2j)\x01\x01\x01\x01\x01\x01"
        )
        self.assertEqual(
            "{0:\x00^12}".format((3 + 2j)), "\x00\x00\x00(3+2j)\x00\x00\x00"
        )
        ...
        self.assertEqual(
            "{0:{width}.{precision}s}".format("hello world", width=10, precision=5),
            "hello     ",
        )
        self.assertEqual(
            "{0:{width}.{precision}s}".format("hello world", width="10", precision="5"),
        ...
        self.assertRaises(
            ValueError, "{0:{1:{2:{3:{4:{5:{6}}}}}}}".format, 0, 1, 2, 3, 4, 5, 6, 7
        )
    ...
    def test_formatting_with_enum(self):
        ...
        self.assertEqual(("%s, %s" % (Str.ABC, Str.ABC)), "abc, abc")
        self.assertEqual(
            (
                "%s, %s, %d, %i, %u, %f, %5.2f"
        ...
        self.assertEqual(("...%(foo)s..." % {"foo": Str.ABC}), "...abc...")
        self.assertEqual(("...%(foo)s..." % {"foo": Int.IDES}), "...IDES...")
    ...
    def test_constructor(self):
        ...
        if not sys.platform.startswith("java"):
            self.assertRaises(
                TypeError, str, "decoding unicode is not supported", "utf-8", "strict"
            )
        self.assertEqual(
        ...
        for sb in invalid_4B_seq_start_bytes:
            for cb1 in continuation_bytes[:3]:
                for cb3 in continuation_bytes[:3]:
                    self.assertRaises(
                        UnicodeDecodeError,
                        (((sb + cb1) + b"\x80") + cb3).decode,
                        "utf-8",
                    )
        for cb in [bytes([x]) for x in range(128, 160)]:
            self.assertRaises(
                UnicodeDecodeError, ((b"\xe0" + cb) + b"\x80").decode, "utf-8"
            )
            self.assertRaises(
                UnicodeDecodeError, ((b"\xe0" + cb) + b"\xbf").decode, "utf-8"
            )
        for cb in [bytes([x]) for x in range(160, 192)]:
            self.assertRaises(
                UnicodeDecodeError, ((b"\xed" + cb) + b"\x80").decode, "utf-8"
            )
            self.assertRaises(
                UnicodeDecodeError, ((b"\xed" + cb) + b"\xbf").decode, "utf-8"
            )
        for cb in [bytes([x]) for x in range(128, 144)]:
            self.assertRaises(
                UnicodeDecodeError, ((b"\xf0" + cb) + b"\x80\x80").decode, "utf-8"
            )
            self.assertRaises(
                UnicodeDecodeError, ((b"\xf0" + cb) + b"\xbf\xbf").decode, "utf-8"
            )
        for cb in [bytes([x]) for x in range(144, 192)]:
            self.assertRaises(
                UnicodeDecodeError, ((b"\xf4" + cb) + b"\x80\x80").decode, "utf-8"
            )
            self.assertRaises(
                UnicodeDecodeError, ((b"\xf4" + cb) + b"\xbf\xbf").decode, "utf-8"
            )
    ...
    def test_issue8271(self):
        ...
        sequences = [
            ...
            (b"\xc2", FFFD),
            (b"\xc2\xc2", (FFFD * 2)),
            (b"\xc2\xc2\xc2", (FFFD * 3)),
            (b"\xc2A", (FFFD + "A")),
            (b"\xe1", FFFD),
            (b"\xe1\xe1", (FFFD * 2)),
            (b"\xe1\xe1\xe1", (FFFD * 3)),
            (b"\xe1\xe1\xe1\xe1", (FFFD * 4)),
            (b"\xe1\x80", FFFD),
            (b"\xe1A", (FFFD + "A")),
            (b"\xe1A\x80", ((FFFD + "A") + FFFD)),
            (b"\xe1AA", (FFFD + "AA")),
            (b"\xe1\x80A", (FFFD + "A")),
            (b"\xe1\x80\xe1A", ((FFFD * 2) + "A")),
            (b"\xe1A\xe1\x80", ((FFFD + "A") + FFFD)),
            (b"\xf1", FFFD),
            (b"\xf1\xf1", (FFFD * 2)),
            (b"\xf1\xf1\xf1", (FFFD * 3)),
            (b"\xf1\xf1\xf1\xf1", (FFFD * 4)),
            (b"\xf1\xf1\xf1\xf1\xf1", (FFFD * 5)),
            (b"\xf1\x80", FFFD),
            (b"\xf1\x80\x80", FFFD),
            (b"\xf1\x80A", (FFFD + "A")),
            (b"\xf1\x80AA", (FFFD + "AA")),
            (b"\xf1\x80\x80A", (FFFD + "A")),
            (b"\xf1A\x80", ((FFFD + "A") + FFFD)),
            (b"\xf1A\x80\x80", ((FFFD + "A") + (FFFD * 2))),
            (b"\xf1A\x80A", (((FFFD + "A") + FFFD) + "A")),
            (b"\xf1AA\x80", ((FFFD + "AA") + FFFD)),
            (b"\xf1A\xf1\x80", ((FFFD + "A") + FFFD)),
            (b"\xf1A\x80\xf1", ((FFFD + "A") + (FFFD * 2))),
            (b"\xf1\xf1\x80A", ((FFFD * 2) + "A")),
            (b"\xf1A\xf1\xf1", ((FFFD + "A") + (FFFD * 2))),
            (b"\xf5", FFFD),
            (b"\xf5\xf5", (FFFD * 2)),
            (b"\xf5\x80", (FFFD * 2)),
            (b"\xf5\x80\x80", (FFFD * 3)),
            (b"\xf5\x80\x80\x80", (FFFD * 4)),
            (b"\xf5\x80A", ((FFFD * 2) + "A")),
            (b"\xf5\x80A\xf5", (((FFFD * 2) + "A") + FFFD)),
            (b"\xf5A\x80\x80A", (((FFFD + "A") + (FFFD * 2)) + "A")),
            (b"\xf8", FFFD),
            (b"\xf8\xf8", (FFFD * 2)),
            (b"\xf8\x80", (FFFD * 2)),
            (b"\xf8\x80A", ((FFFD * 2) + "A")),
            (b"\xf8\x80\x80\x80\x80", (FFFD * 5)),
            (b"\xfc", FFFD),
            (b"\xfc\xfc", (FFFD * 2)),
            (b"\xfc\x80\x80", (FFFD * 3)),
            (b"\xfc\x80\x80\x80\x80\x80", (FFFD * 6)),
            (b"\xfe", FFFD),
            (b"\xfe\x80\x80", (FFFD * 3)),
            (b"\xf1\x80ABC", "�ABC"),
            (b"\xf1\x80\xffBC", "��BC"),
            (b"\xf1\x80\xc2\x81C", "�\x81C"),
    ...
    def assertCorrectUTF8Decoding(self, seq, res, err):
        ...
        self.assertEqual(seq.decode("utf-8", "replace"), res)
        self.assertEqual(
            ((b"aaaa" + seq) + b"bbbb").decode("utf-8", "replace"),
            (("aaaa" + res) + "bbbb"),
        )
        res = res.replace("�", "")
        self.assertEqual(seq.decode("utf-8", "ignore"), res)
        self.assertEqual(
            ((b"aaaa" + seq) + b"bbbb").decode("utf-8", "ignore"),
            (("aaaa" + res) + "bbbb"),
        )
    ...
    def test_invalid_cb_for_2bytes_seq(self):
        ...
        for (seq, res) in sequences:
            self.assertCorrectUTF8Decoding(
                bytes.fromhex(seq), res, "invalid continuation byte"
            )
    ...
    def test_invalid_cb_for_4bytes_seq(self):
        ...
        sequences = [
            ...
            ("F0 BF C0", FFFDx2),
            ("F0 BF FF", FFFDx2),
            ("F0 90 80 00", (FFFD + "\x00")),
            ("F0 90 80 7F", (FFFD + "\x7f")),
            ("F0 90 80 C0", FFFDx2),
            ("F0 90 80 FF", FFFDx2),
            ("F0 90 BF 00", (FFFD + "\x00")),
            ("F0 90 BF 7F", (FFFD + "\x7f")),
            ("F0 90 BF C0", FFFDx2),
            ("F0 90 BF FF", FFFDx2),
            ("F0 BF 80 00", (FFFD + "\x00")),
            ("F0 BF 80 7F", (FFFD + "\x7f")),
            ("F0 BF 80 C0", FFFDx2),
            ("F0 BF 80 FF", FFFDx2),
            ("F0 BF BF 00", (FFFD + "\x00")),
            ("F0 BF BF 7F", (FFFD + "\x7f")),
            ("F0 BF BF C0", FFFDx2),
            ("F0 BF BF FF", FFFDx2),
            ("F1 00", (FFFD + "\x00")),
            ("F1 7F", (FFFD + "\x7f")),
            ...
            ("F1 BF C0", FFFDx2),
            ("F1 BF FF", FFFDx2),
            ("F1 80 80 00", (FFFD + "\x00")),
            ("F1 80 80 7F", (FFFD + "\x7f")),
            ("F1 80 80 C0", FFFDx2),
            ("F1 80 80 FF", FFFDx2),
            ("F1 80 BF 00", (FFFD + "\x00")),
            ("F1 80 BF 7F", (FFFD + "\x7f")),
            ("F1 80 BF C0", FFFDx2),
            ("F1 80 BF FF", FFFDx2),
            ("F1 BF 80 00", (FFFD + "\x00")),
            ("F1 BF 80 7F", (FFFD + "\x7f")),
            ("F1 BF 80 C0", FFFDx2),
            ("F1 BF 80 FF", FFFDx2),
            ("F1 BF BF 00", (FFFD + "\x00")),
            ("F1 BF BF 7F", (FFFD + "\x7f")),
            ("F1 BF BF C0", FFFDx2),
            ("F1 BF BF FF", FFFDx2),
            ("F3 00", (FFFD + "\x00")),
            ("F3 7F", (FFFD + "\x7f")),
            ...
            ("F3 BF C0", FFFDx2),
            ("F3 BF FF", FFFDx2),
            ("F3 80 80 00", (FFFD + "\x00")),
            ("F3 80 80 7F", (FFFD + "\x7f")),
            ("F3 80 80 C0", FFFDx2),
            ("F3 80 80 FF", FFFDx2),
            ("F3 80 BF 00", (FFFD + "\x00")),
            ("F3 80 BF 7F", (FFFD + "\x7f")),
            ("F3 80 BF C0", FFFDx2),
            ("F3 80 BF FF", FFFDx2),
            ("F3 BF 80 00", (FFFD + "\x00")),
            ("F3 BF 80 7F", (FFFD + "\x7f")),
            ("F3 BF 80 C0", FFFDx2),
            ("F3 BF 80 FF", FFFDx2),
            ("F3 BF BF 00", (FFFD + "\x00")),
            ("F3 BF BF 7F", (FFFD + "\x7f")),
            ("F3 BF BF C0", FFFDx2),
            ("F3 BF BF FF", FFFDx2),
            ("F4 00", (FFFD + "\x00")),
            ("F4 7F", (FFFD + "\x7f")),
            ...
            ("F4 8F C0", FFFDx2),
            ("F4 8F FF", FFFDx2),
            ("F4 80 80 00", (FFFD + "\x00")),
            ("F4 80 80 7F", (FFFD + "\x7f")),
            ("F4 80 80 C0", FFFDx2),
            ("F4 80 80 FF", FFFDx2),
            ("F4 80 BF 00", (FFFD + "\x00")),
            ("F4 80 BF 7F", (FFFD + "\x7f")),
            ("F4 80 BF C0", FFFDx2),
            ("F4 80 BF FF", FFFDx2),
            ("F4 8F 80 00", (FFFD + "\x00")),
            ("F4 8F 80 7F", (FFFD + "\x7f")),
            ("F4 8F 80 C0", FFFDx2),
            ("F4 8F 80 FF", FFFDx2),
            ("F4 8F BF 00", (FFFD + "\x00")),
            ("F4 8F BF 7F", (FFFD + "\x7f")),
            ("F4 8F BF C0", FFFDx2),
            ("F4 8F BF FF", FFFDx2),
        ]
        for (seq, res) in sequences:
    ...
    def test_codecs_errors(self):
        ...
        self.assertEqual(
            "Andr\x82 x".encode("ascii", "replace"),
            "Andr\x82 x".encode("ascii", errors="replace"),
        )
        self.assertEqual(
            "Andr\x82 x".encode("ascii", "ignore"),
...
class CAPITest(unittest.TestCase):
    def test_from_format(self):
        ...
        def PyUnicode_FromFormat(format, *args):
            cargs = tuple(
                ((py_object(arg) if isinstance(arg, str) else arg) for arg in args)
            )
        ...
        check_format("%A:'abc\\xe9\\uabcd\\U0010ffff'", b"%%A:%A", "abcéꯍ\U0010ffff")
    ...
    def test_asucs4(self):
        ...
        for s in ["abc", "¡¢", "你好", "a😀", "a\ud800b\udfffc", "\ud834\udd1e"]:
            l = len(s)
            self.assertEqual(unicode_asucs4(s, l, True), (s + "\x00"))
            self.assertEqual(unicode_asucs4(s, l, False), (s + "\uffff"))
            self.assertEqual(unicode_asucs4(s, (l + 1), True), (s + "\x00\uffff"))
            self.assertEqual(unicode_asucs4(s, (l + 1), False), (s + "\x00\uffff"))
            self.assertRaises(SystemError, unicode_asucs4, s, (l - 1), True)
            self.assertRaises(SystemError, unicode_asucs4, s, (l - 2), False)
            s = "\x00".join([s, s])
            self.assertEqual(unicode_asucs4(s, len(s), True), (s + "\x00"))
    ...
    def test_findchar(self):
        ...
        for str in ("¡", "耀肀", "\ud800\udc02", "🄀🇱"):
            for (i, ch) in enumerate(str):
                ...
                self.assertEqual(unicode_findchar(str, ord(ch), 0, len(str), (-1)), i)
        str = "!>_<!"
        self.assertEqual(unicode_findchar(str, 1114112, 0, len(str), 1), (-1))
        self.assertEqual(unicode_findchar(str, 1114112, 0, len(str), (-1)), (-1))
    ...
    def test_copycharacters(self):
        ...
        for (idx, from_) in enumerate(strings):
            for to in strings[:idx]:
                self.assertRaises(
                    SystemError, unicode_copycharacters, to, 0, from_, 0, 5
                )
            for from_start in range(5):
                self.assertEqual(
                    unicode_copycharacters(from_, 0, from_, from_start, 5),
                    (
                        from_[from_start : (from_start + 5)].ljust(5, "\x00"),
                        (5 - from_start),
                    ),
                )
            for to_start in range(5):
                self.assertEqual(
                    ...
                    (from_[to_start : (to_start + 5)].rjust(5, "\x00"), (5 - to_start)),
                )
        s = strings[0]
    ...
    def test_encode_decimal(self):
        ...
        with warnings_helper.check_warnings():
            ...
            self.assertRaises(
                UnicodeEncodeError, unicode_encodedecimal, "123€", "strict"
            )
            self.assertRaisesRegex(
                ValueError,
    ...
    def test_transform_decimal(self):
        ...
        with warnings_helper.check_warnings():
            ...
            self.assertEqual(transform_decimal("\u20033.14\u2002"), "\u20033.14\u2002")
            self.assertEqual(transform_decimal("123€"), "123€")
...
class StringModuleTest(unittest.TestCase):
    def test_formatter_parser(self):
        ...
        self.assertEqual(
            formatter, [("prefix ", "", "", None), (" suffix", None, None, None)]
        )
        formatter = parse("str")
    ...
    def test_formatter_field_name_split(self):
        ...
        self.assertEqual(
            ...
            ["obj", [(True, "arg"), (False, "key1"), (False, "key2")]],
        )
