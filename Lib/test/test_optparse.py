...
class BaseTest(unittest.TestCase):
    ...
    def assertOutput(
        self, cmdline_args, expected_output, expected_status=0, expected_error=None
    ):
...
class TestOptionChecks(BaseTest):
    ...
    def test_opt_string_too_short(self):
        self.assertOptionError(
            "invalid option string 'b': must be at least two characters long", ["b"]
        )

    def test_opt_string_short_invalid(self):
        self.assertOptionError(
            "invalid short option string '--': must be of the form -x, (x any non-dash char)",
            ["--"],
        )

    def test_opt_string_long_invalid(self):
        self.assertOptionError(
            "invalid long option string '---': must start with --, followed by non-dash",
            ["---"],
        )
    ...
    def test_action_invalid(self):
        self.assertOptionError(
            "option -b: invalid action: 'foo'", ["-b"], {"action": "foo"}
        )

    def test_type_invalid(self):
        self.assertOptionError(
            "option -b: invalid option type: 'foo'", ["-b"], {"type": "foo"}
        )
        self.assertOptionError(
            "option -b: invalid option type: 'tuple'", ["-b"], {"type": tuple}
        )
    ...
    def test_bad_choices_list(self):
        ...
        self.assertOptionError(
            (
                "option -b/--bad: choices must be a list of strings ('%s' supplied)"
                % typename
            ),
            ["-b", "--bad"],
    ...
    def test_no_callback_args_for_action(self):
        self.assertOptionError(
            ...
            ["-b"],
            {"action": "store", "callback_args": "foo"},
        )
    ...
    def test_no_callback_kwargs_for_action(self):
        self.assertOptionError(
            ...
            ["-b"],
            {"action": "store", "callback_kwargs": "foo"},
        )

    def test_no_single_dash(self):
        self.assertOptionError(
            "invalid long option string '-debug': must start with --, followed by non-dash",
            ["-debug"],
        )
        self.assertOptionError(
            "option -d: invalid long option string '-debug': must start with --, followed by non-dash",
            ["-d", "-debug"],
        )
        self.assertOptionError(
            "invalid long option string '-debug': must start with --, followed by non-dash",
...
class TestOptionParser(BaseTest):
    def setUp(self):
        ...
        self.parser.add_option(
            "-v", "--verbose", "-n", "--noisy", action="store_true", dest="verbose"
        )
        self.parser.add_option(
            "-q", "--quiet", "--silent", action="store_false", dest="verbose"
        )

    def test_add_option_no_Option(self):
        self.assertTypeError(
            self.parser.add_option, "not an Option instance: None", None
        )
...
class TestExpandDefaults(BaseTest):
    ...
    def test_float_default(self):
        self.parser.add_option(
            "-p", "--prob", help="blow up with probability PROB [default: %default]"
        )
    ...
    def test_no_expand(self):
        self.parser.add_option(
            "-f", "--file", default="foo.txt", help="read from %default file"
        )
        self.parser.formatter.default_tag = None
        expected_help = (
            self.help_prefix + "  -f FILE, --file=FILE  read from %default file\n"
        )
        self.assertHelp(self.parser, expected_help)
...
class TestStandard(BaseTest):
    def setUp(self):
        ...
        self.parser = InterceptingOptionParser(
            usage=SUPPRESS_USAGE, option_list=options
        )
    ...
    def test_long_option_short_option(self):
        self.assertParseOK(
            ["--foo=bar", "-axyz"], {"a": "xyz", "boo": None, "foo": ["bar"]}, []
        )

    def test_abbrev_long_option(self):
        self.assertParseOK(
            ["--f=bar", "-axyz"], {"a": "xyz", "boo": None, "foo": ["bar"]}, []
        )
    ...
    def test_short_and_long_option_split(self):
        self.assertParseOK(
            ["-a", "xyz", "--foo", "bar"], {"a": "xyz", "boo": None, "foo": ["bar"]}, []
        )
    ...
    def test_short_option_split_one_positional_arg(self):
        self.assertParseOK(
            ["-a", "foo", "bar"], {"a": "foo", "boo": None, "foo": None}, ["bar"]
        )
    ...
    def test_hyphen_becomes_positional_arg(self):
        self.assertParseOK(
            ["-ab", "-", "--foo", "bar"], {"a": "b", "boo": None, "foo": ["bar"]}, ["-"]
        )
...
class TestBool(BaseTest):
    def setUp(self):
        options = [
            make_option(
                "-v", "--verbose", action="store_true", dest="verbose", default=""
            ),
...
class TestCount(BaseTest):
    def setUp(self):
        ...
        self.parser.add_option(
            "-q", "--quiet", action="store_const", dest="verbose", const=0
        )
    ...
    def test_count_option_no_value(self):
        self.assertParseFail(
            ["--quiet=3", "-v"], "--quiet option does not take a value"
        )
...
class TestMultipleArgs(BaseTest):
    def setUp(self):
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE)
        self.parser.add_option(
            "-p", "--point", action="store", nargs=3, type="float", dest="point"
        )
    ...
    def test_nargs_invalid_float_value(self):
        self.assertParseFail(
            ["-p", "1.0", "2x", "3.5"], "option -p: invalid floating-point value: '2x'"
        )

    def test_nargs_required_values(self):
        self.assertParseFail(
            ["--point", "1.0", "3.5"], "--point option requires 3 arguments"
        )
...
class TestMultipleArgsAppend(BaseTest):
    def setUp(self):
        ...
        self.parser.add_option(
            "-f", "--foo", action="append", nargs=2, type="int", dest="foo"
        )
        self.parser.add_option(
            "-z", "--zero", action="append_const", dest="foo", const=(0, 0)
        )
...
class TestVersion(BaseTest):
    def test_version(self):
        self.parser = InterceptingOptionParser(
            usage=SUPPRESS_USAGE, version="%prog 0.1"
        )
        save_argv = sys.argv[:]
        try:
...
class TestOptionGroup(BaseTest):
    ...
    def test_add_group_no_group(self):
        self.assertTypeError(
            self.parser.add_option_group, "not an OptionGroup instance: None", None
        )

    def test_add_group_invalid_arguments(self):
        self.assertTypeError(
            self.parser.add_option_group, "invalid arguments", None, None
        )
...
class TestExtendAddTypes(BaseTest):
    def setUp(self):
        self.parser = InterceptingOptionParser(
            usage=SUPPRESS_USAGE, option_class=self.MyOption
        )
    ...
    def test_filetype_noexist(self):
        self.assertParseFail(
            ["--file", os_helper.TESTFN, "-afoo"],
            ("%s: file does not exist" % os_helper.TESTFN),
        )
    ...
    def test_filetype_notfile(self):
        ...
        self.assertParseFail(
            ["--file", os_helper.TESTFN, "-afoo"],
            ("%s: not a regular file" % os_helper.TESTFN),
        )
...
class TestExtendAddActions(BaseTest):
    def setUp(self):
        options = [
            self.MyOption("-a", "--apple", action="extend", type="string", dest="apple")
        ]
        self.parser = OptionParser(option_list=options)
    ...
    def test_extend_add_action(self):
        self.assertParseOK(
            ["-afoo,bar", "--apple=blah"], {"apple": ["foo", "bar", "blah"]}, []
        )
...
class TestCallback(BaseTest):
    def setUp(self):
        options = [
            make_option("-x", None, action="callback", callback=self.process_opt),
            make_option(
                "-f",
                "--file",
                action="callback",
...
class TestCallbackExtraArgs(BaseTest):
    ...
    def test_callback_extra_args(self):
        self.assertParseOK(
            ["-p1,2,3", "--point", "4,5,6"], {"points": [(1, 2, 3), (4, 5, 6)]}, []
        )
...
class TestCallbackMeddleArgs(BaseTest):
    def setUp(self):
        options = [
            make_option(
                str(x), action="callback", callback=self.process_n, dest="things"
            )
...
class TestCallbackCheckAbbrev(BaseTest):
    def setUp(self):
        self.parser = OptionParser()
        self.parser.add_option(
            "--foo-bar", action="callback", callback=self.check_abbrev
        )
...
class TestConflict(ConflictBase):
    ...
    def test_no_such_conflict_handler(self):
        self.assertRaises(
            self.parser.set_conflict_handler,
            ("foo",),
            None,
            ValueError,
...
class TestConflictResolve(ConflictBase):
    ...
    def test_conflict_resolve_long_opts(self):
        self.assertParseOK(
            ["--verbose", "--version"], {"verbose": 1, "show_version": 1}, []
        )
    ...
    def make_parser(self, columns):
        options = [
            ...
            make_option(
                ...
                dest="boo",
                metavar="NUM",
                help='shout "boo!" NUM times (in order to frighten away all the evil spirits that cause trouble and mayhem)',
            ),
            make_option(
                "--foo",
                action="append",
...
class TestMatchAbbrev(BaseTest):
    def test_match_abbrev(self):
        self.assertEqual(
            _match_abbrev(
                "--f", {"--foz": None, "--foo": None, "--fie": None, "--f": None}
            ),
            "--f",
        )
...
class TestParseNumber(BaseTest):
    ...
    def test_parse_num_fail(self):
        self.assertRaises(
            ...
            {},
            ValueError,
            re.compile("invalid literal for int().*: '?'?"),
        )
        self.assertRaises(
            _parse_num,
            ("0xOoops", int),
            {},
            ValueError,
            re.compile("invalid literal for int().*: s?'?0xOoops'?"),
        )
    ...
    def test_numeric_options(self):
        ...
        self.assertParseFail(
            ["-l", "0x12x"], "option -l: invalid integer value: '0x12x'"
        )
