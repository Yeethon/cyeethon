...
def get_lockdata():
    try:
        ...
        start_len = "qq"
    if sys.platform.startswith(("netbsd", "freebsd", "openbsd")) or (
        sys.platform == "darwin"
    ):
        ...
        lockdata = struct.pack(
            (((off_t + off_t) + pid_t) + "hh"), 0, 0, 0, fcntl.F_WRLCK, 0
        )
        ...
        lockdata = struct.pack(
            (("hh" + start_len) + "hh"), fcntl.F_WRLCK, 0, 0, 0, 0, 0
        )
    if lockdata:
        if verbose:
...
class TestFcntl(unittest.TestCase):
    ...
    def test_flock_overflow(self):
        ...
        self.assertRaises(
            OverflowError, fcntl.flock, (_testcapi.INT_MAX + 1), fcntl.LOCK_SH
        )
