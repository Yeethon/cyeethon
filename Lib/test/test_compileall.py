...
class CompileallTestsBase:
    ...
    def test_compile_files(self):
        ...
        self.assertFalse(
            compileall.compile_file(self.bad_source_path, force=False, quiet=2)
        )
    ...
    def test_compile_file_pathlike_ddir(self):
        ...
        self.assertTrue(
            compileall.compile_file(
                pathlib.Path(self.source_path), ddir=pathlib.Path("ddir_path"), quiet=2
            )
        )
        self.assertTrue(os.path.isfile(self.bc_path))
    ...
    def test_compile_path(self):
        ...
        with test.test_importlib.util.import_state(path=[self.directory]):
            self.add_bad_source_file()
            self.assertFalse(
                compileall.compile_path(skip_curdir=False, force=True, quiet=2)
            )
    ...
    def test_compile_workers_non_positive(self):
        with self.assertRaisesRegex(
            ValueError, "workers must be greater or equal to 0"
        ):
    ...
    def _test_ddir_only(self, *, ddir, parallel=True):
        ...
        compileall.compile_dir(
            self.directory, quiet=True, ddir=ddir, workers=(2 if parallel else 1)
        )
    ...
    def test_strip_prepend_and_ddir(self):
        ...
        with self.assertRaises(ValueError):
            compileall.compile_dir(
                path, quiet=True, ddir="/bar", stripdir="/foo", prependdir="/bar"
            )
    ...
    def test_ignore_symlink_destination(self):
        ...
        allowed_script = script_helper.make_script(
            allowed_path, "test_allowed", "a = 0"
        )
        prohibited_script = script_helper.make_script(
            prohibited_path, "test_prohibited", "a = 0"
        )
...
class CompileallTestsWithoutSourceEpoch(
    CompileallTestsBase,
    unittest.TestCase,
    metaclass=SourceDateEpochTestMeta,
...
class CommandLineTestsBase:
    ...
    def _get_run_args(self, args):
        return [
            *support.optim_args_from_interpreter_flags(),
            "-S",
            "-m",
            "compileall",
            *args,
    ...
    def test_symlink_loop(self):
        ...
        self.assertRunOK("-q", self.pkgdir)
        self.assertCompiled(
            os.path.join(self.pkgdir, "spam", "evil", "evil2", "__init__.py")
        )
    ...
    def test_include_bad_file(self):
        ...
        self.assertFalse(
            os.path.exists(importlib.util.cache_from_source(self.pkgdir_cachedir))
        )
...
class CommandLineTestsNoSourceEpoch(
    CommandLineTestsBase,
    unittest.TestCase,
    metaclass=SourceDateEpochTestMeta,
    source_date_epoch=False,
...
class HardlinkDedupTestsBase:
    ...
    def compile_dir(self, *, dedup=True, optimize=(0, 1, 2), force=False):
        compileall.compile_dir(
            self.path, quiet=True, optimize=optimize, hardlink_dupes=dedup, force=force
        )
    ...
    def test_bad_args(self):
        with self.temporary_directory():
            ...
            with self.assertRaises(ValueError):
                compileall.compile_dir(
                    self.path, quiet=True, optimize=0, hardlink_dupes=True
                )
            with self.assertRaises(ValueError):
                compileall.compile_dir(
                    self.path, quiet=True, optimize=[0, 0], hardlink_dupes=True
                )
    ...
    def check_hardlinks(self, script, docstring=False, assertion=False):
        ...
        self.assertEqual(
            is_hardlink(pycs[0], pycs[2]), ((not assertion) and (not docstring))
        )
...
class HardlinkDedupTestsWithSourceEpoch(
    HardlinkDedupTestsBase,
    unittest.TestCase,
    metaclass=SourceDateEpochTestMeta,
    source_date_epoch=True,
):
...
class HardlinkDedupTestsNoSourceEpoch(
    HardlinkDedupTestsBase,
    unittest.TestCase,
    metaclass=SourceDateEpochTestMeta,
    source_date_epoch=False,
