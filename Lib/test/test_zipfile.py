...
class StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.TestCase):
    ...
    def test_bad_zip64_extra(self):
        ...
        missing_compress_size_extra = self.make_zip64_file(
            file_size_64_set=True, file_size_extra=True, compress_size_64_set=True
        )
        with self.assertRaises(zipfile.BadZipFile) as e:
...
class PyZipFileTests(unittest.TestCase):
    ...
    def test_write_python_directory_filtered(self):
        ...
        try:
            ...
            with TemporaryFile() as t, zipfile.PyZipFile(t, "w") as zipfp:
                zipfp.writepy(
                    TESTFN2, filterfunc=(lambda fn: (not fn.endswith("mod2.py")))
                )
                names = zipfp.namelist()
                self.assertCompiledIn("mod1.py", names)
...
class ExtractTests(unittest.TestCase):
    ...
    def test_extract_hackers_arcnames_windows_only(self):
        ...
        windows_hacknames = [
            ...
            ("C://foo/bar", "foo/bar"),
            ("C:\\foo\\bar", "foo/bar"),
            ("//conky/mountpoint/foo/bar", "foo/bar"),
            ("\\\\conky\\mountpoint\\foo\\bar", "foo/bar"),
            ...
            ("//?/C:/foo/bar", "foo/bar"),
            ("\\\\?\\C:\\foo\\bar", "foo/bar"),
            ("C:/../C:/foo/bar", "C_/foo/bar"),
            ('a:b\\c<d>e|f"g?h*i', "b/c_d_e_f_g_h_i"),
            ("../../foo../../ba..r", "foo/ba..r"),
        ]
...
class AbstractTestsWithRandomBinaryFiles:
    ...
    def setUpClass(cls):
        ...
        cls.data = b"".join(
            (
                struct.pack("<f", (random() * randint((-1000), 1000)))
