...
class TestInterpreterStack(IsTestBase):
    ...
    def test_trace(self):
        ...
        self.assertEqual(
            revise(*git.tr[2][1:]), (modfile, 18, "eggs", ["    q = y / 0\n"], 0)
        )
...
class TestRetrievingSourceCode(GetSourceBase):
    ...
    def test_getfunctions(self):
        ...
        self.assertEqual(
            functions,
            [("eggs", mod.eggs), ("lobbest", mod.lobbest), ("spam", mod.spam)],
        )
...
class TestBuggyCases(GetSourceBase):
    ...
    @unittest.skipIf(
        (
            (not hasattr(unicodedata, "__file__"))
            or unicodedata.__file__.endswith(".py")
        ),
...
class TestClassesAndFunctions(unittest.TestCase):
    ...
    def test_getargspec(self):
        ...
        self.assertRaises(
            ValueError, self.assertArgSpecEquals, mod2.keyword_only_arg, []
        )
    ...
    def test_getfullargspec(self):
        ...
        self.assertFullArgSpecEquals(
            mod2.annotated, ["arg1"], ann_e={"arg1": list}, formatted="(arg1: list)"
        )
        self.assertFullArgSpecEquals(
            mod2.keyword_only_arg, [], kwonlyargs_e=["arg"], formatted="(*, arg)"
        )
        self.assertFullArgSpecEquals(
            mod2.all_markers,
            ["a", "b", "c", "d"],
    ...
    def test_argspec_api_ignores_wrapped(self):
        ...
        self.assertFullArgSpecEquals(
            functools.partial(ham), ["x", "y"], formatted="(x, y)"
        )
    ...
    def test_getfullargspec_builtin_methods(self):
        self.assertFullArgSpecEquals(
            _pickle.Pickler.dump, ["self", "obj"], formatted="(self, obj)"
        )
        self.assertFullArgSpecEquals(
            _pickle.Pickler(io.BytesIO()).dump, ["self", "obj"], formatted="(self, obj)"
        )
        self.assertFullArgSpecEquals(
            os.stat,
            args_e=["path"],
...
class TestGetcallargsFunctions(unittest.TestCase):
    ...
    def test_multiple_features(self):
        ...
        self.assertEqualCallArgs(
            f,
            "2, x=8, *collections.UserList([3, (4,[5,6])]), **collections.UserDict(y=9, z=10)",
        )
        ...
        self.assertEqualCallArgs(
            f, 'x=8, *collections.UserList([2, 3, (4,[5,6])]), q=0, **{"y":9, "z":10}'
        )
        self.assertEqualCallArgs(
            f,
            "2, x=8, *collections.UserList([3, (4,[5,6])]), q=0, **collections.UserDict(y=9, z=10)",
        )
...
class TestParameterObject(unittest.TestCase):
    ...
    def test_signature_parameter_replace(self):
        ...
        self.assertNotEqual(p2, p)
        with self.assertRaisesRegex(
            ValueError, "value <class 'inspect._empty'> is not a valid Parameter.kind"
        ):
    ...
    def test_signature_parameter_implicit(self):
        with self.assertRaisesRegex(
            ValueError,
            "implicit arguments must be passed as positional or keyword arguments, not positional-only",
        ):
...
class TestBoundArguments(unittest.TestCase):
    ...
    def test_signature_bound_arguments_repr(self):
        ...
        sig = inspect.signature(foo)
        ba = sig.bind(20, 30, z={})
        self.assertRegex(repr(ba), "<BoundArguments \\(a=20,.*\\}\\}\\)>")
...
class TestSignaturePrivateHelpers(unittest.TestCase):
    ...
    def _strip_non_python_syntax(
        self, input, clean_signature, self_parameter, last_positional_only
    ):
        (
    ...
    def test_signature_strip_non_python_syntax(self):
        ...
        self._strip_non_python_syntax(
            "($module, word, salt, /)", "(module, word, salt)", 0, 2
        )
        self._strip_non_python_syntax(
            "(x, y=None, z=None, /)", "(x, y=None, z=None)", None, 2
        )
        self._strip_non_python_syntax(
            "(x, y=None, z=None)", "(x, y=None, z=None)", None, None
        )
        self._strip_non_python_syntax(
...
class TestMain(unittest.TestCase):
    ...
    def test_qualname_source(self):
        ...
        lines = out.decode().splitlines()
        self.assertEqual(
            lines[:(-1)], inspect.getsource(ThreadPoolExecutor).splitlines()
        )
        self.assertEqual(err, b"")
