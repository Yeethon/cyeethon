...
class PositionalOnlyTestCase(unittest.TestCase):
    ...
    def test_positional_only_and_kwonlyargs_invalid_calls(self):
        ...
        f(1, 2, 3, d=1, e=2)
        with self.assertRaisesRegex(
            TypeError, "missing 1 required keyword-only argument: 'd'"
        ):
            f(1, 2, 3, e=2)
        with self.assertRaisesRegex(
            TypeError, "missing 2 required keyword-only arguments: 'd' and 'e'"
        ):
            f(1, 2, 3)
        with self.assertRaisesRegex(
            TypeError, "f\\(\\) missing 1 required positional argument: 'c'"
        ...
        with self.assertRaisesRegex(
            ...
            f(1)
        with self.assertRaisesRegex(
            TypeError, " missing 3 required positional arguments: 'a', 'b', and 'c'"
        ):
            f()
        with self.assertRaisesRegex(
            TypeError,
    ...
    def test_closures(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "g\\(\\) missing 1 required positional argument: 'y2'"
        ):
            f(1, 2)(3)
        with self.assertRaisesRegex(
    ...
    def test_same_keyword_as_positional_with_kwargs(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "f\\(\\) missing 1 required positional argument: 'something'"
        ):
            f(something=42)
