...
class TestEnum(unittest.TestCase):
    ...
    def test_enum_duplicates(self):
        ...
        lst = list(Season)
        self.assertEqual(
            lst, [Season.SPRING, Season.SUMMER, Season.AUTUMN, Season.WINTER]
        )
    ...
    def test_reserved__sunder_(self):
        with self.assertRaisesRegex(
            ValueError, "_sunder_ names, such as ._bad_., are reserved"
        ):
    ...
    def test_iteration_order(self):
        ...
        self.assertEqual(
            list(Season), [Season.SUMMER, Season.WINTER, Season.AUTUMN, Season.SPRING]
        )
    ...
    def test_duplicate_values_give_unique_enum_items(self):
        ...
        self.assertEqual(
            list(AutoNumber), [AutoNumber.first, AutoNumber.second, AutoNumber.third]
        )
    ...
    def test_init(self):
        ...
        self.assertEqual(Planet.EARTH.value, (5.976e24, 6378140.0))
    ...
    @unittest.skipUnless(
        (sys.version_info[:2] == (3, 9)), "private variables are now normal attributes"
    )
    ...
    @unittest.skipUnless(
        (sys.version_info[:2] < (3, 12)), "member-member access now raises an exception"
    )
...
class TestFlag(unittest.TestCase):
    ...
    def test_bizarre(self):
        with self.assertRaisesRegex(
            TypeError, "invalid Flag 'Bizarre' -- missing values: 1, 2"
        ):
...
class TestIntFlag(unittest.TestCase):
    ...
    def test_and(self):
        ...
        for i in values:
            for j in values:
                self.assertEqual(
                    (i & j), (i.value & j.value), ("i is %r, j is %r" % (i, j))
                )
                self.assertEqual(
...
class TestHelpers(unittest.TestCase):
    ...
    def test_dunder(self):
        ...
        for name in (
            (self.sunder_names + self.private_names) + self.private_and_sunder_names
        ) + self.random_names:
    ...
    def test_is_private(self):
        for name in self.private_names + self.private_and_sunder_names:
            self.assertTrue(
                enum._is_private("MyEnum", name), "%r is a not private name?"
            )
...
class TestStdLib(unittest.TestCase):
    ...
    def test_inspect_classify_class_attrs(self):
        ...
        values = [
            Attribute(
                name="__class__", kind="data", defining_class=object, object=EnumType
            ),
            Attribute(
                name="__doc__",
                kind="data",
    ...
    def test_test_simple_enum(self):
        ...
        self.assertEqual(
            list(CheckedMissing),
            [CM.SIXTY_FOUR, CM.ONE_TWENTY_EIGHT, CM.TWENTY_FORTY_EIGHT],
        )
...
class TestIntEnumConvert(unittest.TestCase):
    ...
    def test_convert_repr_and_str(self):
        ...
        self.assertEqual(
            ...
            ("%s.CONVERT_STRING_TEST_NAME_A" % module),
        )
        self.assertEqual(
            str(test_type.CONVERT_STRING_TEST_NAME_A), "CONVERT_STRING_TEST_NAME_A"
        )
...
class TestStrEnumConvert(unittest.TestCase):
    ...
    def test_convert_repr_and_str(self):
        module = ("test.test_enum", "__main__")[(__name__ == "__main__")]
        test_type = enum.StrEnum._convert_(
            "UnittestConvert", module, filter=(lambda x: x.startswith("CONVERT_STR_"))
        )
        self.assertEqual(
            repr(test_type.CONVERT_STR_TEST_1), ("%s.CONVERT_STR_TEST_1" % module)
        )
