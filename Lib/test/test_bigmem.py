...
class BaseStrTest:
    ...
    def test_find(self, size):
        ...
        self.assertEqual(
            s.find(_("i"), sublen), ((sublen + size) + SUBSTR.find(_("i")))
        )
    ...
    def test_index(self, size):
        ...
        self.assertEqual(
            s.index(_("i"), sublen), ((sublen + size) + SUBSTR.index(_("i")))
        )
        self.assertEqual(
            s.index(_("i"), size), ((sublen + size) + SUBSTR.index(_("i")))
        )
    ...
    def test_rindex(self, size):
        ...
        self.assertEqual(
            s.rindex(_(" "), 0, ((sublen + size) - 1)), SUBSTR.rindex(_(" "))
        )
    ...
    def test_slice_and_getitem(self, size):
        ...
        for i in range(0, (len(s) - stepsize), stepsize):
            ...
            if i > 0:
                self.assertEqual(
                    s[((i + sublen) - 1) : (i - 1) : (-3)], SUBSTR[sublen::(-3)]
                )
...
class StrTest(unittest.TestCase, BaseStrTest):
    ...
    @bigmemtest(
        size=((_2G // 5) + 1),
        memuse=((ucs2_char_size + ucs4_char_size) + (ascii_char_size * 6)),
    )
    ...
    @bigmemtest(
        size=((_2G // 5) + 1), memuse=((ucs4_char_size * 2) + (ascii_char_size * 10))
    )
...
def test_main():
    support.run_unittest(StrTest, BytesTest, BytearrayTest, TupleTest, ListTest)
