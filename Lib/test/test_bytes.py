...
class BaseBytesTest:
    ...
    def test_compare_to_str(self):
        ...
        self.assertEqual(
            (self.type2test(b"\x00\x00\x00a\x00\x00\x00b\x00\x00\x00c") == "abc"), False
        )
        self.assertEqual((self.type2test(b"a\x00b\x00c\x00") == "abc"), False)
        self.assertEqual(
            (self.type2test(b"a\x00\x00\x00b\x00\x00\x00c\x00\x00\x00") == "abc"), False
        )
    ...
    def test_find(self):
        ...
        for index in ((-1), 256, (sys.maxsize + 1)):
            self.assertRaisesRegex(
                ValueError, "byte must be in range\\(0, 256\\)", b.find, index
            )
    ...
    def test_maketrans(self):
        ...
        self.assertEqual(self.type2test.maketrans(b"\xfd\xfe\xff", b"xyz"), transtable)
    ...
    def test_find_etc_raise_correct_error_messages(self):
        ...
        self.assertRaisesRegex(
            TypeError, "\\bstartswith\\b", b.startswith, x, None, None, None
        )
        self.assertRaisesRegex(
            TypeError, "\\bendswith\\b", b.endswith, x, None, None, None
        )
...
class ByteArrayTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_setslice(self):
        ...
        for elem in [5, (-5), 0, int(1e21), "str", 2.3, ["a", "b"], [b"a", b"b"], [[]]]:
            with self.assertRaises(TypeError):
                b[3:4] = elem
...
class AssortedBytesTest(unittest.TestCase):
    ...
    def test_format(self):
        for b in (b"abc", bytearray(b"abc")):
            ...
            with self.assertRaisesRegex(
                TypeError, ("\\b%s\\b" % re.escape(type(b).__name__))
            ):
                format(b, "s")
    ...
    def test_literal(self):
        tests = [
            ...
            (b"Wonderful spam too", "Wonderful spam too"),
            (b"\xaa\x00\x00\x80", "ª\x00\x00\x80"),
