...
class TracebackCases(unittest.TestCase):
    ...
    def test_caret(self):
        ...
        err = self.get_exception_format(
            self.syntax_error_with_caret_non_ascii, SyntaxError
        )
        self.assertIn("^", err[2])
        self.assertEqual(err[2].count("\n"), 1)
        self.assertEqual((err[1].find("+") + 1), err[2].find("^"))
    ...
    def test_bad_indentation(self):
        err = self.get_exception_format(
            self.syntax_error_bad_indentation, IndentationError
        )
        self.assertEqual(len(err), 4)
        ...
        self.assertEqual((err[1].find(")") + 1), err[2].find("^"))
        err = self.get_exception_format(
            self.syntax_error_bad_indentation2, IndentationError
        )
        self.assertEqual(len(err), 3)
        self.assertEqual(err[1].strip(), "print(2)")
...
"""
        ...
        self.assertIn(
            int(re.search("\\d+", actual[(-2)]).group()),
...
class BaseExceptionReportingTests:
    ...
    def test_syntax_error_no_lineno(self):
        ...
        msg = self.get_report(e).splitlines()
        self.assertEqual(
            msg, ['  File "<string>", line 100', "SyntaxError: bad syntax"]
        )
        e = SyntaxError("bad syntax")
        ...
        msg = self.get_report(e).splitlines()
        self.assertEqual(
            msg, ['  File "myfile.py", line 100', "SyntaxError: bad syntax"]
        )
    ...
    def test_syntax_error_various_offsets(self):
        for offset in range((-5), 10):
            for add in [0, 2]:
                ...
                expected.append("")
                err = self.get_report(
                    SyntaxError("msg", ("file.py", 1, (offset + add), text))
                )
                exp = "\n".join(expected)
...
class LimitTests(unittest.TestCase):
    ...
    def test_extract_stack(self):
        ...
        with support.swap_attr(sys, "tracebacklimit", 1000):
            ...
            self.assertEqual(extract(limit=(-2)), nolim[:2])
            assertEqualExcept(
                extract(limit=(-100)), nolim[:100], ((len(nolim) - 5) - 1)
            )
...
class TestFrame(unittest.TestCase):
    def test_basics(self):
        ...
        self.assertEqual(
            tuple(f), ("f", 1, "dummy", '"""Test cases for traceback module"""')
        )
