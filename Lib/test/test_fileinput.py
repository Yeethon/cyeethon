...
class BufferSizesTests(BaseTests, unittest.TestCase):
    def test_buffer_sizes(self):
        t1 = self.writeTmp(
            "".join((("Line %s of file 1\n" % (i + 1)) for i in range(15)))
        )
        t2 = self.writeTmp(
            "".join((("Line %s of file 2\n" % (i + 1)) for i in range(10)))
        )
        t3 = self.writeTmp(
            "".join((("Line %s of file 3\n" % (i + 1)) for i in range(5)))
        )
        t4 = self.writeTmp(
            "".join((("Line %s of file 4\n" % (i + 1)) for i in range(1)))
        )
...
class FileInputTests(BaseTests, unittest.TestCase):
    ...
    def test_file_opening_hook(self):
        try:
            ...
            self.fail(
                "FileInput should raise if both inplace and openhook arguments are given"
            )
        except ValueError:
            pass
        try:
