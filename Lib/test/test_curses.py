...
class TestCurses(unittest.TestCase):
    ...
    def test_beep(self):
        if (curses.tigetstr("bel") is not None) or (
            curses.tigetstr("flash") is not None
        ):
            curses.beep()
        else:
            try:
    ...
    def get_pair_limit(self):
        ...
        if hasattr(curses, "ncurses_version"):
            ...
            if (not curses.has_extended_color_support()) or (
                (6, 1) <= curses.ncurses_version < (6, 2)
            ):
    ...
    def test_use_default_colors(self):
        ...
        self.assertIn(
            old, [(curses.COLOR_WHITE, curses.COLOR_BLACK), ((-1), (-1)), (0, 0)]
        )
