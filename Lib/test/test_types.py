...
class TypesTests(unittest.TestCase):
    ...
    def test_format_spec_errors(self):
        ...
        self.assertRaises(ValueError, format, 0, (("." + ("1" * 10000)) + "d"))
        self.assertRaises(
            ValueError, format, 0, (((("1" * 1000) + ".") + ("1" * 10000)) + "d")
        )
        for code in "xXobns":
    ...
    def test_or_types_operator(self):
        ...
        self.assertEqual(
            (typing.List[int] | None), typing.Union[(typing.List[int], None)]
        )
        self.assertEqual(
            (None | typing.List[int]), typing.Union[(None, typing.List[int])]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            (
                (
                    (
                        (((((BaseException | bool) | bytes) | complex) | float) | int)
                        | list
                    )
                    | map
                )
    ...
    def test_or_type_operator_with_forward(self):
        ...
        assert typing.get_args(typing.get_type_hints(forward_after)["x"]) == (
            int,
            Forward,
        )
        assert typing.get_args(typing.get_type_hints(forward_before)["x"]) == (
            int,
            Forward,
        )
    ...
    def test_or_type_operator_with_genericalias(self):
        ...
        union_ga = (
            (int | list[str]),
            (int | collections.abc.Callable[(..., str)]),
            (int | d),
        )
