...
class TestRmTree(BaseTest, unittest.TestCase):
    ...
    def test_on_error(self):
        ...
        self.assertEqual(
            self.errorState, 3, "Expected call to onerror function did not happen."
        )
...
class TestCopyTree(BaseTest, unittest.TestCase):
    ...
    def test_copytree_dirs_exist_ok(self):
        ...
        self.assertTrue(
            os.path.isfile(os.path.join(dst_dir, "existing_dir", "existing.txt"))
        )
    ...
    def test_copytree_with_exclude(self):
        ...
        try:
            ...
            try:
                ...
                self.assertFalse(
                    exists(join(dst_dir, "test_dir2", "subdir2", "test.py"))
                )
                self.assertFalse(exists(join(dst_dir, "test_dir2", "subdir")))
            finally:
                shutil.rmtree(dst_dir)
    ...
    def test_copytree_retains_permissions(self):
        ...
        restrictive_subdir_dst = os.path.join(
            dst_dir, os.path.split(restrictive_subdir)[1]
        )
        self.assertEqual(
            os.stat(restrictive_subdir).st_mode, os.stat(restrictive_subdir_dst).st_mode
        )
    ...
    @os_helper.skip_unless_symlink
    @unittest.skipIf(
        (sys.platform == "vxworks"), "fifo requires special path on VxWorks"
    )
...
class TestCopy(BaseTest, unittest.TestCase):
        ...
        if os.utime in os.supports_follow_symlinks:
            for attr in ("st_atime", "st_mtime"):
                self.assertLessEqual(
                    getattr(src_link_stat, attr), (getattr(dst_link_stat, attr) + 1)
                )
        if hasattr(os, "lchmod"):
    ...
    @unittest.skipUnless(
        (
            hasattr(os, "chflags")
            and hasattr(errno, "EOPNOTSUPP")
            and hasattr(errno, "ENOTSUP")
        ),
    ...
    def test_copyxattr_symlinks(self):
        ...
        self.assertEqual(
            os.getxattr(dst_link, "trusted.foo", follow_symlinks=False), b"43"
        )
    ...
    def test_copy2_symlinks(self):
        ...
        if os.utime in os.supports_follow_symlinks:
            for attr in ("st_atime", "st_mtime"):
                self.assertLessEqual(
                    getattr(src_link_stat, attr), (getattr(dst_stat, attr) + 1)
                )
        if hasattr(os, "lchmod"):
    ...
    def test_copyfile_named_pipe(self):
        ...
        try:
            ...
            self.assertRaises(
                shutil.SpecialFileError, shutil.copyfile, __file__, TESTFN
            )
        finally:
            os.remove(TESTFN)
...
class TestArchives(BaseTest, unittest.TestCase):
    ...
    def test_tarfile_root_owner(self):
        ...
        with os_helper.change_cwd(root_dir):
            archive_name = make_archive(
                base_name, "gztar", root_dir, "dist", owner=owner, group=group
            )
    ...
    @unittest.skipIf(
        (AIX and (not _maxdataOK())), "AIX MAXDATA must be 0x20000000 or larger"
    )
...
class TestWhich(BaseTest, unittest.TestCase):
    ...
    def test_environ_path_empty(self):
        with os_helper.EnvironmentVarGuard() as env:
            ...
            ), support.swap_attr(os, "defpath", self.dir), os_helper.change_cwd(
                self.dir
            ):
                rv = shutil.which(self.file)
                self.assertIsNone(rv)
...
class TestMove(BaseTest, unittest.TestCase):
        ...
        try:
            for (src, dst) in [("srcdir", "srcdir/dest")]:
                ...
                self.assertTrue(
                    ...
                    msg=(
                        "_destinsrc() wrongly concluded that dst (%s) is not in src (%s)"
                        % (dst, src)
                    ),
                )
        finally:
        ...
        try:
            for (src, dst) in [("srcdir", "src/dest"), ("srcdir", "srcdir.new")]:
                ...
                self.assertFalse(
                    shutil._destinsrc(src, dst),
                    msg=(
                        "_destinsrc() wrongly concluded that dst (%s) is in src (%s)"
                        % (dst, src)
                    ),
                )
        finally:
    ...
    def test_move_return_value(self):
        ...
        self.assertEqual(
            rv, os.path.join(self.dst_dir, os.path.basename(self.src_file))
        )
    ...
    def test_move_dir_caseinsensitive(self):
        ...
        dst_dir = os.path.join(
            os.path.dirname(self.src_dir), os.path.basename(self.src_dir).upper()
        )
...
class TestZeroCopySendfile(_ZeroCopyFileTest, unittest.TestCase):
    ...
    def test_blocksize_arg(self):
        with unittest.mock.patch("os.sendfile", side_effect=ZeroDivisionError) as m:
            ...
            self.assertRaises(
                ZeroDivisionError, shutil.copyfile, TESTFN2, (TESTFN2 + "3")
            )
            blocksize = m.call_args[0][3]
...
class TestGetTerminalSize(unittest.TestCase):
    ...
    @unittest.skipUnless(
        hasattr(os, "get_terminal_size"), "need os.get_terminal_size()"
    )
    ...
    def test_fallback(self):
        with os_helper.EnvironmentVarGuard() as env:
            ...
            with open(os.devnull, "w", encoding="utf-8") as f, support.swap_attr(
                sys, "__stdout__", f
            ):
