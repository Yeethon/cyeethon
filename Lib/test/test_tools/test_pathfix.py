...
class TestPathfixFunctional(unittest.TestCase):
    ...
    def pathfix(
        self, shebang, pathfix_flags, exitcode=0, stdout="", stderr="", directory=""
    ):
        if directory:
...
"""
        ...
        self.assertEqual(
            proc.stdout.decode(encoding), stdout.replace("\n", os.linesep), proc
        )
        self.assertEqual(
            proc.stderr.decode(encoding), stderr.replace("\n", os.linesep), proc
        )
...
"""
    ...
    def test_pathfix(self):
        self.assertEqual(
            self.pathfix("#! /usr/bin/env python", ["-i", "/usr/bin/python3"]),
            "#! /usr/bin/python3",
        )
        self.assertEqual(
            self.pathfix("#! /usr/bin/env python -R", ["-i", "/usr/bin/python3"]),
            "#! /usr/bin/python3",
        )

    def test_pathfix_keeping_flags(self):
        self.assertEqual(
            self.pathfix("#! /usr/bin/env python -R", ["-i", "/usr/bin/python3", "-k"]),
            "#! /usr/bin/python3 -R",
        )
        self.assertEqual(
            self.pathfix("#! /usr/bin/env python", ["-i", "/usr/bin/python3", "-k"]),
            "#! /usr/bin/python3",
        )
    ...
    def test_pathfix_adding_flag(self):
        self.assertEqual(
            self.pathfix(
                "#! /usr/bin/env python", ["-i", "/usr/bin/python3", "-a", "s"]
            ),
            "#! /usr/bin/python3 -s",
        )
        self.assertEqual(
            self.pathfix(
                "#! /usr/bin/env python -S", ["-i", "/usr/bin/python3", "-a", "s"]
            ),
            "#! /usr/bin/python3 -s",
        )
        self.assertEqual(
            self.pathfix(
                "#! /usr/bin/env python -V", ["-i", "/usr/bin/python3", "-a", "v", "-k"]
            ),
            "#! /usr/bin/python3 -vV",
        )
        self.assertEqual(
            self.pathfix(
                "#! /usr/bin/env python", ["-i", "/usr/bin/python3", "-a", "Rs"]
            ),
            "#! /usr/bin/python3 -Rs",
        )
        self.assertEqual(
