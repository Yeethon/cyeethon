...
class ReprTests(unittest.TestCase):
    ...
    def test_cell(self):
        ...
        self.assertRegex(
            repr(x), "<cell at 0x[0-9A-Fa-f]+: int object at 0x[0-9A-Fa-f]+>"
        )
...
class LongReprTest(unittest.TestCase):
    ...
    def _check_path_limitations(self, module_name):
        ...
        cached_path_len = (
            source_path_len + len(importlib.util.cache_from_source("x.py"))
        ) - len("x.py")
    ...
    def test_type(self):
        ...
        eq = self.assertEqual
        write_file(
            os.path.join(self.subpkgname, "foo.py"), "class foo(object):\n    pass\n"
        )
        importlib.invalidate_caches()
        from areallylongpackageandmodulenametotestreprtruncation.areallylongpackageandmodulenametotestreprtruncation import (
            foo,
