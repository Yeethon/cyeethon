...
def hello_app(environ, start_response):
    start_response(
        "200 OK",
        [("Content-Type", "text/plain"), ("Date", "Mon, 05 Jun 2006 18:49:54 GMT")],
    )
...
class IntegrationTests(TestCase):
    ...
    def test_simple_validation_error(self):
        ...
        self.assertTrue(
            out.endswith(b"A server error occurred.  Please contact the administrator.")
        )
        self.assertEqual(
            err.splitlines()[(-2)],
            "AssertionError: Headers (('Content-Type', 'text/plain')) must be of type list: <class 'tuple'>",
        )
...
class UtilityTests(TestCase):
    ...
    def testAppURIs(self):
        ...
        self.checkAppURI(
            ...
            HTTP_HOST="spam.example.com:2071",
            SERVER_PORT="2071",
        )
        self.checkAppURI("http://spam.example.com/", SERVER_NAME="spam.example.com")
        self.checkAppURI(
            "http://127.0.0.1/", HTTP_HOST="127.0.0.1", SERVER_NAME="spam.example.com"
        )
    ...
    def testReqURIs(self):
        ...
        self.checkReqURI(
            "http://127.0.0.1/spammity/spam", SCRIPT_NAME="/spammity", PATH_INFO="/spam"
        )
        self.checkReqURI(
            "http://127.0.0.1/spammity/sp%E4m",
            SCRIPT_NAME="/spammity",
            PATH_INFO="/späm",
        )
        self.checkReqURI(
        ...
        self.checkReqURI(
            ...
            SCRIPT_NAME="/spammity",
            PATH_INFO="/spam;cookie=1234,5678",
        )
        self.checkReqURI(
        ...
        self.checkReqURI(
            "http://127.0.0.1/spammity/spam?s%E4y=ni",
            SCRIPT_NAME="/spammity",
            PATH_INFO="/spam",
            QUERY_STRING="s%E4y=ni",
        )
        self.checkReqURI(
    ...
    def testHopByHop(self):
        for (
            hop
        ) in "Connection Keep-Alive Proxy-Authenticate Proxy-Authorization TE Trailers Transfer-Encoding Upgrade".split():
...
class HandlerTests(TestCase):
    ...
    def testEnviron(self):
        ...
        for attr in (
            ...
            self.assertEqual(
                getattr(handler, ("wsgi_" + attr)), handler.environ[("wsgi." + attr)]
            )
        expected = {}
    ...
    def testContentLength(self):
        ...
        h.run(trivial_app2)
        self.assertEqual(
            h.stdout.getvalue(), "Status: 200 OK\r\n\r\nhttp".encode("iso-8859-1")
        )
        h = TestHandler()
        h.run(trivial_app3)
        self.assertEqual(
        ...
        h.run(trivial_app4)
        self.assertEqual(
            h.stdout.getvalue(), b"Status: 200 OK\r\nContent-Length: 12345\r\n\r\n"
        )
