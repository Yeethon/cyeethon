...
class NetworkedNNTPTestsMixin:
    ...
    @unittest.skipIf(
        True, "temporarily skipped until a permanent solution is found for issue #28971"
    )
    def test_over(self):
        (resp, count, first, last, name) = self.server.group(self.GROUP_NAME)
        start = last - 10
    ...
    def test_with_statement(self):
        ...
        try:
            server = self.NNTP_CLASS(
                self.NNTP_HOST, timeout=support.INTERNET_TIMEOUT, usenetrc=False
            )
            with server:
...
class NetworkedNNTPTests(NetworkedNNTPTestsMixin, unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with socket_helper.transient_internet(cls.NNTP_HOST):
            try:
                cls.server = cls.NNTP_CLASS(
                    cls.NNTP_HOST, timeout=support.INTERNET_TIMEOUT, usenetrc=False
                )
            except SSLError as ssl_err:
                if re.search("(?i)KEY.TOO.SMALL", ssl_err.reason):
                    raise unittest.SkipTest(
...
class NNTPv1v2TestsMixin:
    ...
    def test_newnews(self):
        ...
        expected = "230 list of newsarticles (NNTP v{0}) created after Mon Sep 13 08:20:04 2010 follows".format(
            self.nntp_version
        )
