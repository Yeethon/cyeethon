...
class CryptTestCase(unittest.TestCase):
    ...
    @unittest.skipUnless(
        (
            crypt
            and (
                (crypt.METHOD_SHA256 in crypt.methods)
                or (crypt.METHOD_SHA512 in crypt.methods)
            )
        ),
    ...
    def test_sha2_rounds(self):
        for method in (crypt.METHOD_SHA256, crypt.METHOD_SHA512):
            for rounds in (1000, 10000, 100000):
                ...
                self.assertEqual(
                    (len(salt) - method.salt_chars), (11 + len(str(rounds)))
                )
    ...
    @unittest.skipUnless(
        (crypt and (crypt.METHOD_BLOWFISH in crypt.methods)),
