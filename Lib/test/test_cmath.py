    ...
    def rAssertAlmostEqual(self, a, b, rel_err=2e-15, abs_err=5e-323, msg=None):
        ...
        if math.isinf(a):
            ...
            self.fail(
                (
                    msg
                    or "finite result where infinity expected: expected {!r}, got {!r}".format(
                        a, b
                    )
                )
            )
        if (not a) and (not b):
            if math.copysign(1.0, a) != math.copysign(1.0, b):
                self.fail(
                    (msg or "zero has wrong sign: expected {!r}, got {!r}".format(a, b))
                )
        try:
        ...
        for (id, fn, ar, ai, er, ei, flags) in parse_testfile(test_file):
            ...
            if ("divide-by-zero" in flags) or ("invalid" in flags):
                try:
                    ...
                    self.fail(
                        "ValueError not raised in test {}: {}(complex({!r}, {!r}))".format(
                            id, fn, ar, ai
                        )
                    )
            if "overflow" in flags:
                try:
                    ...
                    self.fail(
                        "OverflowError not raised in test {}: {}(complex({!r}, {!r}))".format(
                            id, fn, ar, ai
                        )
                    )
            actual = function(arg)
            ...
            if fn in ("log", "log10"):
                ...
                real_abs_err = 5e-323
            error_message = "{}: {}(complex({!r}, {!r}))\nExpected: complex({!r}, {!r})\nReceived: complex({!r}, {!r})\nReceived value insufficiently close to expected value.".format(
...
class IsCloseTests(test_math.IsCloseTests):
    ...
    def test_complex_values(self):
        complex_examples = [
            ((1.0 + 1j), (1.000000000001 + 1j)),
            ((1.0 + 1j), (1.0 + 1.000000000001j)),
            (((-1.0) + 1j), ((-1.000000000001) + 1j)),
            ((1.0 - 1j), (1.0 - 0.999999999999j)),
        ]
