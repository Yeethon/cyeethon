...
def requires_load_dynamic(meth):
    ...
    return unittest.skipIf(
        (not hasattr(imp, "load_dynamic")), "imp.load_dynamic() required"
    )(meth)
...
class ImportTests(unittest.TestCase):
    ...
    def test_issue9319(self):
        path = os.path.dirname(__file__)
        self.assertRaises(SyntaxError, imp.find_module, "badsyntax_pep3120", [path])
    ...
    def test_bug7732(self):
        with os_helper.temp_cwd():
            ...
            self.assertRaisesRegex(
                ImportError, "^No module", imp.find_module, os_helper.TESTFN, ["."]
            )

    def test_multiple_calls_to_get_data(self):
        loader = imp._LoadSourceCompatibility(
            "imp", imp.__file__, open(imp.__file__, encoding="utf-8")
        )
...
class NullImporterTests(unittest.TestCase):
    @unittest.skipIf(
        (os_helper.TESTFN_UNENCODABLE is None), "Need an undecodeable filename"
    )
