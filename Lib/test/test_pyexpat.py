...
class ParseTest(unittest.TestCase):
    class Outputter:
        ...
        def StartElementHandler(self, name, attrs):
            self.out.append(
                ((("Start element: " + repr(name)) + " ") + sortdict(attrs))
            )
    ...
    def _verify_parse_output(self, operations):
        expected_operations = [
            ...
            ("Entity declaration", ("acirc", 0, "â", None, None, None, None)),
            (
                "Entity declaration",
...
class HandlerExceptionTest(unittest.TestCase):
    ...
    def test_exception(self):
        ...
        try:
            ...
            if sysconfig.is_python_build() and (
                not ((sys.platform == "win32") and (platform.machine() == "ARM"))
            ):
...
class ChardataBufferTest(unittest.TestCase):
    ...
    def small_buffer_test(self, buffer_len):
        xml = (
            b"<?xml version='1.0' encoding='iso8859'?><s>" + (b"a" * buffer_len)
        ) + b"</s>"
        parser = expat.ParserCreate()
        parser.CharacterDataHandler = self.counting_handler
        parser.buffer_size = 1024
