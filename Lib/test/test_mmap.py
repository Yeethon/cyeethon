...
class MmapTests(unittest.TestCase):
    ...
    def test_access_parameter(self):
        ...
        with open(TESTFN, "rb") as f:
            ...
            self.assertEqual(
                m[:], (b"a" * mapsize), "Readonly memory map data incorrect."
            )
            try:
                m[:] = b"b" * mapsize
            except TypeError:
    ...
    def test_move(self):
        with open(TESTFN, "wb+") as f:
            ...
            mf.move(5, 0, 5)
            self.assertEqual(
                mf[:], b"ABCDEABCDE", "Map move should have duplicated front 5"
            )
            mf.close()
        data = b"0123456789"
        for dest in range(len(data)):
            for src in range(len(data)):
                for count in range((len(data) - max(dest, src))):
                    expected = (data[:dest] + data[src : (src + count)]) + data[
                        (dest + count) :
                    ]
    ...
    def test_repr(self):
        open_mmap_repr_pat = re.compile(
            "<mmap.mmap closed=False, access=(?P<access>\\S+), length=(?P<length>\\d+), pos=(?P<pos>\\d+), offset=(?P<offset>\\d+)>"
        )
        ...
        offsets = tuple(
            (
                (
                    ((mapsize // 2) // mmap.ALLOCATIONGRANULARITY)
        ...
        for (offset, mapsize) in zip(offsets, mapsizes):
            ...
            with open(TESTFN, "wb+") as fp:
                ...
                for (access, pos) in itertools.product(accesses, positions):
                    ...
                    with mmap.mmap(
                        fp.fileno(), length, access=accint, offset=offset
                    ) as mm:
                        mm.seek(pos)
...
class LargeMmapTests(unittest.TestCase):
    ...
    def _make_test_file(self, num_zeroes, tail):
        if (sys.platform[:3] == "win") or (sys.platform == "darwin"):
            requires(
                "largefile",
                ("test requires %s bytes and a long time to run" % str(6442450944)),
            )
        f = open(TESTFN, "w+b")
        try:
            f.seek(num_zeroes)
    ...
    def test_large_offset(self):
        with self._make_test_file(5637144575, b" ") as f:
            with mmap.mmap(
                f.fileno(), 0, offset=5368709120, access=mmap.ACCESS_READ
            ) as m:
