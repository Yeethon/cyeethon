    ...
    def test_pipesizes(self):
        ...
        try:
            for fifo in [p.stdin, p.stdout, p.stderr]:
                self.assertEqual(
                    fcntl.fcntl(fifo.fileno(), fcntl.F_GETPIPE_SZ), pipesize
                )
            p.stdin.write(b"pear")
            p.stdin.close()
        finally:
...
class POSIXProcessTestCase(BaseTestCase):
    ...
    @unittest.skipIf(
        (not os.path.exists("/proc/self/status")), "need /proc/self/status"
    )
    ...
    def test_umask(self):
        ...
        try:
            ...
            subprocess.check_call(
                [sys.executable, "-c", f"open({name!r}, 'w').close()"], umask=43
            )
            ...
            with mock.patch.object(subprocess.Popen, "_wait") as mock__wait:
                ...
                for call in mock__wait.call_args_list[1:]:
                    self.assertNotEqual(
                        ...
                        f"no open-ended wait() after the first allowed: {mock__wait.call_args_list}",
                    )
                sigint_calls = []
