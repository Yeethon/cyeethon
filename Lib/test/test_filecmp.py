...
class FileCompareTestCase(unittest.TestCase):
    ...
    def test_matching(self):
        self.assertTrue(
            filecmp.cmp(self.name, self.name), "Comparing file to itself fails"
        )
        self.assertTrue(
            filecmp.cmp(self.name, self.name, shallow=False),
            "Comparing file to itself fails",
        )
        self.assertTrue(
            filecmp.cmp(self.name, self.name_same),
    ...
    def test_different(self):
        self.assertFalse(
            filecmp.cmp(self.name, self.name_diff), "Mismatched files compare as equal"
        )
        self.assertFalse(
            filecmp.cmp(self.name, self.dir), "File and directory compare as equal"
        )
    ...
    def test_cache_clear(self):
        ...
        filecmp.clear_cache()
        self.assertTrue(
            (len(filecmp._cache) == 0), "Cache not cleared after calling clear_cache"
        )
...
class DirCompareTestCase(unittest.TestCase):
    def setUp(self):
        ...
        for dir in (self.dir, self.dir_same, self.dir_diff, self.dir_ignored):
            ...
            with open(os.path.join(dir, fn), "w", encoding="utf-8") as output:
                output.write(data)
        with open(
            os.path.join(self.dir_diff, "file2"), "w", encoding="utf-8"
        ) as output:
    ...
    def test_cmpfiles(self):
        self.assertTrue(
            (filecmp.cmpfiles(self.dir, self.dir, ["file"]) == (["file"], [], [])),
        ...
        self.assertTrue(
            (
                filecmp.cmpfiles(self.dir, self.dir, ["file"], shallow=False)
                == (["file"], [], [])
            ),
            "Comparing directory to itself fails",
        )
        ...
        self.assertFalse(
            (
                filecmp.cmpfiles(self.dir, self.dir_same, ["file", "file2"])
                == (["file"], ["file2"], [])
            ),
    ...
    def test_dircmp(self):
        ...
        (left_dir, right_dir) = (self.dir, self.dir_diff)
        shutil.move(
            os.path.join(self.dir_diff, "file2"), os.path.join(self.dir, "file2")
        )
        d = filecmp.dircmp(left_dir, right_dir)
        self.assertEqual(d.left, left_dir)
        self.assertEqual(d.right, right_dir)
