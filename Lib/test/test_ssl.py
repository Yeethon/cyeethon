...
if is_ubuntu():
    ...
    def seclevel_workaround(*ctxs):
        ...
        for ctx in ctxs:
            if hasattr(ctx, "minimum_version") and (
...
class BasicSocketTests(unittest.TestCase):
        ...
        if support.verbose:
            sys.stdout.write(
                (
                    ...
                    % (
                        v,
                        ((v and "sufficient randomness") or "insufficient randomness"),
                    )
                )
            )
    ...
    def test_match_hostname(self):
        ...
        with self.assertRaisesRegex(
            ssl.CertificateError, "wildcard can only be present in the leftmost label"
        ):
        ...
        with self.assertRaisesRegex(
            ssl.CertificateError, "hostname 'com' doesn't match '\\*.com'"
        ):
...
class ContextTests(unittest.TestCase):
    ...
    def test_min_max_version(self):
        ...
        self.assertIn(ctx.maximum_version, {ssl.TLSVersion.TLSv1, ssl.TLSVersion.SSLv3})
        ...
        def wrap_conn(self):
            try:
                ...
                if self.server.context.verify_mode == ssl.CERT_REQUIRED:
                    ...
                    if support.verbose and self.server.chatty:
                        sys.stdout.write(
                            ((" cert binary is " + str(len(cert_binary))) + " bytes\n")
                        )
...
class ThreadedTests(unittest.TestCase):
        ...
        with server:
            with client_context.wrap_socket(
                ...
                if (("organizationName", "Python Software Foundation"),) not in cert[
                    ...
                    self.fail(
                        "Missing or invalid 'organizationName' field in certificate subject; should be 'Python Software Foundation'."
                    )
    ...
    def test_protocol_sslv2(self):
        ...
        try_protocol_combo(
            ssl.PROTOCOL_SSLv2, ssl.PROTOCOL_SSLv2, True, ssl.CERT_OPTIONAL
        )
        try_protocol_combo(
            ssl.PROTOCOL_SSLv2, ssl.PROTOCOL_SSLv2, True, ssl.CERT_REQUIRED
        )
        ...
        try_protocol_combo(
            ssl.PROTOCOL_SSLv2, ssl.PROTOCOL_TLS, False, client_options=ssl.OP_NO_SSLv3
        )
        try_protocol_combo(
            ssl.PROTOCOL_SSLv2, ssl.PROTOCOL_TLS, False, client_options=ssl.OP_NO_TLSv1
        )
        ...
        with server:
            ...
            for (
                ...
                try:
                    ...
                    if outdata != indata.lower():
                        self.fail(
                            "While sending with <<{name:s}>> bad data <<{outdata:r}>> ({nout:d}) received; expected <<{indata:r}>> ({nin:d})\n".format(
                                name=meth_name,
                                outdata=outdata[:20],
                                nout=len(outdata),
                                indata=indata[:20],
                    ...
                    if expect_success:
                        self.fail(
                            "Failed to send with method <<{name:s}>>; expected to succeed.\n".format(
                                name=meth_name
                            )
                        )
                    if not str(e).startswith(meth_name):
                        self.fail(
                            "Method <<{name:s}>> failed with unexpected exception message: {exp:s}\n".format(
            ...
            for (meth_name, recv_meth, expect_success, args) in recv_methods:
                ...
                try:
                    ...
                    if outdata != indata.lower():
                        self.fail(
                            "While receiving with <<{name:s}>> bad data <<{outdata:r}>> ({nout:d}) received; expected <<{indata:r}>> ({nin:d})\n".format(
                                name=meth_name,
                                outdata=outdata[:20],
                                nout=len(outdata),
                                indata=indata[:20],
                    ...
                    if expect_success:
                        self.fail(
                            "Failed to receive with method <<{name:s}>>; expected to succeed.\n".format(
                                name=meth_name
                            )
                        )
                    if not str(e).startswith(meth_name):
