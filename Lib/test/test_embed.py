...
class EmbeddingTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    def test_pre_initialization_api(self):
        ...
        (out, err) = self.run_embedded_interpreter(
            "test_pre_initialization_api", env=env
        )
        if MS_WINDOWS:
...
class InitConfigTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    if MS_WINDOWS:
        PRE_CONFIG_COMPAT.update({"legacy_windows_fs_encoding": 0})
    ...
    COPY_PRE_CONFIG = ["dev_mode", "isolated", "use_environment"]
    CONFIG_COMPAT = {
        "_config_init": API_COMPAT,
        "isolated": 0,
        "use_environment": 1,
    ...
    if MS_WINDOWS:
        CONFIG_COMPAT.update({"legacy_windows_stdio": 0})
    CONFIG_PYTHON = dict(
        CONFIG_COMPAT, _config_init=API_PYTHON, configure_c_stdio=1, parse_argv=2
    )
    ...
    COPY_GLOBAL_PRE_CONFIG = [("Py_UTF8Mode", "utf8_mode")]
    COPY_GLOBAL_CONFIG = [
    ...
    if MS_WINDOWS:
        ...
        PATH_CONFIG = {}
    COPY_PATH_CONFIG = ["prefix", "exec_prefix", "program_name", "home"]
    if MS_WINDOWS:
        COPY_PATH_CONFIG.extend(("base_executable",))
    ...
    def _get_expected_config_impl(self):
        ...
        if proc.returncode:
            raise Exception(
                f"failed to get the default config: stdout={proc.stdout!r} stderr={proc.stderr!r}"
            )
    ...
    def check_path_config(self, configs, expected):
        ...
        for key in self.COPY_PATH_CONFIG:
            expected[key] = config[key]
        expected["module_search_path"] = os.path.pathsep.join(
    ...
    def test_init_warnoptions(self):
        warnoptions = [
            ...
            "ignore:::cmdline2",
            "default::BytesWarning",
            "ignore:::PySys_AddWarnOption1",
            "ignore:::PySys_AddWarnOption2",
            "ignore:::PyConfig_BeforeRead",
