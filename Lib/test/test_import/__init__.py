...
class ImportTests(unittest.TestCase):
    ...
    def test_from_import_missing_attr_has_name_and_so_path(self):
        ...
        self.assertRegex(
            str(cm.exception),
            "cannot import name 'i_dont_exist' from '_testcapi' \\(.*\\.(so|pyd)\\)",
        )
    ...
    def test_import_in_del_does_not_crash(self):
        testfn = script_helper.make_script(
            "",
            TESTFN,
            textwrap.dedent(
                "            import sys\n            class C:\n               def __del__(self):\n                  import importlib\n            sys.argv.insert(0, C())\n            "
...
class PathsTests(unittest.TestCase):
    ...
    def test_trailing_slash(self):
        with open(
            os.path.join(self.path, "test_trailing_slash.py"), "w", encoding="utf-8"
        ) as f:
...
class RelativeImportTests(unittest.TestCase):
    ...
    def test_parentless_import_shadowed_by_global(self):
        script_helper.assert_python_failure(
            "-W", "ignore", "-c", "foo = 1; from . import foo"
        )
...
class PycacheTests(unittest.TestCase):
    ...
    def test_package___cached__(self):
        ...
        self.assertEqual(
            sys.modules["pep3147.foo"].__cached__,
...
class CircularImportTests(unittest.TestCase):
    ...
    def test_circular_from_import(self):
        ...
        self.assertIn(
            "cannot import name 'b' from partially initialized module 'test.test_import.data.circular_imports.from_cycle1' (most likely due to a circular import)",
            str(cm.exception),
        )
