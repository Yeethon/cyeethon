...
class DebuggingServerTest(unittest.TestCase):
    ...
    def test_process_message_with_enable_SMTPUTF8_true(self):
        server = smtpd.DebuggingServer(
            (socket_helper.HOST, 0), ("b", 0), enable_SMTPUTF8=True
        )
        (conn, addr) = server.accept()
...
class TestMailOptionParsing(unittest.TestCase):
    ...
    def test_with_decode_data_false(self):
        ...
        for line in [
            ...
            self.assertEqual(channel.socket.last, self.error_response)
        self.write_line(
            channel, b"MAIL from: <foo@example.com> size=20 SMTPUTF8 BODY=UNKNOWN"
        )
        self.assertEqual(
            channel.socket.last,
    ...
    def test_with_enable_smtputf8_true(self):
        ...
        self.write_line(channel, b"EHLO example")
        self.write_line(
            channel, b"MAIL from: <foo@example.com> size=20 body=8bitmime smtputf8"
        )
        self.assertEqual(channel.socket.last, b"250 OK\r\n")
...
class SMTPDChannelTest(unittest.TestCase):
    ...
    def test_HELP_command(self):
        ...
        self.assertEqual(
            self.channel.socket.last, b"250 Syntax: MAIL FROM: <address>\r\n"
        )
    ...
    def test_HELO_parameter_rejected_when_extensions_not_enabled(self):
        ...
        self.assertEqual(
            self.channel.socket.last, b"501 Syntax: MAIL FROM: <address>\r\n"
        )
    ...
    def test_MAIL_command_limit_extended_with_SIZE(self):
        ...
        self.assertEqual(self.channel.socket.last, b"250 OK\r\n")
        self.write_line(
            ((b"MAIL from:<" + (b"a" * (fill_len + 26))) + b"@example> SIZE=1234")
        )
        self.assertEqual(self.channel.socket.last, b"500 Error: line too long\r\n")
    ...
    def test_nested_MAIL(self):
        ...
        self.assertEqual(
            self.channel.socket.last, b"503 Error: nested MAIL command\r\n"
        )
    ...
    def test_RCPT_syntax_HELO(self):
        ...
        self.assertEqual(
            self.channel.socket.last, b"501 Syntax: RCPT TO: <address>\r\n"
        )
    ...
    def test_data_dialog(self):
        ...
        self.write_line(b"DATA")
        self.assertEqual(
            self.channel.socket.last, b"354 End data with <CR><LF>.<CR><LF>\r\n"
        )
        self.write_line(b"data\r\nmore\r\n.")
        self.assertEqual(self.channel.socket.last, b"250 OK\r\n")
        self.assertEqual(
...
class SMTPDChannelWithDecodeDataTrue(unittest.TestCase):
    ...
    def test_utf8_data(self):
        ...
        self.write_line(b".")
        self.assertEqual(
            self.channel.received_data, "utf8 enriched text: żźć\nand some plain ascii"
        )
...
class SMTPDChannelTestWithEnableSMTPUTF8True(unittest.TestCase):
    ...
    def test_process_smtputf8_message(self):
        ...
        for mail_parameters in [b"", b"BODY=8BITMIME SMTPUTF8"]:
            ...
            if mail_parameters == b"":
                ...
                self.assertEqual(
                    self.channel.socket.last, b"250 SMTPUTF8 message okish\r\n"
                )
    ...
    def test_utf8_data(self):
        ...
        self.write_line(b".")
        self.assertEqual(
            self.channel.received_data, b"utf8 enriched text: \xc5\xbc\xc5\xba\xc4\x87"
        )
