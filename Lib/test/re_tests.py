...
tests = [
    ...
    ("\\0", "\x00", SUCCEED, "found", "\x00"),
    ("[\\0a]", "\x00", SUCCEED, "found", "\x00"),
    ("[a\\0]", "\x00", SUCCEED, "found", "\x00"),
    ("[^a\\0]", "\x00", FAIL),
    (
        "\\a[\\b]\\f\\n\\r\\t\\v",
        "\x07\x08\x0c\n\r\t\x0b",
    ...
    ("\\u", "", SYNTAX_ERROR),
    ("\\x00ffffffffffffff", "ÿ", FAIL, "found", chr(255)),
    ("\\x00f", "\x0f", FAIL, "found", chr(15)),
    ("\\x00fe", "þ", FAIL, "found", chr(254)),
    (
        "^\\w+=(\\\\[\\000-\\277]|[^\\n\\\\])*",
    ...
    ("(([a-z]+):)?([a-z]+)$", "smil", SUCCEED, 'g1+"-"+g2+"-"+g3', "None-None-smil"),
    ("((.)\\1+)", "", SYNTAX_ERROR),
    (".*d", "abc\nabd", SUCCEED, "found", "abd"),
    ("(", "", SYNTAX_ERROR),
    ("[\\41]", "!", SUCCEED, "found", "!"),
    ("(x?)?", "x", SUCCEED, "found", "x"),
    ("(?x) foo ", "foo", SUCCEED, "found", "foo"),
    ("(?<!abc)(d.f)", "abcdefdof", SUCCEED, "found", "dof"),
    ("[\\w-]+", "laser_beam", SUCCEED, "found", "laser_beam"),
    (".*?\\S *:", "xx:", SUCCEED, "found", "xx:"),
    ("a[ ]*?\\ (\\d+).*", "a   10", SUCCEED, "found", "a   10"),
    ("a[ ]*?\\ (\\d+).*", "a    10", SUCCEED, "found", "a    10"),
    ("(?ms).*?x\\s*\\Z(.*)", "xx\nx\n", SUCCEED, "g1", ""),
    ("(?i)M+", "MMM", SUCCEED, "found", "MMM"),
    ("(?i)m+", "MMM", SUCCEED, "found", "MMM"),
    ("(?i)[M]+", "MMM", SUCCEED, "found", "MMM"),
    ("(?i)[m]+", "MMM", SUCCEED, "found", "MMM"),
    ("^*", "", SYNTAX_ERROR),
    ...
    ("a[^>]*?b", "a>b", FAIL),
    ("^a*?$", "foo", FAIL),
    ("^((a)c)?(ab)$", "ab", SUCCEED, 'g1+"-"+g2+"-"+g3', "None-None-ab"),
...
tests.extend(
    [
        ("\\b.\\b", "a", SUCCEED, "found", "a"),
        ("(?u)\\b.\\b", u, SUCCEED, "found", u),
        ("(?u)\\w", u, SUCCEED, "found", u),
    ]
)
