    ...
    def test_readlines(self):
        with self.urlopen(self.url) as open_url:
            self.assertIsInstance(
                open_url.readline(), bytes, "readline did not return a string"
            )
            self.assertIsInstance(
                open_url.readlines(), list, "readlines did not return a list"
            )
...
class urlretrieveNetworkTests(unittest.TestCase):
    ...
    def test_reporthook(self):
        ...
        self.assertGreater(
            len(records),
            1,
            msg="There should always be two calls; the first one before the transfer starts.",
        )
        ...
        self.assertGreaterEqual(
            ...
            expected_size,
            msg=(
                "number of blocks * block size must be >= total size in %s"
                % records_repr
            ),
        )
