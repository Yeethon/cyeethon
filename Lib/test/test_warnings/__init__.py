...
class PublicAPITests(BaseTest):
    ...
    def test_module_all_attribute(self):
        ...
        self.assertSetEqual(set(self.module.__all__), set(target_api))
...
class FilterTests(BaseTest):
    ...
    def test_error_after_default(self):
        with original_warnings.catch_warnings(module=self.module) as w:
            ...
            self.assertIn("UserWarning: FilterTests.test_ignore_after_default", stderr)
    ...
    def test_once(self):
        with original_warnings.catch_warnings(record=True, module=self.module) as w:
            ...
            self.module.warn_explicit(message, UserWarning, "__init__.py", 42)
            self.assertEqual(w[(-1)].message, message)
            del w[:]
            self.module.warn_explicit(message, UserWarning, "__init__.py", 13)
            self.assertEqual(len(w), 0)
    ...
    def test_module_globals(self):
        with original_warnings.catch_warnings(record=True, module=self.module) as w:
            ...
            self.module.warn_explicit(
                "msg", UserWarning, "filename", 42, module_globals=None
            )
            self.assertEqual(len(w), 1)
            with self.assertRaises(TypeError):
                self.module.warn_explicit(
                    "msg", UserWarning, "filename", 42, module_globals=True
                )
            self.assertEqual(len(w), 1)
            self.module.warn_explicit(
                "msg", UserWarning, "filename", 42, module_globals={}
            )
            self.assertEqual(len(w), 2)
    ...
    def test_filterwarnings_duplicate_filters(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            self.module.filterwarnings("error", category=UserWarning)
            self.assertEqual(
                len(self.module.filters), 2, "filterwarnings inserted duplicate filter"
            )
            self.assertEqual(
    ...
    def test_simplefilter_duplicate_filters(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            self.module.simplefilter("error", category=UserWarning)
            self.assertEqual(
                len(self.module.filters), 2, "simplefilter inserted duplicate filter"
            )
            self.assertEqual(
                self.module.filters[0][0],
                "error",
    ...
    def test_append_duplicate(self):
        with original_warnings.catch_warnings(module=self.module, record=True) as w:
            ...
            self.assertEqual(len(w), 0, "appended duplicate changed order of filters")
...
class WarnTests(BaseTest):
    ...
    def test_stacklevel(self):
        with warnings_state(self.module):
            with original_warnings.catch_warnings(record=True, module=self.module) as w:
                ...
                self.assertEqual(os.path.basename(w[(-1)].filename), "__init__.py")
    ...
    def test_exec_filename(self):
        ...
        codeobj = compile(
            "import warnings\nwarnings.warn('hello', UserWarning)", filename, "exec"
        )
    ...
    def test_warn_explicit_non_ascii_filename(self):
        with original_warnings.catch_warnings(record=True, module=self.module) as w:
            self.module.resetwarnings()
            self.module.filterwarnings("always", category=UserWarning)
            for filename in ("nonasciié€", "surrogate\udc80"):
                try:
    ...
    def test_warn_explicit_type_errors(self):
        self.assertRaises(
            TypeError, self.module.warn_explicit, None, UserWarning, None, None
        )
    ...
    def test_warning_classes(self):
        ...
        self.assertIn("category must be a Warning subclass, not ", str(cm.exception))
        with self.assertRaises(TypeError) as cm:
...
class WCmdLineTests(BaseTest):
    def test_improper_input(self):
        with original_warnings.catch_warnings(module=self.module):
            self.assertRaises(
                self.module._OptionError, self.module._setoption, "1:2:3:4:5:6"
            )
            self.assertRaises(
                self.module._OptionError, self.module._setoption, "bogus::Warning"
            )
            self.assertRaises(
                self.module._OptionError, self.module._setoption, "ignore:2::4:-5"
            )
...
class _WarningsTests(BaseTest, unittest.TestCase):
    ...
    def test_default_action(self):
        ...
        try:
            with original_warnings.catch_warnings(record=True, module=self.module) as w:
                ...
                registry = {}
                self.module.warn_explicit(
                    message, UserWarning, "<test>", 42, registry=registry
                )
                self.assertEqual(w[(-1)].message, message)
                self.assertEqual(len(w), 1)
    ...
    def test_filename_none(self):
        ...
        try:
            catch = original_warnings.catch_warnings(record=True, module=self.module)
            with catch as w:
                self.module.filterwarnings("always", category=UserWarning)
    ...
    def test_stderr_none(self):
        (rc, stdout, stderr) = assert_python_ok(
            "-c",
            "import sys; sys.stderr = None; import warnings; warnings.simplefilter('always'); warnings.warn('Warning!')",
        )
...
class WarningsDisplayTests(BaseTest):
    ...
    def test_formatwarning(self):
        ...
        format = "%s:%s: %s: %s\n  %s\n"
        expect = format % (file_name, line_num, category.__name__, message, file_line)
        self.assertEqual(
        ...
        self.assertEqual(
            expect,
            self.module.formatwarning(
                message, category, file_name, line_num, file_line
            ),
        )
    ...
    def test_showwarning(self):
        ...
        expect = self.module.formatwarning(
            message, category, file_name, line_num, expected_file_line
        )
        file_object = StringIO()
        self.module.showwarning(
            message, category, file_name, line_num, file_object, expected_file_line
        )
    ...
    def test_formatwarning_override(self):
        ...
        file_object = StringIO()
        expected = (
            f"m={message}:c={category}:f={file_name}:l={line_num}" + f":t={file_line}"
        )
        with support.swap_attr(self.module, "formatwarning", myformatwarning):
            self.module.showwarning(
                message, category, file_name, line_num, file_object, file_line
            )
...
class EnvironmentVariableTests(BaseTest):
    ...
    def test_comma_separated_warnings(self):
        ...
        self.assertEqual(
            stdout, b"['ignore::DeprecationWarning', 'ignore::UnicodeWarning']"
        )
    ...
    def test_conflicting_envvar_and_command_line(self):
        (rc, stdout, stderr) = assert_python_failure(
            ...
            "-c",
            "import sys, warnings; sys.stdout.write(str(sys.warnoptions)); warnings.warn('Message', DeprecationWarning)",
            PYTHONWARNINGS="default::DeprecationWarning",
            PYTHONDEVMODE="",
        )
        self.assertEqual(
            stdout, b"['default::DeprecationWarning', 'error::DeprecationWarning']"
        )
        self.assertEqual(
            stderr.splitlines(),
            [
