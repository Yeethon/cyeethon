...
class DateTimeTests(unittest.TestCase):
    ...
    def test_parsedate_to_datetime_with_invalid_raises_valueerror(self):
        invalid_dates = [
            "",
            "0",
            "A Complete Waste of TimeTue, 06 Jun 2017 27:39:33 +0600",
...
class LocaltimeTests(unittest.TestCase):
    ...
    @unittest.skipIf(
        sys.platform.startswith("win"), "Windows does not use Olson's TZ database"
    )
    @unittest.skipUnless(
        (os.path.exists("/usr/share/zoneinfo") or os.path.exists("/usr/lib/zoneinfo")),
