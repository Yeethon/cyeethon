...
class TestDecode(TestEmailBase):
    ...
    def test_b_padding_defect(self):
        self._test(
            "=?us-ascii?b?dmk?=", "vi", defects=[errors.InvalidBase64PaddingDefect]
        )
...
class TestEncode(TestEmailBase):
    ...
    def test_auto_b_if_long_unsafe(self):
        self.assertEqual(
            _ew.encode("vi.vi.vi.vi.vi.", "utf-8"), "=?utf-8?b?dmkudmkudmkudmkudmku?="
        )

    def test_auto_q_if_long_mostly_safe(self):
        self.assertEqual(
            _ew.encode("vi vi vi.vi ", "utf-8"), "=?utf-8?q?vi_vi_vi=2Evi_?="
        )
