...
class TestContentTypeHeader(TestHeaderBase):
    ...
    content_type_params = {
        ...
        "spaces_around_semis": (
            'image/jpeg; name="wibble.JPG" ; x-mac-type="4A504547" ; x-mac-creator="474B4F4E"',
            "image/jpeg",
            "image",
            "jpeg",
            {
        ...
        "lots_of_mime_params": (
            'image/jpeg; name="wibble.JPG"; x-mac-type="4A504547"; x-mac-creator="474B4F4E"; x-extrastuff="make it longer"',
            "image/jpeg",
            "image",
            "jpeg",
            {
        ...
        "non_ascii_in_params": (
            "foo§/bar; b§r=two; baz=thr§e".encode("latin-1").decode(
        ...
        "quotes_inside_rfc2231_value": (
            'image/jpeg; bar*0="baz\\"foobar"; bar*1="\\"baz"',
            "image/jpeg",
            "image",
            "jpeg",
            {"bar": 'baz"foobar"baz'},
            [],
            'image/jpeg; bar="baz\\"foobar\\"baz"',
        ),
        "non_ascii_rfc2231_value": (
            "text/plain; charset=us-ascii; title*=us-ascii'en'This%20is%20not%20f§n".encode(
                "latin-1"
            ).decode(
                "us-ascii", "surrogateescape"
            ),
        ...
        "rfc2231_single_quote_inside_double_quotes": (
            'text/plain; charset=us-ascii;\ttitle*0*="us-ascii\'en\'This%20is%20really%20";\ttitle*1*="%2A%2A%2Afun%2A%2A%2A%20";\ttitle*2="isn\'t it!"',
            "text/plain",
            "text",
            "plain",
            {"charset": "us-ascii", "title": "This is really ***fun*** isn't it!"},
            ([errors.InvalidHeaderDefect] * 2),
            'text/plain; charset="us-ascii"; title="This is really ***fun*** isn\'t it!"',
            'Content-Type: text/plain; charset="us-ascii";\n title="This is really ***fun*** isn\'t it!"\n',
        ),
        "rfc2231_single_quote_in_value_with_charset_and_lang": (
            'application/x-foo;\tname*0*="us-ascii\'en-us\'Frank\'s"; name*1*=" Document"',
            "application/x-foo",
            "application",
            "x-foo",
        ...
        "rfc2231_encoded_then_unencoded_segments": (
            'application/x-foo;\tname*0*="us-ascii\'en-us\'My";\tname*1=" Document";\tname*2=" For You"',
            "application/x-foo",
            "application",
            "x-foo",
...
class TestContentDisposition(TestHeaderBase):
    ...
    content_disp_params = {
        ...
        "invalid_parameter_value_with_fws_between_ew": (
            'attachment; filename="=?UTF-8?Q?Schulbesuchsbest=C3=A4ttigung=2E?=               =?UTF-8?Q?pdf?="',
            "attachment",
...
class TestMIMEVersionHeader(TestHeaderBase):
    ...
    version_string_params = {
        ...
        "2_x": ("2.x", "2.x", None, None, None, [errors.InvalidHeaderDefect]),
...
class TestAddressHeader(TestHeaderBase):
    ...
    def test_set_from_Address_and_Group_list(self):
        ...
        self.assertEqual(
            h,
            "me <foo@example.com>, bing: fiz <z@b.com>, zif <f@c.com>;, you <bar@example.com>",
        )
        self.assertEqual(
...
class TestFolding(TestHeaderBase):
    def test_address_display_names(self):
        ...
        for (name, result) in (
            ...
            (
                "Lôrem ipsum dôlôr sit amet, cônsectetuer adipiscing. Suspendisse pôtenti. Aliquam nibh. Suspendisse pôtenti.",
                "=?utf-8?q?L=C3=B4rem_ipsum_d=C3=B4l=C3=B4r_sit_amet=2C_c=C3=B4nsectetuer?=\n =?utf-8?q?adipiscing=2E_Suspendisse_p=C3=B4tenti=2E_Aliquam_nibh=2E?=\n Suspendisse =?utf-8?q?p=C3=B4tenti=2E?=",
            ),
        ):
    ...
    def test_long_unstructured(self):
        h = self.make_header(
            "Subject",
            "This is a long header line that will need to be folded into two lines and will demonstrate basic folding",
        )
        self.assertEqual(
            h.fold(policy=policy.default),
            "Subject: This is a long header line that will need to be folded into two lines\n and will demonstrate basic folding\n",
        )
    ...
    def test_fold_unstructured_with_two_overlong_words(self):
        h = self.make_header(
            "Subject",
            "thisisaverylonglineconsistingofasinglewordthatwontfit plusanotherverylongwordthatwontfit",
        )
        self.assertEqual(
            h.fold(policy=policy.default.clone(max_line_length=20)),
            "Subject: \n =?utf-8?q?thisisa?=\n =?utf-8?q?verylon?=\n =?utf-8?q?glineco?=\n =?utf-8?q?nsistin?=\n =?utf-8?q?gofasin?=\n =?utf-8?q?gleword?=\n =?utf-8?q?thatwon?=\n =?utf-8?q?tfit_pl?=\n =?utf-8?q?usanoth?=\n =?utf-8?q?erveryl?=\n =?utf-8?q?ongword?=\n =?utf-8?q?thatwon?=\n =?utf-8?q?tfit?=\n",
