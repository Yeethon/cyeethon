...
class TestParserMixin:
    ...
    def _test_get_x(
        self, method, source, string, value, defects, remainder, comments=None
    ):
...
class TestParser(TestParserMixin, TestEmailBase):
    ...
    def test__wsp_splitter_ws_runs(self):
        self.assertEqual(
            parser._wsp_splitter("foo \t def jik", 1), ["foo", " \t ", "def jik"]
        )
    ...
    def test_get_encoded_word_leading_internal_space(self):
        self._test_get_x(
            parser.get_encoded_word, "=?us-ascii?q?=20foo?=", " foo", " foo", [], ""
        )

    def test_get_encoded_word_quopri_utf_escape_follows_cte(self):
        self._test_get_x(
            parser.get_encoded_word, "=?utf-8?q?=C3=89ric?=", "Éric", "Éric", [], ""
        )
    ...
    def test_get_unstructured_normal_phrase(self):
        self._test_get_x(
            self._get_unst, "foo bar bird", "foo bar bird", "foo bar bird", [], ""
        )
    ...
    def test_get_unstructured_leading_and_trailing_whitespace(self):
        self._test_get_x(
            self._get_unst, "  foo bar  ", "  foo bar  ", " foo bar ", [], ""
        )
    ...
    def test_get_unstructured_one_ew_trailing_ws(self):
        self._test_get_x(
            self._get_unst, "=?us-ascii?q?bar?=  ", "bar  ", "bar ", [], ""
        )

    def test_get_unstructured_one_valid_ew_trailing_text(self):
        self._test_get_x(
            self._get_unst, "=?us-ascii?q?bar?= bird", "bar bird", "bar bird", [], ""
        )
    ...
    def test_get_unstructured_invalid_ew(self):
        self._test_get_x(
            self._get_unst, "=?test val", "=?test val", "=?test val", [], ""
        )
    ...
    def test_get_qcontent_only(self):
        ptext = self._test_get_x(
            parser.get_qcontent, "foobar", "foobar", "foobar", [], ""
        )
        self.assertEqual(ptext.token_type, "ptext")
    ...
    def test_get_bare_quoted_string_only(self):
        bqs = self._test_get_x(
            parser.get_bare_quoted_string, '"foo"', '"foo"', "foo", [], ""
        )
    ...
    def test_get_bare_quoted_string_following_wsp_preserved(self):
        self._test_get_x(
            parser.get_bare_quoted_string, '"foo"\t bar', '"foo"', "foo", [], "\t bar"
        )
    ...
    def test_get_comment_only(self):
        comment = self._test_get_x(
            parser.get_comment, "(comment)", "(comment)", " ", [], "", ["comment"]
        )
    ...
    def test_get_comment_end_paren_mid_word(self):
        self._test_get_x(
            parser.get_comment, "(foo)bar", "(foo)", " ", [], "bar", ["foo"]
        )
    ...
    def test_get_comment_nested_comment(self):
        comment = self._test_get_x(
            parser.get_comment, "(foo(bar))", "(foo(bar))", " ", [], "", ["foo(bar)"]
        )
    ...
    def test_get_cfws_ends_at_non_leader(self):
        cfws = self._test_get_x(
            parser.get_cfws, "(foo) bar", "(foo) ", " ", [], "bar", ["foo"]
        )
        self.assertEqual(cfws[0].content, "foo")

    def test_get_cfws_ends_at_non_printable(self):
        cfws = self._test_get_x(
            parser.get_cfws, "(foo) \x07", "(foo) ", " ", [], "\x07", ["foo"]
        )
        self.assertEqual(cfws[0].content, "foo")
    ...
    def test_get_quoted_string_with_wsp(self):
        qs = self._test_get_x(
            parser.get_quoted_string, '\t "bob"  ', '\t "bob"  ', " bob ", [], ""
        )
        self.assertEqual(qs.quoted_value, ' "bob" ')
        self.assertEqual(qs.content, "bob")
    ...
    def test_get_quoted_string_qs_ends_at_noncfws(self):
        qs = self._test_get_x(
            parser.get_quoted_string, '\t "bob" fee', '\t "bob" ', " bob ", [], "fee"
        )
        self.assertEqual(qs.content, "bob")
        self.assertEqual(qs.quoted_value, ' "bob" ')
    ...
    def test_get_atom_with_comments_and_wsp(self):
        atom = self._test_get_x(
            parser.get_atom, " (foo) bob(bar)", " (foo) bob(bar)", " bob ", [], ""
        )
    ...
    def test_get_dot_atom_only(self):
        dot_atom = self._test_get_x(
            parser.get_dot_atom, "foo.bar.bing", "foo.bar.bing", "foo.bar.bing", [], ""
        )
    ...
    def test_get_dot_atom_rfc2047_atom(self):
        self._test_get_x(
            parser.get_dot_atom, "=?utf-8?q?=20bob?=", " bob", " bob", [], ""
        )

    def test_get_word_atom_yields_atom(self):
        word = self._test_get_x(
            parser.get_word,
            " (foo) bar (bang) :ah",
            " (foo) bar (bang) ",
            " bar ",
    ...
    def test_get_word_qs_yields_qs(self):
        word = self._test_get_x(
            parser.get_word, '"bar " (bang) ah', '"bar " (bang) ', "bar  ", [], "ah"
        )
    ...
    def test_get_domain_literal_with_internal_ws(self):
        domain_literal = self._test_get_x(
            parser.get_domain_literal,
            "[  127.0.0.1\t ]",
            "[  127.0.0.1\t ]",
            "[ 127.0.0.1 ]",
    ...
    def test_get_domain_regular_domain_only(self):
        domain = self._test_get_x(
            parser.get_domain, "example.com", "example.com", "example.com", [], ""
        )
    ...
    def test_get_domain_domain_literal_only(self):
        domain = self._test_get_x(
            parser.get_domain, "[127.0.0.1]", "[127.0.0.1]", "[127.0.0.1]", [], ""
        )
        self.assertEqual(domain.token_type, "domain")
    ...
    def test_get_addr_spec_normal(self):
        addr_spec = self._test_get_x(
            parser.get_addr_spec,
    ...
    def test_get_angle_addr_empty(self):
        angle_addr = self._test_get_x(
            parser.get_angle_addr, "<>", "<>", "<>", [errors.InvalidHeaderDefect], ""
        )
        self.assertEqual(angle_addr.token_type, "angle-addr")
    ...
    def test_get_mailbox_list_single_addr(self):
        mailbox_list = self._test_get_x(
            parser.get_mailbox_list,
            "dinsdale@example.com",
            "dinsdale@example.com",
            "dinsdale@example.com",
            [],
    ...
    def test_get_mailbox_list_two_simple_addr(self):
        ...
        self.assertEqual(
            mailbox_list.mailboxes[1].addr_spec, "dinsdale@test.example.com"
        )
        self.assertEqual(mailbox_list.mailboxes, mailbox_list.all_mailboxes)
    ...
    def test_get_mailbox_list_two_name_addr(self):
        mailbox_list = self._test_get_x(
            parser.get_mailbox_list,
            '"Roy A. Bear" <dinsdale@example.com>, "Fred Flintstone" <dinsdale@test.example.com>',
            '"Roy A. Bear" <dinsdale@example.com>, "Fred Flintstone" <dinsdale@test.example.com>',
            '"Roy A. Bear" <dinsdale@example.com>, "Fred Flintstone" <dinsdale@test.example.com>',
            [],
            "",
    ...
    def test_get_mailbox_list_two_complex(self):
        mailbox_list = self._test_get_x(
            parser.get_mailbox_list,
            '(foo) "Roy A. Bear" <dinsdale@example.com>(bar), "Fred Flintstone" <dinsdale@test.(bird)example.com>',
            '(foo) "Roy A. Bear" <dinsdale@example.com>(bar), "Fred Flintstone" <dinsdale@test.(bird)example.com>',
            ' "Roy A. Bear" <dinsdale@example.com> , "Fred Flintstone" <dinsdale@test. example.com>',
            [errors.ObsoleteHeaderDefect],
            "",
    ...
    def test_get_mailbox_list_unparseable_mailbox_null(self):
        mailbox_list = self._test_get_x(
            parser.get_mailbox_list,
            '"Roy A. Bear"[] dinsdale@example.com, "Fred Flintstone" <dinsdale@test.(bird)example.com>',
            '"Roy A. Bear"[] dinsdale@example.com, "Fred Flintstone" <dinsdale@test.(bird)example.com>',
            '"Roy A. Bear"[] dinsdale@example.com, "Fred Flintstone" <dinsdale@test. example.com>',
            [
                errors.InvalidHeaderDefect,
        ...
        self.assertEqual(
            mailbox_list.mailboxes[0].addr_spec, "dinsdale@test.example.com"
        )
    ...
    def test_get_mailbox_list_junk_after_valid_address(self):
        ...
        self.assertEqual(len(mailbox_list.all_mailboxes), 2)
        self.assertEqual(
            mailbox_list.all_mailboxes[0].addr_spec, "dinsdale@example.com"
        )
    ...
    def test_get_mailbox_list_empty_list_element(self):
        mailbox_list = self._test_get_x(
            parser.get_mailbox_list,
            '"Roy A. Bear" <dinsdale@example.com>, (bird),, "Fred Flintstone" <dinsdale@test.example.com>',
            '"Roy A. Bear" <dinsdale@example.com>, (bird),, "Fred Flintstone" <dinsdale@test.example.com>',
            '"Roy A. Bear" <dinsdale@example.com>, ,, "Fred Flintstone" <dinsdale@test.example.com>',
            ([errors.ObsoleteHeaderDefect] * 2),
            "",
    ...
    def test_get_group_list_cfws_only(self):
        group_list = self._test_get_x(
            parser.get_group_list, "(hidden);", "(hidden)", " ", [], ";"
        )
    ...
    def test_get_group_mixed_list(self):
        group = self._test_get_x(
            parser.get_group,
            'Monty Python: "Fred A. Bear" <dinsdale@example.com>,(foo) Roger <ping@exampele.com>, x@test.example.com;',
            'Monty Python: "Fred A. Bear" <dinsdale@example.com>,(foo) Roger <ping@exampele.com>, x@test.example.com;',
            'Monty Python: "Fred A. Bear" <dinsdale@example.com>, Roger <ping@exampele.com>, x@test.example.com;',
            [],
            "",
        )
        self.assertEqual(group.token_type, "group")
    ...
    def test_get_group_missing_final_semicol(self):
        group = self._test_get_x(
            parser.get_group,
            'Monty Python:"Fred A. Bear" <dinsdale@example.com>,eric@where.test,John <jdoe@test>',
            'Monty Python:"Fred A. Bear" <dinsdale@example.com>,eric@where.test,John <jdoe@test>;',
            'Monty Python:"Fred A. Bear" <dinsdale@example.com>,eric@where.test,John <jdoe@test>;',
            [errors.InvalidHeaderDefect],
            "",
    ...
    def test_get_address_quoted_strings_in_atom_list(self):
        ...
        self.assertEqual(
            address.all_mailboxes[0].addr_spec, '"example example"@example.com'
        )

    def test_get_address_list_CFWS(self):
        address_list = self._test_get_x(
    ...
    def test_get_address_list_mailboxes_simple(self):
        ...
        self.assertEqual(address_list.mailboxes, address_list.all_mailboxes)
        self.assertEqual(
            [str(x) for x in address_list.mailboxes],
    ...
    def test_get_address_list_mailboxes_complex(self):
        address_list = self._test_get_x(
            parser.get_address_list,
            '"Roy A. Bear" <dinsdale@example.com>, (ping) Foo <x@example.com>,Nobody Is. Special <y@(bird)example.(bad)com>',
            '"Roy A. Bear" <dinsdale@example.com>, (ping) Foo <x@example.com>,Nobody Is. Special <y@(bird)example.(bad)com>',
            '"Roy A. Bear" <dinsdale@example.com>, Foo <x@example.com>,"Nobody Is. Special" <y@example. com>',
            [errors.ObsoleteHeaderDefect, errors.ObsoleteHeaderDefect],
            "",
        )
        self.assertEqual(address_list.token_type, "address-list")
    ...
    def test_get_address_list_mailboxes_invalid_addresses(self):
        address_list = self._test_get_x(
            parser.get_address_list,
            '"Roy A. Bear" <dinsdale@example.com>, (ping) Foo x@example.com[],Nobody Is. Special <(bird)example.(bad)com>',
            '"Roy A. Bear" <dinsdale@example.com>, (ping) Foo x@example.com[],Nobody Is. Special <(bird)example.(bad)com>',
            '"Roy A. Bear" <dinsdale@example.com>, Foo x@example.com[],"Nobody Is. Special" < example. com>',
            [
                errors.InvalidHeaderDefect,
                errors.InvalidHeaderDefect,
                errors.InvalidHeaderDefect,
        ...
        self.assertEqual(
            [str(x) for x in address_list.all_mailboxes],
            [str(x) for x in address_list.addresses],
        )
        self.assertEqual(address_list.mailboxes[0].domain, "example.com")
        self.assertEqual(address_list.mailboxes[0].token_type, "mailbox")
    ...
    def test_get_address_list_group_and_mailboxes(self):
        address_list = self._test_get_x(
            parser.get_address_list,
            'Monty Python: dinsdale@example.com, "Fred" <flint@example.com>;, Abe <x@example.com>, Bee <y@example.com>',
            'Monty Python: dinsdale@example.com, "Fred" <flint@example.com>;, Abe <x@example.com>, Bee <y@example.com>',
            'Monty Python: dinsdale@example.com, "Fred" <flint@example.com>;, Abe <x@example.com>, Bee <y@example.com>',
            [],
            "",
...
class Test_parse_mime_parameters(TestParserMixin, TestEmailBase):
    ...
    mime_parameters_params = {
        ...
        "duplicate_key_with_split_value": (
            "filename*0*=iso-8859-1''%32%30%31%2E; filename*1*=%74%69%66; filename=abc.gif",
            ' filename="201.tif"',
            "filename*0*=iso-8859-1''%32%30%31%2E; filename*1*=%74%69%66; filename=abc.gif",
            [("filename", "201.tif")],
            [errors.InvalidHeaderDefect],
        ...
        "duplicate_key_with_split_value_other_order": (
            "filename=abc.gif;  filename*0*=iso-8859-1''%32%30%31%2E; filename*1*=%74%69%66",
            ' filename="abc.gif"',
            "filename=abc.gif; filename*0*=iso-8859-1''%32%30%31%2E; filename*1*=%74%69%66",
            [("filename", "abc.gif")],
            [errors.InvalidHeaderDefect],
        ...
        "duplicate_in_split_value": (
            "filename*0*=iso-8859-1''%32%30%31%2E; filename*1*=%74%69%66; filename*1*=abc.gif",
        ...
        "duplicate_and_missing_split_value": (
            "filename*0*=iso-8859-1''%32%30%31%2E; filename*3*=%74%69%66; filename*3*=abc.gif",
            ' filename="201.tifabc.gif"',
            "filename*0*=iso-8859-1''%32%30%31%2E; filename*3*=%74%69%66; filename*3*=abc.gif",
            [("filename", "201.tifabc.gif")],
            ([errors.InvalidHeaderDefect] * 2),
        ...
        "duplicate_with_broken_split_value": (
            "filename=abc.gif;  filename*2*=iso-8859-1''%32%30%31%2E; filename*3*=%74%69%66",
...
class Test_parse_mime_version(TestParserMixin, TestEmailBase):
    ...
    mime_version_params = {
        ...
        "empty": ("", "", "", None, None, [errors.HeaderMissingRequiredValue]),
    }
...
class TestFolding(TestEmailBase):
    ...
    def test_simple_unstructured_folded(self):
        self._test(
            ...
            "This is also a test, but this time there are enough words (and even some\n symbols) to make it wrap; at least in theory.\n",
        )

    def test_unstructured_with_unicode_no_folds(self):
        self._test(
            parser.get_unstructured("hübsch kleiner beißt"),
    ...
    def test_one_ew_on_each_of_two_wrapped_lines(self):
        self._test(
            parser.get_unstructured(
                "Mein kleiner Kaktus ist sehr hübsch.  Es hat viele Stacheln und oft beißt mich."
            ),
            "Mein kleiner Kaktus ist sehr =?utf-8?q?h=C3=BCbsch=2E?=  Es hat viele Stacheln\n und oft =?utf-8?q?bei=C3=9Ft?= mich.\n",
        )
    ...
    def test_ews_combined_before_wrap(self):
        self._test(
            ...
            "Mein Kaktus ist =?utf-8?q?h=C3=BCbsch=2E__Es_bei=C3=9Ft?= mich.  And that's\n all I'm sayin.\n",
        )

    def test_simple_address(self):
        self._test(
            parser.get_address_list("abc <xyz@example.com>")[0],
            "abc <xyz@example.com>\n",
        )
    ...
    def test_address_list_folding_at_commas(self):
        self._test(
            parser.get_address_list(
                'abc <xyz@example.com>, "Fred Blunt" <sharp@example.com>, "J.P.Cool" <hot@example.com>, "K<>y" <key@example.com>, Firesale <cheap@example.com>, <end@example.com>'
            )[0],
    ...
    def test_split_at_whitespace_before_long_token(self):
        self._test(
            parser.get_unstructured(("xxx   " + ("y" * 77))),
            (("xxx  \n " + ("y" * 77)) + "\n"),
        )
    ...
    def test_overlong_encodeable_is_wrapped(self):
        ...
        self._test(
            parser.get_unstructured((first_token_with_whitespace + ("y" * 80))),
            (
                (
                    (
                        (
    ...
    def test_long_filename_attachment(self):
        self._test(
            parser.parse_content_disposition_header(
                'attachment; filename="TEST_TEST_TEST_TEST_TEST_TEST_TEST_TEST_TEST_TEST_TEST_TEST_TES.txt"'
            ),
