...
class InheritanceTests:
    ...
    def setUp(self):
        self.superclasses = [
            getattr(self.abc, class_name) for class_name in self.superclass_names
        ]
...
def make_abc_subclasses(base_class, name=None, inst=False, **kwargs):
    ...
    base = {kind: getattr(splitabc, name) for (kind, splitabc) in abc.items()}
    return {
        cls._KIND: (cls() if inst else cls)
...
(Frozen_MPFDefaultTests, Source_MPFDefaultTests) = test_util.test_both(
...
(Frozen_PEFDefaultTests, Source_PEFDefaultTests) = test_util.test_both(
...
(Frozen_LDefaultTests, SourceLDefaultTests) = test_util.test_both(LoaderDefaultsTests)
...
(Frozen_RLDefaultTests, Source_RLDefaultTests) = test_util.test_both(
    ResourceLoaderDefaultsTests
)
...
(Frozen_ILDefaultTests, Source_ILDefaultTests) = test_util.test_both(
...
(Frozen_ELDefaultTests, Source_ELDefaultsTests) = test_util.test_both(
    InspectLoaderDefaultsTests
)
...
(Frozen_RRDefaultTests, Source_RRDefaultsTests) = test_util.test_both(
    ResourceReaderDefaultsTests
)
...
(Frozen_MPFFindModuleTests, Source_MPFFindModuleTests) = test_util.test_both(
    MetaPathFinderFindModuleTests, abc=abc, util=util
)
...
(Frozen_PEFFindLoaderTests, Source_PEFFindLoaderTests) = test_util.test_both(
    PathEntryFinderFindLoaderTests, abc=abc, util=util, machinery=machinery
)
...
(Frozen_LoaderLoadModuleTests, Source_LoaderLoadModuleTests) = test_util.test_both(
    LoaderLoadModuleTests, abc=abc, util=util
)
...
(Frozen_ILSourceToCodeTests, Source_ILSourceToCodeTests) = test_util.test_both(
    InspectLoaderSourceToCodeTests, InspectLoaderSubclass=SPLIT_IL
)
...
(Frozen_ILGetCodeTests, Source_ILGetCodeTests) = test_util.test_both(
    InspectLoaderGetCodeTests, InspectLoaderSubclass=SPLIT_IL
)
...
(Frozen_ILLoadModuleTests, Source_ILLoadModuleTests) = test_util.test_both(
    InspectLoaderLoadModuleTests, InspectLoaderSubclass=SPLIT_IL, init=init, util=util
)
...
class ExecutionLoaderGetCodeTests:
    def mock_methods(self, *, get_source=False, get_filename=False):
        ...
        if get_source:
            source_mock_context = mock.patch.object(
                self.ExecutionLoaderSubclass, "get_source"
            )
        if get_filename:
            filename_mock_context = mock.patch.object(
                self.ExecutionLoaderSubclass, "get_filename"
            )
...
(Frozen_ELGetCodeTests, Source_ELGetCodeTests) = test_util.test_both(
    ExecutionLoaderGetCodeTests, ExecutionLoaderSubclass=SPLIT_EL
)


class SourceOnlyLoader:
    source = b"_ = '::'.join([__name__, __file__, __cached__, __package__, repr(__loader__)])"
...
class SourceLoader(SourceOnlyLoader):
    ...
    def __init__(self, path, magic=None):
        ...
        code_object = compile(self.source, self.path, "exec", dont_inherit=True)
...
(Frozen_SourceOnlyLoaderTests, Source_SourceOnlyLoaderTests) = test_util.test_both(
    SourceOnlyLoaderTests, util=util, loader_mock=SPLIT_SOL
)
...
(Frozen_SLBytecodeTests, SourceSLBytecodeTests) = test_util.test_both(
    SourceLoaderBytecodeTests, init=init, util=util, loader_mock=SPLIT_SL
)
...
) = test_util.test_both(SourceLoaderGetSourceTests, SourceOnlyLoaderMock=SPLIT_SOL)
