...
(Frozen_ImportModuleTests, Source_ImportModuleTests) = test_util.test_both(
    ImportModuleTests, init=init
)
        ...
        with test_util.uncache(name):
            with test_util.import_state(meta_path=[self.FakeMetaFinder]):
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", DeprecationWarning)
                    warnings.simplefilter("ignore", ImportWarning)
                    self.assertEqual((name, path), self.init.find_loader(name, path))
...
(Frozen_FindLoaderPEP451Tests, Source_FindLoaderPEP451Tests) = test_util.test_both(
    FindLoaderPEP451Tests, init=init
)
...
(Frozen_FindLoaderPEP302Tests, Source_FindLoaderPEP302Tests) = test_util.test_both(
    FindLoaderPEP302Tests, init=init
)
...
class ReloadTests:
    ...
    def test_module_replaced(self):
        ...
        mock = test_util.mock_spec("top_level", module_code={"top_level": code})
        with mock:
            with test_util.import_state(meta_path=[mock]):
                module = self.init.import_module("top_level")
...
(Frozen_ReloadTests, Source_ReloadTests) = test_util.test_both(
    ReloadTests, init=init, util=util
)
...
(Frozen_InvalidateCacheTests, Source_InvalidateCacheTests) = test_util.test_both(
    InvalidateCacheTests, init=init
)
...
class FrozenImportlibTests(unittest.TestCase):
    def test_no_frozen_importlib(self):
        ...
        self.assertNotEqual(source_init.__loader__.__class__.__name__, "FrozenImporter")
...
(Frozen_StartupTests, Source_StartupTests) = test_util.test_both(
    StartupTests, machinery=machinery
)
