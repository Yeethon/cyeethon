    ...
    def test_property_set_name_incorrect_args(self):
        ...
        for i in (0, 1, 3):
            with self.assertRaisesRegex(
                TypeError,
                f"^__set_name__\(\) takes 2 positional arguments but {i} were given$",
            ):
                p.__set_name__(*([0] * i))
...
class _PropertyUnreachableAttribute:
    ...
    def test_get_property(self):
        with self.assertRaisesRegex(
            AttributeError, self._format_exc_msg("unreadable attribute")
        ):
            self.obj.foo

    def test_set_property(self):
        with self.assertRaisesRegex(
            AttributeError, self._format_exc_msg("can't set attribute")
        ):
            self.obj.foo = None

    def test_del_property(self):
        with self.assertRaisesRegex(
            AttributeError, self._format_exc_msg("can't delete attribute")
        ):
            del self.obj.foo
