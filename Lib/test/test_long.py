...
class LongTest(unittest.TestCase):
    ...
    def slow_format(self, x, base):
        ...
        digits.reverse()
        digits = digits or [0]
        return ("-"[:sign] + {2: "0b", 8: "0o", 10: "", 16: "0x"}[base]) + "".join(
            ("0123456789abcdef"[i] for i in digits)
        )
    ...
    def check_float_conversion(self, n):
        ...
        try:
            ...
            expected = "overflow"
        msg = (
            "Error in conversion of integer {} to float.  Got {}, expected {}.".format(
                n, actual, expected
            )
        )
    ...
    def test_true_division(self):
        ...
        for underflow in [
            ...
            self.assertEqual(
                result, 0.0, ("expected underflow to 0 from %r" % underflow)
            )
    ...
    def check_truediv(self, a, b, skip_small=True):
        ...
        self.assertEqual(
            expected,
            got,
            "Incorrectly rounded division {}/{}: expected {}, got {}".format(
                a, b, expected, got
            ),
        )
    ...
    def test_correctly_rounded_true_division(self):
        ...
        for base in bases:
            for exp in range((base - 15), (base + 15)):
                self.check_truediv(
                    (75312 * (2 ** max(exp, 0))), (69187 * (2 ** max((-exp), 0)))
                )
                self.check_truediv(
                    (69187 * (2 ** max(exp, 0))), (75312 * (2 ** max((-exp), 0)))
                )
        for m in [
            1,
            2,
        ...
        for n in range(250):
            self.check_truediv(
                (((((2 ** DBL_MANT_DIG) + 1) * 12345) * (2 ** 200)) + (2 ** n)),
