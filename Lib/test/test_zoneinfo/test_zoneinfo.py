...
class TzPathUserMixin:
    ...
    def setUp(self):
        with contextlib.ExitStack() as stack:
            stack.enter_context(
                self.tzpath_context(
                    self.tzpath, block_tzdata=self.block_tzdata, lock=TZPATH_TEST_LOCK
                )
            )
...
class TZStrTest(ZoneInfoTestBase):
    ...
    def _populate_test_cases(cls):
        ...
        def _add():
            tzstr = "<+11>-11"
            cases[tzstr] = (
                (datetime(2020, 1, 1), ZoneOffset("+11", timedelta(hours=11)), NORMAL),
            )
...
class TzPathTest(TzPathUserMixin, ZoneInfoTestBase):
    ...
    def test_env_variable_relative_paths(self):
        test_cases = [
            [("path/to/somewhere",), ()],
            [("/usr/share/zoneinfo", "path/to/somewhere"), ("/usr/share/zoneinfo",)],
            [("../relative/path",), ()],
            [("/usr/share/zoneinfo", "../relative/path"), ("/usr/share/zoneinfo",)],
...
class ZoneInfoData:
    ...
    def _convert_to_v1(self, contents):
        ...
        file_size = (
            ((((timecnt * 5) + (typecnt * 6)) + charcnt) + (leapcnt * 8)) + isstdcnt
        ) + isutcnt
