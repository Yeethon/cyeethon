...
class ReTests(unittest.TestCase):
    ...
    def test_basic_re_sub(self):
        ...
        self.assertEqual(
            re.sub("\\d+", self.bump_num, "08.2 -2 23x99y"), "9.3 -3 24x100y"
        )
        self.assertEqual(
            re.sub("\\d+", self.bump_num, "08.2 -2 23x99y", 3), "9.3 -3 23x99y"
        )
        self.assertEqual(
            re.sub("\\d+", self.bump_num, "08.2 -2 23x99y", count=3), "9.3 -3 23x99y"
        )
    ...
    def test_sub_template_numeric_escape(self):
        ...
        self.checkTemplateError(
            "x", "\\400", "x", "octal escape value \\400 outside of range 0-0o377", 0
        )
        self.checkTemplateError(
            "x", "\\777", "x", "octal escape value \\777 outside of range 0-0o377", 0
        )
    ...
    def test_bug_114660(self):
        self.assertEqual(
            re.sub("(\\S)\\s+(\\S)", "\\1 \\2", "hello  there"), "hello there"
        )
    ...
    def test_symbolic_groups(self):
        ...
        self.checkPatternError(
            "(?P<a>)(?P<a>)", "redefinition of group name 'a' as group 2; was group 1"
        )
    ...
    def test_symbolic_refs(self):
        self.checkTemplateError(
            "(?P<a>x)", "\\g<a", "xx", "missing >, unterminated name", 3
        )
        self.checkTemplateError("(?P<a>x)", "\\g<", "xx", "missing group name", 3)
        self.checkTemplateError("(?P<a>x)", "\\g", "xx", "missing <", 2)
        self.checkTemplateError(
            "(?P<a>x)", "\\g<a a>", "xx", "bad character in group name 'a a'", 3
        )
        self.checkTemplateError("(?P<a>x)", "\\g<>", "xx", "missing group name", 3)
        self.checkTemplateError(
            "(?P<a>x)", "\\g<1a1>", "xx", "bad character in group name '1a1'", 3
        )
        self.checkTemplateError(
            "(?P<a>x)", "\\g<2>", "xx", "invalid group reference 2", 3
        )
    ...
    def test_re_fullmatch(self):
        ...
        self.assertEqual(
            re.compile("bc").fullmatch("abcd", pos=1, endpos=3).span(), (1, 3)
        )
        self.assertEqual(
            re.compile(".*?$").fullmatch("abcd", pos=1, endpos=3).span(), (1, 3)
        )
        self.assertEqual(
            re.compile(".*?").fullmatch("abcd", pos=1, endpos=3).span(), (1, 3)
        )
    ...
    def test_re_groupref_exists(self):
        ...
        self.checkPatternError(
            "()(?(1)a|b|c)", "conditional backref with more than two branches", 10
        )
    ...
    def test_special_escapes(self):
        ...
        self.assertEqual(
            re.search("\\b(b.)\\b", "abcd abc bcd bx", re.ASCII).group(1), "bx"
        )
        self.assertEqual(
            re.search("\\B(b.)\\B", "abc bcd bc abxd", re.ASCII).group(1), "bx"
        )
        ...
        self.assertEqual(
            re.search(b"\\b(b.)\\b", b"abcd abc bcd bx", re.LOCALE).group(1), b"bx"
        )
        self.assertEqual(
            re.search(b"\\B(b.)\\B", b"abc bcd bc abxd", re.LOCALE).group(1), b"bx"
        )
        ...
        self.assertEqual(
            re.search(b"\\d\\D\\w\\W\\s\\S", b"1aa! a").group(0), b"1aa! a"
        )
        self.assertEqual(
            re.search("\\d\\D\\w\\W\\s\\S", "1aa! a", re.ASCII).group(0), "1aa! a"
        )
        self.assertEqual(
            re.search(b"\\d\\D\\w\\W\\s\\S", b"1aa! a", re.LOCALE).group(0), b"1aa! a"
        )
    ...
    def test_case_helpers(self):
        ...
        for i in list(range(128, 4096)) + [66560, 66600]:
            ...
            self.assertEqual(
                _sre.unicode_iscased(i), ((c != c.lower()) or (c != c.upper()))
            )
    ...
    def test_sre_character_literals(self):
        ...
        self.checkPatternError(
            "\\567", "octal escape value \\567 outside of range 0-0o377", 0
        )
    ...
    def test_sre_character_class_literals(self):
        for i in [0, 8, 16, 32, 64, 127, 128, 255, 256, 65535, 65536, 1114111]:
            ...
            self.assertTrue(re.match(("[\\U%08xz]" % i), (chr(i) + "z")))
        self.checkPatternError(
            "[\\567]", "octal escape value \\567 outside of range 0-0o377", 1
        )
    ...
    def test_sre_byte_literals(self):
        ...
        self.checkPatternError(
            b"\\567", "octal escape value \\567 outside of range 0-0o377", 0
        )
    ...
    def test_sre_byte_class_literals(self):
        ...
        self.assertRaises(re.error, re.compile, b"[\\U00012345]")
        self.checkPatternError(
            b"[\\567]", "octal escape value \\567 outside of range 0-0o377", 1
        )
    ...
    def test_bug_418626(self):
        ...
        self.assertEqual(
            re.match(".*?cd", ((((5000 * "ab") + "c") + (5000 * "ab")) + "cde")).end(0),
            20003,
        )
    ...
    def test_nothing_to_repeat(self):
        for reps in ("*", "+", "?", "{1,2}"):
            for mod in ("", "?"):
                ...
                self.checkPatternError(
                    ("(?:%s%s)" % (reps, mod)), "nothing to repeat", 3
                )
    ...
    def test_scoped_flags(self):
        ...
        self.checkPatternError(
            "(?i-i:a)", "bad inline flags: flag turned on and off", 5
        )
        self.checkPatternError(
            "(?au:a)", "bad inline flags: flags 'a', 'u' and 'L' are incompatible", 4
        )
        self.checkPatternError(
            b"(?aL:a)", "bad inline flags: flags 'a', 'u' and 'L' are incompatible", 4
        )
    ...
    def test_repeat_minmax_overflow(self):
        ...
        self.assertRaises(OverflowError, re.compile, (".{%d,}?" % (2 ** 128)))
        self.assertRaises(
            OverflowError, re.compile, (".{%d,%d}" % ((2 ** 129), (2 ** 128)))
        )
    ...
    def test_repeat_minmax_overflow_maxrepeat(self):
        ...
        self.assertEqual(
            re.match((".{,%d}" % (MAXREPEAT - 1)), string).span(), (0, 100000)
        )
        self.assertIsNone(re.match((".{%d,}?" % (MAXREPEAT - 1)), string))
    ...
    def test_zerowidth(self):
        ...
        self.assertEqual(
            [m.span() for m in re.finditer("\\b|:+", "a::bc")],
            [(0, 0), (1, 1), (1, 3), (3, 3), (5, 5)],
        )
        self.assertEqual(
            [m.span() for m in re.finditer("\\b|\\w+", "a::bc")],
...
class PatternReprTests(unittest.TestCase):
    ...
    def test_unknown_flags(self):
        self.check_flags(
            "random pattern", 1191936, "re.compile('random pattern', 0x123000)"
        )
        self.check_flags(
            "random pattern",
            (1191936 | re.I),
