...
class ClassPropertiesAndMethods(unittest.TestCase):
        ...
        for attr in ("fget", "fset", "fdel"):
            try:
                ...
                if str(msg).find("readonly") < 0:
                    self.fail(
                        (
                            "when setting readonly attr %r on a property, got unexpected AttributeError msg %r"
                            % (attr, str(msg))
                        )
                    )
            else:
    ...
    def test_bound_method_repr(self):
        ...
        self.assertRegex(
            repr(Foo().method), "<bound method .*Foo\\.method of <.*Foo object at .*>>"
        )
        ...
        self.assertRegex(
            repr(base.method), "<bound method .*Base\\.method of <.*Base object at .*>>"
        )
        self.assertRegex(
            repr(derived1.method),
            "<bound method .*Base\\.method of <.*Derived1 object at .*>>",
        )
        self.assertRegex(
            repr(derived2.method),
            "<bound method .*Derived2\\.method of <.*Derived2 object at .*>>",
        )
        self.assertRegex(
            repr(super_d2.method),
            "<bound method .*Base\\.method of <.*Derived2 object at .*>>",
        )
        ...
        foo = Foo()
        self.assertRegex(
            repr(foo.method), "<bound method .*Foo\\.method of <class '.*Foo'>>"
        )
        self.assertRegex(
            repr(Foo.method), "<bound method .*Foo\\.method of <class '.*Foo'>>"
        )
        ...
        self.assertRegex(
            repr(method), "<bound method qualname of <object object at .*>>"
        )
