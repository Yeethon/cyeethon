...
class FractionTest(unittest.TestCase):
    ...
    def testFromString(self):
        ...
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '3 /2'", F, "3 /2"
        )
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '3/+2'", F, "3/+2"
        )
        ...
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '3a2'", F, "3a2"
        )
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '3/7.2'", F, "3/7.2"
        )
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '3.2/7'", F, "3.2/7"
        )
        self.assertRaisesMessage(
            ValueError, "Invalid literal for Fraction: '.'", F, "."
        )
    ...
    def testFromDecimal(self):
        ...
        self.assertEqual(
            (1 - F(1, (10 ** 30))), F.from_decimal(Decimal(("0." + ("9" * 30))))
        )
        self.assertRaisesMessage(
            OverflowError,
            "cannot convert Infinity to integer ratio",
    ...
    def testLargeArithmetic(self):
        ...
        self.assertTypedEquals(
            ((-(2 ** 200)) // 15), (F((-(2 ** 100)), 3) // F(5, (2 ** 100)))
        )
        self.assertTypedEquals(1, (F(5, (2 ** 100)) // F(3, (2 ** 100))))
        self.assertTypedEquals(
            (1, F(2, (2 ** 100))), divmod(F(5, (2 ** 100)), F(3, (2 ** 100)))
        )
        self.assertTypedTupleEquals(
    ...
    def testStringification(self):
        ...
        self.assertEqual(
            "Fraction(-1, 100000000000000000000)", repr(F(1, (-(10 ** 20))))
        )
