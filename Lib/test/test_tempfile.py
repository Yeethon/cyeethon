...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        warnings.filterwarnings(
            "ignore", category=RuntimeWarning, message="mktemp", module=__name__
        )
    ...
    def nameCheck(self, name, dir, pre, suf):
        ...
        if dir is not None:
            self.assertIs(
                ...
                (
                    str
                    if ((type(dir) is str) or isinstance(dir, os.PathLike))
                    else bytes
                ),
        ...
        if pre is not None:
            self.assertIs(
                type(name),
                (str if (type(pre) is str) else bytes),
                "unexpected return type",
            )
        if suf is not None:
            self.assertIs(
                type(name),
                (str if (type(suf) is str) else bytes),
                "unexpected return type",
            )
    ...
    def test_no_files_left_behind(self):
        with tempfile.TemporaryDirectory() as our_temp_directory:
            ...
            with support.swap_attr(
                tempfile, "_candidate_tempdir_list", our_candidate_list
            ):
...
class TestMkstempInner(TestBadTempdir, BaseTestCase):
    ...
    def test_noinherit(self):
        ...
        try:
            ...
            me = sys.argv[0]
        tester = os.path.join(
            os.path.dirname(os.path.abspath(me)), "tf_inherit_check.py"
        )
        if sys.platform == "win32":
        ...
        self.assertFalse(
            (retval < 0), ("child process caught fatal signal %d" % (-retval))
        )
...
class TestTemporaryDirectory(BaseTestCase):
    ...
    def do_create(
        ...
        tmp = tempfile.TemporaryDirectory(
            dir=dir, prefix=pre, suffix=suf, ignore_cleanup_errors=ignore_cleanup_errors
        )
    ...
    def test_explict_cleanup_ignore_errors(self):
        ...
        with tempfile.TemporaryDirectory() as working_dir:
            ...
            self.assertTrue(
                temp_path.exists(), f"TemporaryDirectory {temp_path!s} does not exist"
            )
            ...
            self.assertEqual(
                ...
                f"Unexpected number of files in TemporaryDirectory {temp_path!s}",
            )
            self.assertEqual(
                temp_path.exists(),
    ...
    def test_warnings_on_cleanup(self):
        with self.do_create() as dir:
            ...
            with warnings_helper.check_warnings(
                ("Implicitly", ResourceWarning), quiet=False
            ):
                warnings.filterwarnings("always", category=ResourceWarning)
                del d
                support.gc_collect()
