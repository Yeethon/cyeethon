...
class TestTimeZone(unittest.TestCase):
    ...
    def test_tzname(self):
        ...
        self.assertEqual("UTC-01:06:40", timezone((-timedelta(0, 4000))).tzname(None))
        self.assertEqual(
            "UTC+01:06:40.000001", timezone(timedelta(0, 4000, 1)).tzname(None)
        )
        self.assertEqual(
            "UTC-01:06:40.000001", timezone((-timedelta(0, 4000, 1))).tzname(None)
        )
...
class TestTimeDelta(HarmlessMixedComparison, unittest.TestCase):
    ...
    def test_carries(self):
        t1 = timedelta(
            ...
            minutes=(-3),
            seconds=12,
            microseconds=((((3 * 60) - 12) * 1000000.0) + 1),
        )
    ...
    def test_str(self):
        ...
        eq(
            str(td(weeks=(-30), hours=23, minutes=12, seconds=34)),
            "-210 days, 23:12:34",
        )
    ...
    def test_resolution_info(self):
        ...
        self.assertEqual(
            timedelta.max, timedelta(999999999, ((24 * 3600) - 1), (1000000.0 - 1))
        )
    ...
    def test_microsecond_rounding(self):
        ...
        eq(td(hours=((-0.2) / us_per_hour)), td(0))
        eq(
            td(days=((-0.4) / us_per_day), hours=((-0.2) / us_per_hour)),
            td(microseconds=(-1)),
        )
        eq(td(microseconds=0.5), (0.5 * td(microseconds=1.0)))
        eq(
            (td(microseconds=0.5) // td.resolution),
            ((0.5 * td.resolution) // td.resolution),
        )
    ...
    def test_subclass_timedelta(self):
        class T(timedelta):
            ...
            def as_hours(self):
                sum = ((self.days * 24) + (self.seconds / 3600.0)) + (
                    self.microseconds / 3600000000.0
                )
                return round(sum)
...
class TestDate(HarmlessMixedComparison, unittest.TestCase):
    ...
    def test_timetuple(self):
        for i in range(7):
            ...
            t = d.timetuple()
            self.assertEqual(
                t, (1956, 2, (1 + i), 0, 0, 0, ((2 + i) % 7), (32 + i), (-1))
            )
            d = self.theclass(1956, 3, (1 + i))
            t = d.timetuple()
            self.assertEqual(
                t, (1956, 3, (1 + i), 0, 0, 0, ((3 + i) % 7), (61 + i), (-1))
            )
    ...
    def test_backdoor_resistance(self):
        ...
        for month_byte in (b"9", b"\x00", b"\r", b"\xff"):
            self.assertRaises(
                TypeError, self.theclass, ((base[:2] + month_byte) + base[3:])
            )
        if issubclass(self.theclass, datetime):
...
class TestDateTime(TestDate):
    ...
    def test_timestamp_naive(self):
        ...
        self.assertEqual(
            t.timestamp(), ((((18000.0 + 3600) + (2 * 60)) + 3) + (4 * 1e-06))
        )
    ...
    def test_timestamp_aware(self):
        ...
        self.assertEqual(
            t.timestamp(), ((((18000 + 3600) + (2 * 60)) + 3) + (4 * 1e-06))
        )
    ...
    @unittest.skipIf(
        (sys.platform == "win32"), "Windows doesn't accept negative timestamps"
    )
    ...
    def test_more_timetuple(self):
        ...
        self.assertEqual(
            tt.tm_yday, ((t.toordinal() - date(t.year, 1, 1).toordinal()) + 1)
        )
...
class TestTimeTZ(TestTime, TZInfoBase, unittest.TestCase):
    ...
    def test_zones(self):
        ...
        self.assertEqual(
            t1.strftime("%H:%M:%S %%Z=%Z %%z=%z"), "07:47:00 %Z=EST %z=-0500"
        )
        ...
        self.assertEqual(
            t1.strftime("%H:%M %%Z='%Z' %%z='%z'"), "23:59 %Z='%z %Z %%z%%Z' %z='-2359'"
        )
    ...
    def test_compat_unpickle(self):
        tests = [
            ...
            b"cdatetime\ntime\n(U\x06\x05\x06\x07\x01\xe2@ctest.datetimetester\nPicklableFixedOffset\n)R}(U\x14_FixedOffset__offsetcdatetime\ntimedelta\n(J\xff\xff\xff\xffJ0\x0b\x01\x00K\x00tRU\x17_FixedOffset__dstoffsetNU\x12_FixedOffset__nameU\x06cookieubtR.",
...
class TestDateTimeTZ(TestDateTime, TZInfoBase, unittest.TestCase):
    ...
    def test_even_more_compare(self):
        ...
        t2 = self.theclass(
            MAXYEAR, 12, 31, 23, 59, 59, 999999, tzinfo=FixedOffset((-1439), "")
        )
        self.assertTrue((t1 < t2))
        self.assertTrue((t1 != t2))
        self.assertTrue((t2 > t1))
        ...
        t2 = self.theclass(
            2, 1, 1, 3, 13, tzinfo=FixedOffset((((3 * 60) + 13) + 2), "")
        )
        self.assertEqual(t1, t2)
    ...
    def test_tz_aware_arithmetic(self):
        ...
        max = self.theclass(
            MAXYEAR, 12, 31, 23, 59, 59, 999999, tzinfo=FixedOffset((-1439), "max")
        )
        maxdiff = max - min
        self.assertEqual(
            maxdiff,
            ((self.theclass.max - self.theclass.min) + timedelta(minutes=(2 * 1439))),
        )
        tza = timezone(HOUR, "A")
        tzb = timezone(HOUR, "B")
    ...
    def test_utctimetuple(self):
        ...
        for dstvalue in ((-33), 33, 0, None):
            ...
            self.assertEqual(
                ((d.toordinal() - date(1, 1, 1).toordinal()) + 1), t.tm_yday
            )
...
class Oddballs(unittest.TestCase):
    def test_bug_1028306(self):
        ...
        datetime_sc = SubclassDatetime(
            as_datetime.year, as_datetime.month, as_date.day, 0, 0, 0
        )
...
class Europe_Vilnius_1941(tzinfo):
    def _utc_fold(self):
        return [
            datetime(1941, 6, 23, 21, tzinfo=self),
            datetime(1941, 6, 23, 22, tzinfo=self),
        ]

    def _loc_fold(self):
        return [
            datetime(1941, 6, 23, 23, tzinfo=self),
            datetime(1941, 6, 24, 0, tzinfo=self),
        ]
...
class ZoneInfo(tzinfo):
    ...
    def fromutc(self, dt):
        ...
        if dt.tzinfo is not self:
            raise ValueError("dt.tzinfo is not self")
        timestamp = (
            (((dt.toordinal() - self.EPOCHORDINAL) * 86400) + (dt.hour * 3600))
            + (dt.minute * 60)
        ) + dt.second
...
class ZoneInfoTest(unittest.TestCase):
    ...
    def assertEquivDatetimes(self, a, b):
        self.assertEqual(
            (a.replace(tzinfo=None), a.fold, id(a.tzinfo)),
    ...
    def test_folds(self):
        ...
        for (dt, shift) in tz.folds():
            for x in [(0 * shift), (0.5 * shift), (shift - timedelta.resolution)]:
                ...
                self.assertEquivDatetimes(
                    ldt.astimezone(timezone.utc), udt.replace(tzinfo=timezone.utc)
                )
    ...
    def test_gaps(self):
        ...
        for (dt, shift) in tz.gaps():
            for x in [(0 * shift), (0.5 * shift), (shift - timedelta.resolution)]:
                ...
                utcoffset = ldt.utcoffset()
                self.assertEqual(
                    ldt.replace(tzinfo=None), (udt.replace(tzinfo=None) + utcoffset)
                )
    ...
    def test_PyDateTime_DATE_GET(self):
        ...
        for klass in [datetime, DateTimeSubclass]:
            for args in [
                ...
                (1993, 8, 26, 22, 12, 55, 99999, timezone.utc),
            ]:
                d = klass(*args)
                with self.subTest(cls=klass, date=args):
                    (
        ...
        for macro in (False, True):
            with self.subTest(macro=macro):
                c_api_date = _testcapi.get_date_fromdate(
                    macro, exp_date.year, exp_date.month, exp_date.day
                )
        ...
        for macro in (False, True):
            with self.subTest(macro=macro):
                c_api_delta = _testcapi.get_delta_fromdsu(
                    macro, exp_delta.days, exp_delta.seconds, exp_delta.microseconds
                )
