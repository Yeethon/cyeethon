...
USING_PTHREAD_COND = (sys.thread_info.name == "pthread") and (
    sys.thread_info.lock == "mutex+cond"
)
...
class ThreadSignals(unittest.TestCase):
    def test_signals(self):
        with threading_helper.wait_threads_exit():
            ...
            signalled_all.acquire()
        if (signal_blackboard[signal.SIGUSR1]["tripped"] == 0) or (
            signal_blackboard[signal.SIGUSR2]["tripped"] == 0
        ):
            try:
                signal.alarm(1)
                signal.pause()
