...
class ElementTreeTest(unittest.TestCase):
    ...
    def test_iterparse(self):
        ...
        self.assertEqual(
            ...
            [("end", "element"), ("end", "empty-element"), ("end", "root")],
        )
        ...
        self.assertEqual(
            ...
            [
                ("start-ns", ("", "http://éffbot.org/ns")),
                ("start-ns", ("clé", "http://effbot.org/ns")),
            ],
        )
...
<body><tag /></body>""",
    ...
    def test_doctype_public(self):
        elem = ET.XML(
            '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html>text</html>'
        )
    ...
    def test_processinginstruction(self):
        ...
        self.assertEqual(
            ET.tostring(ET.PI("test", "<testing&>ã"), "latin-1"),
...
class XMLPullParserTest(unittest.TestCase):
    ...
    def test_simple_xml_with_ns(self):
        ...
        self._feed(parser, "<empty-element/>\n")
        self.assert_event_tags(
            parser, [("end", "{namespace}element"), ("end", "{namespace}empty-element")]
        )
        self._feed(parser, "</root>\n")
    ...
    def test_events(self):
        ...
        self._feed(parser, "<element xmlns='foo'>text<empty-element/></element>tail\n")
        self.assert_event_tags(
            parser, [("start", "{foo}element"), ("start", "{foo}empty-element")]
        )
        self._feed(parser, "</root>")
        self.assertIsNone(parser.close())
...
class XIncludeTest(unittest.TestCase):
    ...
    def test_xinclude_failures(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "xi:fallback tag must be child of xi:include ('{http://www.w3.org/2001/XInclude}fallback')",
        )
...
class ElementFindTest(unittest.TestCase):
    ...
    def test_findall(self):
        ...
        self.assertEqual(
            summarize_list(e.findall('.//tag[@class!="a"]')), (["tag"] * 2)
        )
    ...
    def test_find_through_ElementTree(self):
        ...
        self.assertEqual(
            summarize_list(ET.ElementTree(e).findall("tag")), (["tag"] * 2)
        )
...
class C14NTest(unittest.TestCase):
    ...
    def test_simple_roundtrip(self):
        ...
        self.assertEqual(
            c14n_roundtrip("<doc xmlns='uri'/>"), '<doc xmlns="uri"></doc>'
        )
        self.assertEqual(
    ...
    def test_c14n_exclusion(self):
        ...
        self.assertEqual(
            ...
            '<root><a xmlns:x="http://example.com/x" x:attr="attrx"><b>abtext</b></a><b>btext</b><c><x:d xmlns:x="http://example.com/x">dtext</x:d></c></root>',
        )
        self.assertEqual(
            c14n_roundtrip(
                xml, strip_text=True, exclude_attrs=["{http://example.com/x}attr"]
            ),
            '<root><a><b>abtext</b></a><b>btext</b><c><x:d xmlns:x="http://example.com/x">dtext</x:d></c></root>',
        )
        self.assertEqual(
            c14n_roundtrip(
                xml, strip_text=True, exclude_tags=["{http://example.com/x}d"]
            ),
            '<root><a xmlns:x="http://example.com/x" x:attr="attrx"><b>abtext</b></a><b>btext</b><c></c></root>',
        )
        self.assertEqual(
            c14n_roundtrip(
                xml,
    ...
    def test_xml_c14n2(self):
        ...
        tests = {
            input_file: [
                ...
                for filename in files
                if (
                    filename.startswith(f"out_{input_file}_")
        ...
        for (input_file, output_files) in tests.items():
            for (output_file, config) in output_files:
                ...
                with self.subTest(f"{output_file}({config_descr})"):
                    ...
                    if ("QNameAware" in config) and (
                        config["QNameAware"][1].find(
