...
def verify_structure(memlen, itemsize, ndim, shape, strides, offset):
    ...
    if 0 in shape:
        return True
    imin = sum(
        ((strides[j] * (shape[j] - 1)) for j in range(ndim) if (strides[j] <= 0))
    )
...
def rand_structure(itemsize, valid, maxdim=5, maxshape=16, shape=()):
    ...
    for i in range((ndim - 2), (-1), (-1)):
        ...
        if zero_stride:
            ...
            strides[i] = ((1, (-1))[randrange(2)] * itemsize) * randrange(
                1, (maxstride + 1)
            )
    imin = imax = 0
    if not (0 in shape):
...
class TestBufferProtocol(unittest.TestCase):
        ...
        if not cast:
            ...
            if not buf_err:
                ...
                for order in ["C", "F", "A"]:
                    ...
                    elif (not is_contiguous(result, "A")) or (
                    ...
                    if len(initlst[0]) == 1:
                        initlst = [v[0] for v in initlst]
                    y = ndarray(
                        initlst, shape=shape, flags=(ro | ND_FORTRAN), format=fmt
                    )
                    self.assertEqual(memoryview(y), memoryview(result))
    ...
    def verify_getbuf(self, orig_ex, ex, req, sliced=False):
        ...
        if (
            ...
            or (match(req, PyBUF_C_CONTIGUOUS) and (not ex.c_contiguous))
            or (match(req, PyBUF_F_CONTIGUOUS) and (not ex.f_contiguous))
            or (match(req, PyBUF_ANY_CONTIGUOUS) and (not ex.contiguous))
            or ((not match(req, PyBUF_INDIRECT)) and ex.suboffsets)
            or ((not match(req, PyBUF_STRIDES)) and (not ex.c_contiguous))
            or ((not match(req, PyBUF_ND)) and match(req, PyBUF_FORMAT))
        ):
    ...
    def test_memoryview_cast_invalid(self):
        ...
        ex = ndarray(
            list([9 for _ in range((((3 * 5) * 7) * 11))]), shape=[3, 5, 7, 11]
        )
        m = memoryview(ex)
    ...
    def test_memoryview_cast(self):
        bytespec = (
            ...
            (
                ...
                (
                    lambda ex: [
                        ((x - 256) if (x > 127) else x) for x in list(ex.tobytes())
                    ]
                ),
            ),
    ...
    def test_memoryview_compare_multidim_mixed(self):
        ...
        self.assertNotEqual(v, w)
        ex1 = ndarray(
            ([(((2 ** 31) - 1), (-(2 ** 31)))] * 22), shape=[11, 2], format="=ii"
        )
        nd1 = ex1[3:1:(-1), ::(-2)]
        ex2 = ndarray(
            ([(((2 ** 31) - 1), (-(2 ** 31)))] * 22), shape=[11, 2], format=">ii"
    ...
    def test_memoryview_compare_not_equal(self):
        for byteorder in ["=", "<", ">", "!"]:
            x = ndarray(
                ([(2 ** 63)] * 120), shape=[3, 5, 2, 2, 2], format=(byteorder + "Q")
            )
            y = ndarray(
                ([(2 ** 63)] * 120),
                shape=[3, 5, 2, 2, 2],
