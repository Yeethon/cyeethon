...
class TestOptionalsDoubleDashPartialMatch(ParserTestCase):
    ...
    argument_signatures = [Sig("--badger", action="store_true"), Sig("--bat")]
...
class TestOptionalsDoubleDashPrefixMatch(ParserTestCase):
    ...
    argument_signatures = [Sig("--badger", action="store_true"), Sig("--ba")]
...
class TestOptionalsShortLong(ParserTestCase):
    ...
    argument_signatures = [Sig("-v", "--verbose", "-n", "--noisy", action="store_true")]
...
class TestOptionalsNargsZeroOrMore(ParserTestCase):
    ...
    argument_signatures = [Sig("-x", nargs="*"), Sig("-y", nargs="*", default="spam")]
    failures = ["a"]
    successes = [
        ("", NS(x=None, y="spam")),
        ("-x", NS(x=[], y="spam")),
...
class TestOptionalsRequired(ParserTestCase):
    ...
    argument_signatures = [Sig("-x", type=int, required=True)]
    failures = ["a", ""]
...
class TestOptionalsActionStoreConst(ParserTestCase):
    ...
    failures = ["a"]
    successes = [("", NS(y=None)), ("-y", NS(y=object))]
...
class TestBooleanOptionalActionRequired(ParserTestCase):
    ...
    failures = [""]
    successes = [("--foo", NS(foo=True)), ("--no-foo", NS(foo=False))]
...
class TestDisallowLongAbbreviationAllowsShortGrouping(ParserTestCase):
    "Do not allow abbreviations of long options at all"
    parser_signature = Sig(allow_abbrev=False)
    argument_signatures = [Sig("-r"), Sig("-c", action="count")]
    failures = ["-r", "-c -r"]
    successes = [
        ("", NS(r=None, c=None)),
        ("-ra", NS(r="a", c=None)),
...
class TestPositionalsNargsOptionalConvertedDefault(ParserTestCase):
    ...
    argument_signatures = [Sig("foo", nargs="?", type=int, default="42")]
    failures = ["-x", "a b", "1 2"]
...
class TestPositionalsNargs2None(ParserTestCase):
    ...
    successes = [("a b c", NS(foo=["a", "b"], bar="c"))]
...
class TestPositionalsNargsNoneOptional(ParserTestCase):
    ...
    argument_signatures = [Sig("foo"), Sig("bar", nargs="?")]
    failures = ["", "--foo", "a b c"]
    successes = [("a", NS(foo="a", bar=None)), ("a b", NS(foo="a", bar="b"))]
...
class TestPositionalsNargsOptionalNone(ParserTestCase):
    ...
    failures = ["", "--foo", "a b c"]
    successes = [("a", NS(foo=42, bar="a")), ("a b", NS(foo="a", bar="b"))]
...
class TestPositionalsNargsOptionalOptional(ParserTestCase):
    ...
    argument_signatures = [Sig("foo", nargs="?"), Sig("bar", nargs="?", default=42)]
    failures = ["--foo", "a b c"]
    successes = [
        ("", NS(foo=None, bar=42)),
        ("a", NS(foo="a", bar=42)),
...
class TestEmptyAndSpaceContainingArguments(ParserTestCase):
    argument_signatures = [Sig("x", nargs="?"), Sig("-y", "--yyy", dest="y")]
    failures = ["-y"]
    successes = [
        ([""], NS(x="", y=None)),
...
class TestArgumentsFromFile(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [Sig("-a"), Sig("x"), Sig("y", nargs="+")]
...
class TestArgumentsFromFileConverter(TempDirMixin, ParserTestCase):
    ...
    parser_signature = Sig(fromfile_prefix_chars="@")
    argument_signatures = [Sig("y", nargs="+")]
    failures = []
...
class TestTypeCallable(ParserTestCase):
    ...
    argument_signatures = [Sig("--eggs", type=complex), Sig("spam", type=float)]
...
class TestTypeUserDefined(ParserTestCase):
    ...
    argument_signatures = [Sig("-x", type=MyType), Sig("spam", type=MyType)]
    failures = []
    successes = [
...
class TestTypeClassicClass(ParserTestCase):
    ...
    argument_signatures = [Sig("-x", type=C), Sig("spam", type=C)]
    failures = []
    successes = [
...
class TestActionExtend(ParserTestCase):
    argument_signatures = [Sig("--foo", action="extend", nargs="+", type=str)]
    failures = ()
    successes = [("--foo f1 --foo f2 f3 f4", NS(foo=["f1", "f2", "f3", "f4"]))]
...
class TestAddSubparsers(TestCase):
    ...
    def test_help_non_breaking_spaces(self):
        ...
        parser.add_argument(
            "--non-breaking",
            action="store_false",
            help="help message containing non-breaking spaces shall not wrap\xa0at non-breaking spaces",
        )
        self.assertEqual(
            parser.format_help(),
            textwrap.dedent(
...
class TestMutuallyExclusiveManySuppressed(MEMixin, TestCase):
    ...
    successes_when_not_required = [("", NS(spam=False, badger=True, bladder=None))]
    usage_when_required = usage_when_not_required = "        usage: PROG [-h]\n        "
    help = "\n        options:\n          -h, --help  show this help message and exit\n        "
...
class TestMutuallyExclusiveOptionalAndPositional(MEMixin, TestCase):
    ...
    successes_when_not_required = [("", NS(foo=False, spam=None, badger="X"))]
    usage_when_not_required = (
...
class TestMutuallyExclusiveNested(MEMixin, TestCase):
    ...
    usage_when_not_required = "        usage: PROG [-h] [-a A | -b B | [-c C | -d D | [-e E | -f F]]]\n        "
    usage_when_required = "        usage: PROG [-h] (-a A | -b B | (-c C | -d D | (-e E | -f F)))\n        "
    help = "\n        options:\n          -h, --help  show this help message and exit\n          -a A\n          -b B\n          -c C\n          -d D\n          -e E\n          -f F\n        "
...
class TestHelpUsage(HelpTestCase):
    ...
    usage = "        usage: PROG [-h] [-w W [W ...]] [-x [X ...]] [--foo | --no-foo]\n                    [--bar | --no-bar]\n                    [-f | --foobar | --no-foobar | --barfoo | --no-barfoo] [-y [Y]]\n                    [-z Z Z Z]\n                    a b b [c] [d ...] e [e ...]\n        "
    help = (
        usage
...
class TestHelpOnlyUserGroups(HelpTestCase):
    ...
    argument_signatures = []
    argument_group_signatures = [
        (Sig("xxxx"), [Sig("-x", help="x"), Sig("a", help="a")]),
        (Sig("yyyy"), [Sig("b", help="b"), Sig("-y", help="y")]),
    ]
...
class TestHelpUsageLongProgPositionalsWrap(HelpTestCase):
    ...
    argument_signatures = [Sig(("a" * 25)), Sig(("b" * 25)), Sig(("c" * 25))]
    argument_group_signatures = []
...
class TestHelpSuppressUsage(HelpTestCase):
    ...
    argument_signatures = [Sig("--foo", help="foo help"), Sig("spam", help="spam help")]
    argument_group_signatures = []
...
class TestHelpSuppressOptionalGroup(HelpTestCase):
    ...
    parser_signature = Sig(prog="PROG")
    argument_signatures = [Sig("--foo", help="foo help"), Sig("spam", help="spam help")]
    argument_group_signatures = [(Sig("group"), [Sig("--bar", help=argparse.SUPPRESS)])]
...
class TestHelpRequiredOptional(HelpTestCase):
    ...
    parser_signature = Sig(prog="PROG")
    argument_signatures = [Sig("--foo", required=True, help="foo help")]
    argument_group_signatures = []
    usage = "        usage: PROG [-h] --foo FOO\n        "
    help = (
        usage
...
class TestHelpNone(HelpTestCase):
    ...
    parser_signature = Sig(prog="PROG")
    argument_signatures = [Sig("--foo"), Sig("spam")]
    argument_group_signatures = []
    usage = "        usage: PROG [-h] [--foo FOO] spam\n        "
    help = (
        usage
...
class TestStrings(TestCase):
    ...
    def test_argument(self):
        ...
        string = (
            "Action(option_strings=[], dest='x', nargs='?', const=None, default=2.5, type=%r, choices=[0.5, 1.5, 2.5], help='H HH H', metavar='MV MV MV')"
            % float
        )
    ...
    def test_parser(self):
        ...
        string = (
            "ArgumentParser(prog='PROG', usage=None, description=None, formatter_class=%r, conflict_handler='error', add_help=True)"
...
class TestInvalidNargs(TestCase):
    ...
    EXPECTED_RANGE_MESSAGE = "nargs for store actions must be != 0; if you have nothing to store, actions such as store true or store const may be more appropriate"
