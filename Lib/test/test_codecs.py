...
class ReadTest(MixInCheckStateHandling):
    ...
    def test_readline(self):
        ...
        for size in range(80):
            for lineend in lineends:
                ...
                for i in range(10):
                    self.assertEqual(
                        reader.readline(keepends=True), ((size * "a") + lineend)
                    )
    ...
    def test_lone_surrogates(self):
        ...
        self.assertEqual(
            "[\udc80]".encode(self.encoding, "backslashreplace"),
        ...
        self.assertEqual(
            "[\udc80]".encode(self.encoding, "ignore"), "[]".encode(self.encoding)
        )
        self.assertEqual(
            "[\udc80]".encode(self.encoding, "replace"), "[?]".encode(self.encoding)
        )
        self.assertEqual(
            "[\ud800\udc80]".encode(self.encoding, "ignore"), "[]".encode(self.encoding)
        )
        self.assertEqual(
            "[\ud800\udc80]".encode(self.encoding, "replace"),
            "[??]".encode(self.encoding),
        )
        ...
        for (before, after) in [("\U00010fff", "A"), ("[", "]"), ("A", "\U00010fff")]:
            ...
            test_sequence = (
                (bom + before_sequence) + self.ill_formed_sequence
            ) + after_sequence
            self.assertRaises(UnicodeDecodeError, test_sequence.decode, self.encoding)
            self.assertEqual(
                test_string.encode(self.encoding, "surrogatepass"), test_sequence
            )
            self.assertEqual(
                test_sequence.decode(self.encoding, "surrogatepass"), test_string
            )
            self.assertEqual(
                test_sequence.decode(self.encoding, "ignore"), (before + after)
            )
            self.assertEqual(
                test_sequence.decode(self.encoding, "replace"),
                ((before + self.ill_formed_sequence_replace) + after),
            )
            backslashreplace = "".join(
                (("\\x%02x" % b) for b in self.ill_formed_sequence)
            )
            self.assertEqual(
...
class UTF32Test(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        self.assertRaises(
            UnicodeDecodeError, codecs.utf_32_decode, b"\xff", "strict", True
        )
...
class UTF32LETest(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        self.assertRaises(
            UnicodeDecodeError, codecs.utf_32_le_decode, b"\xff", "strict", True
        )
...
class UTF32BETest(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        self.assertRaises(
            UnicodeDecodeError, codecs.utf_32_be_decode, b"\xff", "strict", True
        )
...
class UTF16Test(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        self.assertRaises(
            UnicodeDecodeError, codecs.utf_16_decode, b"\xff", "strict", True
        )
...
class UTF16LETest(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        ...
        for (raw, expected) in tests:
            self.assertRaises(
                UnicodeDecodeError, codecs.utf_16_le_decode, raw, "strict", True
            )
...
class UTF16BETest(ReadTest, unittest.TestCase):
    ...
    def test_errors(self):
        ...
        for (raw, expected) in tests:
            self.assertRaises(
                UnicodeDecodeError, codecs.utf_16_be_decode, raw, "strict", True
            )
...
class UTF8Test(ReadTest, unittest.TestCase):
    ...
    def test_decode_error(self):
        for (data, error_handler, expected) in (
            ...
            with self.subTest(
                data=data, error_handler=error_handler, expected=expected
            ):
    ...
    def test_lone_surrogates(self):
        ...
        self.assertEqual(
            "[\udc80]".encode(self.encoding, "surrogateescape"), (self.BOM + b"[\x80]")
        )
    ...
    def test_surrogatepass_handler(self):
        self.assertEqual(
            "abc\ud800def".encode(self.encoding, "surrogatepass"),
            (self.BOM + b"abc\xed\xa0\x80def"),
        )
        self.assertEqual(
            "\U00010fff\ud800".encode(self.encoding, "surrogatepass"),
            (self.BOM + b"\xf0\x90\xbf\xbf\xed\xa0\x80"),
        )
        self.assertEqual(
            "[\ud800\udc80]".encode(self.encoding, "surrogatepass"),
            (self.BOM + b"[\xed\xa0\x80\xed\xb2\x80]"),
        )
        self.assertEqual(
            b"abc\xed\xa0\x80def".decode(self.encoding, "surrogatepass"), "abc\ud800def"
        )
        self.assertEqual(
            b"\xf0\x90\xbf\xbf\xed\xa0\x80".decode(self.encoding, "surrogatepass"),
            "\U00010fff\ud800",
        )
...
class UTF7Test(ReadTest, unittest.TestCase):
    ...
    def test_ascii(self):
        ...
        other_ascii = "".join(
            sorted(
                (set(bytes(range(128)).decode()) - set((((set_d + set_o) + "+") + ws)))
            )
        )
        self.assertEqual(
            other_ascii.encode(self.encoding),
    ...
    def test_errors(self):
        ...
        for (raw, expected) in tests:
            with self.subTest(raw=raw):
                self.assertRaises(
                    UnicodeDecodeError, codecs.utf_7_decode, raw, "strict", True
                )
...
class ReadBufferTest(unittest.TestCase):
    def test_array(self):
        ...
        self.assertEqual(
            codecs.readbuffer_encode(array.array("b", b"spam")), (b"spam", 4)
        )
...
punycode_testcases = [
    ("ليهمابتكلموشعربي؟", b"egbpdaj6bu4bxfgehfvwxn"),
    ("他们为什么不说中文", b"ihqwcrb4cv8a8dqg056pqjye"),
    ("他們爲什麽不說中文", b"ihqwctvzc91f659drss3x8bo0yb"),
    ("Pročprostěnemluvíčesky", b"Proprostnemluvesky-uyb24dma41a"),
    ("למההםפשוטלאמדבריםעברית", b"4dbcagdahymbxekheh6e0a7fei0b"),
    ("यहलोगहिन्दीक्योंनहींबोलसकतेहैं", b"i1baa7eci9glrd9b2ae1bj0hfcgg6iyaf8o0a1dig0cd"),
    ("なぜみんな日本語を話してくれないのか", b"n8jok5ay5dzabd5bym9f0cm5685rrjetr6pdxa"),
    (
    ...
    ("安室奈美恵-with-SUPER-MONKEYS", b"-with-SUPER-MONKEYS-pc58ag80a8qai00g7n9n"),
    ("Hello-Another-Way-それぞれの場所", b"Hello-Another-Way--fc4qua05auwb3674vfr0b"),
    ("ひとつ屋根の下2", b"2-u9tlzr9756bt3uc0v"),
    ("MajiでKoiする5秒前", b"MajiKoi5-783gue6qz075azm5e"),
    ("パフィーdeルンバ", b"de-jg4avhby1noc0d"),
...
class PunycodeTest(unittest.TestCase):
    def test_encode(self):
        for (uni, puny) in punycode_testcases:
            self.assertEqual(
                str(uni.encode("punycode"), "ascii").lower(), str(puny, "ascii").lower()
            )
...
nameprep_tests = [
    ...
    (b"CAFE", b"cafe"),
    (b"\xc3\x9f", b"ss"),
    (b"\xc4\xb0", b"i\xcc\x87"),
    (b"\xc5\x83\xcd\xba", b"\xc5\x84 \xce\xb9"),
    (None, None),
    (b"j\xcc\x8c\xc2\xa0\xc2\xaa", b"\xc7\xb0 a"),
    (b"\xe1\xbe\xb7", b"\xe1\xbe\xb6\xce\xb9"),
    (b"\xc7\xb0", b"\xc7\xb0"),
    (b"\xce\x90", b"\xce\x90"),
    (b"\xce\xb0", b"\xce\xb0"),
    (b"\xe1\xba\x96", b"\xe1\xba\x96"),
    (b"\xe1\xbd\x96", b"\xe1\xbd\x96"),
    (b" ", b" "),
    (b"\xc2\xa0", b" "),
    (b"\xe1\x9a\x80", None),
    (b"\xe2\x80\x80", b" "),
    ...
    (b"\xe1\xa0\x8e", None),
    (b"\xef\xbb\xbf", b""),
    (b"\xf0\x9d\x85\xb5", None),
    (b"\xef\x84\xa3", None),
    (b"\xf3\xb1\x88\xb4", None),
    (b"\xf4\x8f\x88\xb4", None),
    (b"\xf2\x8f\xbf\xbe", None),
    (b"\xf4\x8f\xbf\xbf", None),
    (b"\xed\xbd\x82", None),
    (b"\xef\xbf\xbd", None),
    (b"\xe2\xbf\xb5", None),
    (b"\xcd\x81", b"\xcc\x81"),
    (b"\xe2\x80\x8e", None),
    (b"\xe2\x80\xaa", None),
    (b"\xf3\xa0\x80\x81", None),
    (b"\xf3\xa0\x81\x82", None),
    (b"foo\xd6\xbebar", None),
    (b"foo\xef\xb5\x90bar", None),
    (b"foo\xef\xb9\xb6bar", b"foo \xd9\x8ebar"),
    (b"\xd8\xa71", None),
    (b"\xd8\xa71\xd8\xa8", b"\xd8\xa71\xd8\xa8"),
    (None, None),
    (
...
class IDNACodecTest(unittest.TestCase):
    ...
    def test_incremental_decode(self):
        ...
        self.assertEqual(
            "".join(
                codecs.iterdecode((bytes([c]) for c in b"xn--pythn-mua.org."), "idna")
            ),
            "pythön.org.",
        )
        self.assertEqual(
    ...
    def test_incremental_encode(self):
        self.assertEqual(
            b"".join(codecs.iterencode("python.org", "idna")), b"python.org"
        )
        self.assertEqual(
            b"".join(codecs.iterencode("python.org.", "idna")), b"python.org."
        )
        self.assertEqual(
            b"".join(codecs.iterencode("pythön.org.", "idna")), b"xn--pythn-mua.org."
        )
        self.assertEqual(
            b"".join(codecs.iterencode("pythön.org.", "idna")), b"xn--pythn-mua.org."
        )
...
class CodecsModuleTest(unittest.TestCase):
    ...
    def test_open(self):
        ...
        for mode in ("w", "r", "r+", "w+", "a", "a+"):
            with self.subTest(mode), codecs.open(
                os_helper.TESTFN, mode, "ascii"
            ) as file:
        ...
        for encoding in all_unicode_encodings:
            ...
            if encoding not in broken_unicode_with_stateful:
                try:
                    ...
                    result = "".join(
                        codecs.iterdecode(codecs.iterencode("", encoding), encoding)
                    )
    ...
    def test_basics_capi(self):
        ...
        for encoding in all_unicode_encodings:
            if encoding not in broken_unicode_with_stateful:
                ...
                if encoding not in ("idna", "mbcs"):
                    try:
                        cencoder = _testcapi.codec_incrementalencoder(
                            encoding, "ignore"
                        )
                    except LookupError:
                        pass
                        ...
                        cdecoder = _testcapi.codec_incrementaldecoder(
                            encoding, "ignore"
                        )
                        decodedresult = "".join(
...
class CharmapTest(unittest.TestCase):
    def test_decode_with_string_map(self):
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "strict", "abc"), ("abc", 3)
        )
        self.assertEqual(
        ...
        self.assertRaises(
            UnicodeDecodeError, codecs.charmap_decode, b"\x00\x01\x02", "strict", "ab"
        )
        self.assertRaises(
            UnicodeDecodeError,
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", "ab"), ("ab�", 3)
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", "ab\ufffe"), ("ab�", 3)
        )
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", "ab"), ("ab", 3)
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", "ab\ufffe"), ("ab", 3)
        )
        ...
        self.assertEqual(
            codecs.charmap_decode(allbytes, "ignore", ""), ("", len(allbytes))
        )

    def test_decode_with_int2str_map(self):
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "strict", {0: "a", 1: "b", 2: "c"}),
            ("abc", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "strict", {0: "a", 1: "b", 2: ""}),
            ("ab", 3),
        )
        self.assertRaises(
            UnicodeDecodeError,
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", {0: "a", 1: "b"}),
            ("ab�", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "replace", {0: "a", 1: "b", 2: None}
            ),
            ("ab�", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "replace", {0: "a", 1: "b", 2: "\ufffe"}
            ),
            ("ab�", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "backslashreplace", {0: "a", 1: "b"}
            ),
            ("ab\\x02", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "backslashreplace", {0: "a", 1: "b", 2: None}
            ),
            ("ab\\x02", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "backslashreplace", {0: "a", 1: "b", 2: "\ufffe"}
            ),
            ("ab\\x02", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", {0: "a", 1: "b"}),
            ("ab", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", {0: "a", 1: "b", 2: None}),
            ("ab", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "ignore", {0: "a", 1: "b", 2: "\ufffe"}
            ),
            ("ab", 3),
        )
        allbytes = bytes(range(256))
    ...
    def test_decode_with_int2int_map(self):
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "strict", {0: 1114111, 1: b, 2: c}),
            ("\U0010ffffbc", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
        ...
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", {0: a, 1: b}), ("ab�", 3)
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "replace", {0: a, 1: b, 2: 65534}),
            ("ab�", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "backslashreplace", {0: a, 1: b}),
            ("ab\\x02", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(
                b"\x00\x01\x02", "backslashreplace", {0: a, 1: b, 2: 65534}
            ),
            ("ab\\x02", 3),
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", {0: a, 1: b}), ("ab", 3)
        )
        self.assertEqual(
            codecs.charmap_decode(b"\x00\x01\x02", "ignore", {0: a, 1: b, 2: 65534}),
            ("ab", 3),
        )
...
class WithStmtTest(unittest.TestCase):
    ...
    def test_streamreaderwriter(self):
        ...
        with codecs.StreamReaderWriter(
            f, info.streamreader, info.streamwriter, "strict"
        ) as srw:
...
class TypesTest(unittest.TestCase):
    ...
    def test_unicode_escape(self):
        ...
        self.assertRaises(
            UnicodeDecodeError, codecs.unicode_escape_decode, b"\\U00110000"
        )
        self.assertEqual(
            codecs.unicode_escape_decode("\\U00110000", "replace"), ("�", 10)
        )
        self.assertEqual(
            codecs.unicode_escape_decode("\\U00110000", "backslashreplace"),
            ("\\x5c\\x55\\x30\\x30\\x31\\x31\\x30\\x30\\x30\\x30", 10),
        )
        self.assertRaises(
            UnicodeDecodeError, codecs.raw_unicode_escape_decode, b"\\U00110000"
        )
        self.assertEqual(
            codecs.raw_unicode_escape_decode("\\U00110000", "replace"), ("�", 10)
        )
        self.assertEqual(
            codecs.raw_unicode_escape_decode("\\U00110000", "backslashreplace"),
            ("\\x5c\\x55\\x30\\x30\\x31\\x31\\x30\\x30\\x30\\x30", 10),
        )
...
class UnicodeEscapeTest(unittest.TestCase):
    ...
    def test_decode_errors(self):
        ...
        for (c, d) in ((b"x", 2), (b"u", 4), (b"U", 4)):
            for i in range(d):
                self.assertRaises(
                    UnicodeDecodeError, decode, ((b"\\" + c) + (b"0" * i))
                )
                self.assertRaises(
                    UnicodeDecodeError, decode, (((b"[\\" + c) + (b"0" * i)) + b"]")
                )
...
class EscapeEncodeTest(unittest.TestCase):
    def test_escape_encode(self):
        tests = [
            (b"", (b"", 0)),
            (b"foobar", (b"foobar", 6)),
            (b"spam\x00eggs", (b"spam\\x00eggs", 9)),
            (b"a'b", (b"a\\'b", 3)),
            (b"b\\c", (b"b\\\\c", 3)),
            (b"c\nd", (b"c\\nd", 3)),
            (b"d\re", (b"d\\re", 3)),
...
class SurrogateEscapeTest(unittest.TestCase):
    def test_utf8(self):
        self.assertEqual(
            b"foo\x80bar".decode("utf-8", "surrogateescape"), "foo\udc80bar"
        )
        self.assertEqual(
            "foo\udc80bar".encode("utf-8", "surrogateescape"), b"foo\x80bar"
        )
        self.assertEqual(
            b"\xed\xb0\x80".decode("utf-8", "surrogateescape"), "\udced\udcb0\udc80"
        )
        self.assertEqual(
            "\udced\udcb0\udc80".encode("utf-8", "surrogateescape"), b"\xed\xb0\x80"
        )

    def test_ascii(self):
        self.assertEqual(
            b"foo\x80bar".decode("ascii", "surrogateescape"), "foo\udc80bar"
        )
        self.assertEqual(
            "foo\udc80bar".encode("ascii", "surrogateescape"), b"foo\x80bar"
        )

    def test_charmap(self):
        self.assertEqual(
            b"foo\xa5bar".decode("iso-8859-3", "surrogateescape"), "foo\udca5bar"
        )
        self.assertEqual(
            "foo\udca5bar".encode("iso-8859-3", "surrogateescape"), b"foo\xa5bar"
        )
...
class ExceptionChainingTest(unittest.TestCase):
    ...
    def assertWrapped(self, operation, exc_type, msg):
        full_msg = "{} with {!r} codec failed \\({}: {}\\)".format(
...
class CodePageTest(unittest.TestCase):
    ...
    def test_code_page_name(self):
        self.assertRaisesRegex(
            UnicodeEncodeError, "cp932", codecs.code_page_encode, 932, "ÿ"
        )
        self.assertRaisesRegex(
            UnicodeDecodeError,
            "cp932",
    ...
    def check_decode(self, cp, tests):
        for (raw, errors, expected) in tests:
            if expected is not None:
                ...
                self.assertRaises(
                    UnicodeDecodeError, codecs.code_page_decode, cp, raw, errors, True
                )
    ...
    def check_encode(self, cp, tests):
        for (text, errors, expected) in tests:
            if expected is not None:
                ...
                self.assertRaises(
                    UnicodeEncodeError, codecs.code_page_encode, cp, text, errors
                )
    ...
    def test_cp932(self):
        self.check_encode(
            ...
            (
                ...
                ("[ÿ]", "replace", b"[y]"),
                ("[€]", "replace", b"[?]"),
                ("[ÿ]", "backslashreplace", b"[\\xff]"),
                ("[ÿ]", "namereplace", b"[\\N{LATIN SMALL LETTER Y WITH DIAERESIS}]"),
                ("[ÿ]", "xmlcharrefreplace", b"[&#255;]"),
                ("\udcff", "strict", None),
    ...
    def test_multibyte_encoding(self):
        self.check_decode(
            932, ((b"\x84\xe9\x80", "ignore", "騾"), (b"\x84\xe9\x80", "replace", "�騾"))
        )
        self.check_decode(
            self.CP_UTF8,
            (
        ...
        for cp in (
            ...
            *range(57002, (57011 + 1)),
            ...
            if is_code_page_present(cp):
                self.assertEqual(
                    codecs.code_page_decode(cp, b"abc"), ("abc", 3), f"cp{cp}"
                )
...
class Latin1Test(unittest.TestCase):
    ...
    def test_encode_errors(self):
        for (data, error_handler, expected) in (
            ("[€\udc80]", "ignore", b"[]"),
            ("[€\udc80]", "replace", b"[??]"),
            ("[€\U000abcde]", "backslashreplace", b"[\\u20ac\\U000abcde]"),
            ("[€\udc80]", "xmlcharrefreplace", b"[&#8364;&#56448;]"),
            ("[\udc80\udcff]", "surrogateescape", b"[\x80\xff]"),
        ):
            with self.subTest(
...
class LocaleCodecTest(unittest.TestCase):
    ...
    def test_encode_surrogatepass(self):
        try:
            ...
            if str(exc) == "unsupported error handler":
                self.skipTest(
                    f"{self.ENCODING!r} encoder doesn't support surrogatepass error handler"
                )
            else:
                raise
    ...
    def test_decode_surrogatepass(self):
        try:
            ...
            if str(exc) == "unsupported error handler":
                self.skipTest(
                    f"{self.ENCODING!r} decoder doesn't support surrogatepass error handler"
                )
            else:
                raise
