...
class StrftimeTest(unittest.TestCase):
    ...
    def strftest1(self, now):
        ...
        expectations = (
            ...
            (
                "%U",
                ("%02d" % ((now[7] + self.jan1[6]) // 7)),
                "week number of the year (Sun 1st)",
            ),
            ("%w", ("0?%d" % ((1 + now[6]) % 7)), "weekday as a number (Sun 1st)"),
            (
                "%W",
                ("%02d" % ((now[7] + ((self.jan1[6] - 1) % 7)) // 7)),
    ...
    def strftest2(self, now):
        ...
        nonstandard_expectations = (
            ...
            (
                "%x",
                ("%02d/%02d/%02d" % (now[1], now[2], (now[0] % 100))),
                "%m/%d/%y %H:%M:%S",
            ),
            ...
            (
                "%r",
                ("%02d:%02d:%02d %s" % (self.clock12, now[4], now[5], self.ampm)),
                "%I:%M:%S %p",
            ),
        ...
        for e in nonstandard_expectations:
            ...
            if re.match(escapestr(e[1], self.ampm), result):
                ...
                if support.verbose:
                    print(
                        ("Does not appear to support '%s' format (%s)" % (e[0], e[2]))
                    )
