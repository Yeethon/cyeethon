    ...
    def test_setformat(self):
        for t in ("double", "float"):
            ...
            if self.save_formats[t] == "IEEE, big-endian":
                self.assertRaises(
                    ValueError, float.__setformat__, t, "IEEE, little-endian"
                )
            elif self.save_formats[t] == "IEEE, little-endian":
                self.assertRaises(
                    ValueError, float.__setformat__, t, "IEEE, big-endian"
                )
            else:
                self.assertRaises(
                    ValueError, float.__setformat__, t, "IEEE, big-endian"
...
class IEEEFormatTestCase(unittest.TestCase):
    ...
    def test_serialized_float_rounding(self):
        ...
        self.assertEqual(
            struct.pack("<f", (-3.40282356e38)), struct.pack("<f", (-FLT_MAX))
        )
...
class ReprTestCase(unittest.TestCase):
    def test_repr(self):
        with open(
            os.path.join(os.path.split(__file__)[0], "floating_points.txt"),
            encoding="utf-8",
        ) as floats_file:
...
class HexFloatTestCase(unittest.TestCase):
    ...
    def test_from_hex(self):
        ...
        self.identical(
            fromHex("0X1.00000000000018000000000000000001p0"), (1.0 + (2 * EPS))
        )
