...
class HTMLDoc(Doc):
    ...
    def markup(self, text, escape=None, funcs={}, classes={}, methods={}):
        ...
        here = 0
        pattern = re.compile(
            "\\b((http|https|ftp)://\\S+[\\w/]|RFC[- ]?(\\d+)|PEP[- ]?(\\d+)|(self\\.)?(\\w+))"
        )
        while True:
...
class Helper:
    ...
    topics = {
        ...
        "EXPRESSIONS": (
            "operator-summary",
            "lambda or and not in is BOOLEAN COMPARISON BITWISE SHIFTING BINARY FORMATTING POWER UNARY ATTRIBUTES SUBSCRIPTS SLICINGS CALLS TUPLES LISTS DICTIONARIES",
        ),
...
def _url_handler(url, content_type="text/html"):
    ...
    def html_index():
        ...
        contents.append(
            '<p align=right><font color="#909090" face="helvetica,arial"><strong>pydoc</strong> by Ka-Ping Yee&lt;ping@lfw.org&gt;</font>'
        )
