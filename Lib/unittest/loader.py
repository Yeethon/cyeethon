...
class TestLoader(object):
    ...
    def loadTestsFromModule(self, module, *args, pattern=None, **kws):
        ...
        if (len(args) > 0) or ("use_load_tests" in kws):
            warnings.warn(
                "use_load_tests is deprecated and ignored", DeprecationWarning
            )
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        for part in parts:
            try:
                ...
                if (getattr(obj, "__path__", None) is not None) and (
                    error_case is not None
                ):
                    self.errors.append(error_message)
                    return error_case
                else:
                        ...
                        if spec.submodule_search_locations is not None:
                            ...
                            for path in the_module.__path__:
                                if (not set_implicit_top) and (
                                    ...
                                    continue
                                self._top_level_dir = path.split(
                                    the_module.__name__.replace(".", os.path.sep)
                                )[0]
                                tests.extend(
                                    self._find_tests(path, pattern, namespace=True)
                                )
