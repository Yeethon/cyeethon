...
class NonCallableMock(Base):
    ...
    def configure_mock(self, /, **kwargs):
        ...
        for (arg, val) in sorted(
            kwargs.items(), key=(lambda entry: entry[0].count("."))
        ):
            args = arg.split(".")
            final = args.pop()
            obj = self
    ...
    def __getattr__(self, name):
        ...
        if not self._mock_unsafe:
            if name.startswith(("assert", "assret", "asert", "aseert", "assrt")):
                raise AttributeError(
                    f"{name!r} is not a valid assertion. Use a spec for the mock if {name!r} is meant to be an attribute."
                )
        ...
        if result is _deleted:
            ...
            try:
                ...
                raise InvalidSpecError(
                    f"Cannot autospec attr {name!r} from target {target_name!r} as it has already been mocked out. [target={self!r}, attr={result.spec!r}]"
                )
            self._mock_children[name] = result
        return result
        ...
        if not any_order:
            if expected not in all_calls:
                ...
                raise AssertionError(
                    f"""{problem}
Expected: {_CallList(calls)}{self._calls_repr(prefix='Actual').rstrip('.')}"""
                ) from cause
            return
        ...
        if not_found:
            raise AssertionError(
                (
                    "%r does not contain all of %r in its call list, found %r instead"
    ...
    def __init__(
        ...
        *,
        ...
        if _is_instance_mock(spec):
            raise InvalidSpecError(
                f"Cannot spec attr {attribute!r} as the spec has already been mocked out. [spec={spec!r}]"
            )
        if _is_instance_mock(spec_set):
            raise InvalidSpecError(
                f"Cannot spec attr {attribute!r} as the spec_set target has already been mocked out. [spec_set={spec_set!r}]"
            )
        self.getter = getter
    ...
    def __enter__(self):
        ...
        if (new is DEFAULT) and (autospec is None):
            ...
            if _is_instance_mock(self.target):
                raise InvalidSpecError(
                    f"Cannot autospec attr {self.attribute!r} as the patch target has already been mocked out. [target={self.target!r}, attr={autospec!r}]"
                )
            if _is_instance_mock(autospec):
                target_name = getattr(self.target, "__name__", self.target)
                raise InvalidSpecError(
                    f"Cannot autospec attr {self.attribute!r} from target {target_name!r} as it has already been mocked out. [target={self.target!r}, attr={autospec!r}]"
                )
            new = create_autospec(
...
magic_methods = "lt le gt ge eq ne getitem setitem delitem len contains iter hash str sizeof enter exit divmod rdivmod neg pos abs invert complex int float index round trunc floor ceil bool next fspath aiter "
