...
class TestCleanUp(unittest.TestCase):
    ...
    def testCleanUpWithErrors(self):
        ...
        ((_, (Type1, instance1, _)), (_, (Type2, instance2, _))) = reversed(
            outcome.errors
        )
    ...
    def testCleanupInRun(self):
        ...
        test.run(result)
        self.assertEqual(
            ordering, ["setUp", "test", "tearDown", "cleanup2", "cleanup1", "success"]
        )
        blowUp = True
        ordering = []
        test = TestableTest("testNothing")
...
class TestClassCleanup(unittest.TestCase):
    ...
    def test_run_class_cleanUp(self):
        ...
        runTests(TestableTest)
        self.assertEqual(
            ordering, ["setUpClass", "test", "tearDownClass", "cleanup_good"]
        )
    ...
    def test_with_errors_addCleanUp(self):
        ...
        self.assertEqual(
            result.errors[0][1].splitlines()[(-1)], "Exception: CleanUpExc"
        )
        self.assertEqual(
            ordering,
            ["setUpClass", "setUp", "cleanup_exc", "tearDownClass", "cleanup_good"],
        )
    ...
    def test_with_errors_in_addClassCleanup_and_setUps(self):
        ...
        self.assertEqual(
            ordering, ["setUpClass", "setUp", "test", "tearDownClass", "cleanup_exc"]
        )
        ordering = []
        class_blow_up = True
        method_blow_up = False
        result = runTests(TestableTest)
        self.assertEqual(result.errors[0][1].splitlines()[(-1)], "Exception: ClassExc")
        self.assertEqual(
            result.errors[1][1].splitlines()[(-1)], "Exception: CleanUpExc"
        )
        ...
        method_blow_up = True
        result = runTests(TestableTest)
        self.assertEqual(result.errors[0][1].splitlines()[(-1)], "Exception: MethodExc")
        self.assertEqual(
            result.errors[1][1].splitlines()[(-1)], "Exception: CleanUpExc"
        )
        self.assertEqual(
            ordering, ["setUpClass", "setUp", "tearDownClass", "cleanup_exc"]
        )
...
class TestModuleCleanUp(unittest.TestCase):
    ...
    def test_run_module_cleanUp(self):
        ...
        self.assertEqual(
            result.errors[0][1].splitlines()[(-1)], "Exception: setUpModule Exc"
        )
        ordering = []
        blowUp = False
        runTests(TestableTest)
    ...
    def test_with_errors_in_addModuleCleanup_and_setUps(self):
        ...
        self.assertEqual(result.errors[1][1].splitlines()[(-1)], "Exception: ModuleExc")
...
class Test_TextTestRunner(unittest.TestCase):
    ...
    def testBufferAndFailfast(self):
        ...
        result = unittest.TestResult()
        runner = unittest.TextTestRunner(
            stream=io.StringIO(), failfast=True, buffer=True
        )
    ...
    def test_resultclass(self):
        ...
        VERBOSITY = object()
        runner = unittest.TextTestRunner(
            STREAM, DESCRIPTIONS, VERBOSITY, resultclass=MockResultClass
        )
