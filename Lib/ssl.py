...
def _dnsname_match(dn, hostname):
    ...
    if not sep:
        raise CertificateError(
            "sole wildcard without additional labels are not support: {!r}.".format(dn)
        )
    if dn_leftmost != "*":
        raise CertificateError(
...
class SSLContext(_SSLContext):
    ...
    def __new__(cls, protocol=None, *args, **kwargs):
        if protocol is None:
            warnings.warn(
                "ssl module: SSLContext() without protocol argument is deprecated.",
                category=DeprecationWarning,
                stacklevel=2,
            )
...
class SSLObject:
    ...
    def __init__(self, *args, **kwargs):
        raise TypeError(
            f"{self.__class__.__name__} does not have a public constructor. Instances are returned by SSLContext.wrap_bio()."
        )
    ...
    def selected_npn_protocol(self):
        ...
        warnings.warn("ssl module: NPN is deprecated, use ALPN instead", stacklevel=2)
...
class SSLSocket(socket):
    ...
    def __init__(self, *args, **kwargs):
        raise TypeError(
            f"{self.__class__.__name__} does not have a public constructor. Instances are returned by SSLContext.wrap_socket()."
        )
    ...
    def dup(self):
        raise NotImplementedError(
            ("Can't dup() %s instances" % self.__class__.__name__)
        )
    ...
    def recvfrom(self, buflen=1024, flags=0):
        ...
        if self._sslobj is not None:
            raise ValueError(
                ("recvfrom not allowed on instances of %s" % self.__class__)
            )
        else:
