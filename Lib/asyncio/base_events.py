...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def set_default_executor(self, executor):
        if not isinstance(executor, concurrent.futures.ThreadPoolExecutor):
            warnings.warn(
                "Using the default executor that is not an instance of ThreadPoolExecutor is deprecated and will be prohibited in Python 3.9",
                DeprecationWarning,
                2,
            )
        self._default_executor = executor
        ...
        if sock is not None:
            ...
            if not (local_addr or remote_addr):
                ...
                for (idx, addr) in ((0, local_addr), (1, remote_addr)):
                    if addr is not None:
                        assert isinstance(addr, tuple) and (
                            len(addr) == 2
                        ), "2-tuple is expected"
                        infos = await self._ensure_resolved(
                            addr,
            ...
            if reuse_address is not _unset:
                if reuse_address:
                    raise ValueError(
                        "Passing `reuse_address=True` is no longer supported, as the usage of SO_REUSEPORT in UDP poses a significant security concern."
                    )
                else:
                    warnings.warn(
                        "The *reuse_address* parameter has been deprecated as of 3.5.10 and is scheduled for removal in 3.11.",
                        DeprecationWarning,
                        stacklevel=2,
                    )
