...
class OptionMenuTest(MenubuttonTest, unittest.TestCase):
    ...
    def test_bad_kwarg(self):
        with self.assertRaisesRegex(TclError, "^unknown option -image$"):
...
class PanedWindowTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_configure_handlesize(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "handlesize", 8, 9.4, 10.6, (-3), "2m", conv=noconv
        )

    def test_configure_height(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "height", 100, 101.2, 102.6, (-100), 0, "1i", conv=noconv
        )
    ...
    def test_configure_proxyborderwidth(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "proxyborderwidth", 0, 1.3, 2.9, 6, (-2), "10p", conv=noconv
        )
    ...
    def test_configure_sashwidth(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "sashwidth", 10, 11.1, 15.6, (-3), "1m", conv=noconv
        )
    ...
    def test_configure_width(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "width", 402, 403.4, 404.6, (-402), 0, "5i", conv=noconv
        )
    ...
    def test_paneconfigure_sticky(self):
        ...
        self.check_paneconfigure_bad(
            ...
            b,
            "sticky",
            'bad stickyness value "badValue": must be a string containing zero or more of n, e, s, and w',
        )
