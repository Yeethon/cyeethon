...
class AbstractWidgetTest(AbstractTkTest):
    ...
    def checkPixelsParam(
        self, widget, name, *values, conv=None, keep_orig=True, **kwargs
    ):
        if conv is None:
        ...
        self.checkInvalidParam(
            widget, name, "6x", errmsg='bad screen distance "6x"', keep_orig=keep_orig
        )
        self.checkInvalidParam(
            widget,
            name,
    ...
    def test_keys(self):
        ...
        if test.support.verbose:
            ...
            for k in sorted((keys - expected)):
                if not (
                    (k in aliases) and (aliases[k] in keys) and (aliases[k] in expected)
                ):
                    print(
                        (
