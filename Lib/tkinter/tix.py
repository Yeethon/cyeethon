...
class Form:
    ...
    def slaves(self):
        return [
            ...
            for x in self.tk.splitlist(self.tk.call("tixForm", "slaves", self._w))
        ]
        ...
        if not check_intermediate:
            ...
            for i in range((len(plist) - 1)):
                ...
                try:
                    ...
                    parent = TixSubWidget(
                        parent, plist[i], destroy_physically=0, check_intermediate=0
                    )
            if plist:
                name = plist[(-1)]
...
class Select(TixWidget):
    ...
    def __init__(self, master, cnf={}, **kw):
        TixWidget.__init__(
            ...
            master,
            "tixSelect",
            ["allowzero", "radio", "orientation", "labelside", "options"],
            cnf,
            kw,
        )
        self.subwidget_list["label"] = _dummyLabel(self, "label")
...
class Grid(TixWidget, XView, YView):
    ...
    def size_column(self, index, **kw):
        ...
        return self.tk.splitlist(
            self.tk.call(self._w, "size", "column", index, *self._options({}, kw))
        )
