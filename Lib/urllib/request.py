...
class HTTPRedirectHandler(BaseHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        m = req.get_method()
        if not (
            ((code in (301, 302, 303, 307)) and (m in ("GET", "HEAD")))
...
class AbstractBasicAuthHandler:
    ...
    def http_error_auth_reqed(self, authreq, host, req, headers):
        ...
        if unsupported is not None:
            raise ValueError(
                (
                    "AbstractBasicAuthHandler does not support the following scheme: %r"
                    % (scheme,)
                )
            )
    ...
    def http_error_auth_reqed(self, auth_header, host, req, headers):
        ...
        if authreq:
            ...
            if scheme.lower() == "digest":
                ...
                raise ValueError(
                    (
                        "AbstractDigestAuthHandler does not support the following scheme: '%s'"
                        % scheme
                    )
                )
...
class AbstractHTTPHandler(BaseHandler):
    ...
    def do_request_(self, request):
        ...
        if request.data is not None:
            ...
            if (not request.has_header("Content-length")) and (
                not request.has_header("Transfer-encoding")
            ):
