...
class SequenceDefVisitor(EmitVisitor):
    ...
    def emit_sequence_constructor(self, name, depth):
        ...
        self.emit(
            (
                "asdl_%(name)s_seq *_Py_asdl_%(name)s_seq_new(Py_ssize_t size, PyArena *arena);"
                % locals()
            ),
            depth,
        )
...
class PrototypeVisitor(EmitVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        ...
        if args:
            argstr = ", ".join(
                [("%s %s" % (atype, aname)) for (atype, aname, opt) in args]
            )
...
class FunctionVisitor(PrototypeVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        ...
        argstr = ", ".join(
            [("%s %s" % (atype, aname)) for (atype, aname, opt) in (args + attrs)]
        )
        if argstr:
            argstr += ", PyArena *arena"
        else:
...
class Obj2ModVisitor(PickleVisitor):
    def funcHeader(self, name):
        ...
        self.emit(
            (
                "obj2ast_%s(struct ast_state *state, PyObject* obj, %s* out, PyArena* arena)"
                % (name, ctype)
            ),
            0,
        )
    ...
    def complexSum(self, sum, name):
        ...
        for t in sum.types:
            ...
            self.emit(
                ("*out = %s(%s);" % (ast_func_name(t.name), self.buildArgs(args))), 2
            )
    ...
    def visitField(self, field, name, sum=None, prod=None, depth=0):
        ...
        if not field.opt:
            ...
            if self.isNumeric(field):
                ...
                raise TypeError(
                    ("could not determine the default value for %s" % field.name)
                )
        self.emit("}", depth)
        ...
        if field.seq:
            ...
            self.emit(
                (
                    'PyErr_Format(PyExc_TypeError, "%s field \\"%s\\" must be a list, not a %%.200s", _PyType_Name(Py_TYPE(tmp)));'
                    % (name, field.name)
                ),
                (depth + 2),
                reflow=False,
            ...
            if self.isSimpleType(field):
                self.emit(
                    ("%s = _Py_asdl_int_seq_new(len, arena);" % field.name), (depth + 1)
                )
            else:
                self.emit(
                    (
                        "%s = _Py_asdl_%s_seq_new(len, arena);"
            ...
            self.emit(
                ("res = obj2ast_%s(state, tmp2, &val, arena);" % field.type),
                (depth + 2),
                reflow=False,
            )
            ...
            self.emit(
                (
                    'PyErr_SetString(PyExc_RuntimeError, "%s field \\"%s\\" changed size during iteration");'
                    % (name, field.name)
                ),
                (depth + 3),
                reflow=False,
            ...
            self.emit(
                (
                    "res = obj2ast_%s(state, tmp, &%s, arena);"
...
class PyTypesDeclareVisitor(PickleVisitor):
    def visitProduct(self, prod, name):
        ...
        self.emit(
            ("static PyObject* ast2obj_%s(struct ast_state *state, void*);" % name), 0
        )
        if prod.attributes:
            for a in prod.attributes:
    ...
    def visitSum(self, sum, name):
        ...
        self.emit(
            (
                "static PyObject* ast2obj_%s(struct ast_state *state, %s);"
                % (name, ptype)
            ),
            0,
        )
...
class PyTypesVisitor(PickleVisitor):
    ...
    def visitProduct(self, prod, name):
        ...
        self.emit(
            (
                'state->%s_type = make_type(state, "%s", state->AST_type, %s, %d,'
        ...
        if prod.attributes:
            self.emit(
                (
                    "if (!add_attributes(state, state->%s_type, %s_attributes, %d)) return 0;"
                    % (name, name, len(prod.attributes))
                ),
                1,
            )
        else:
            self.emit(
                (
                    "if (!add_attributes(state, state->%s_type, NULL, 0)) return 0;"
                    % name
                ),
                1,
            )
    ...
    def visitSum(self, sum, name):
        self.emit(
            (
                'state->%s_type = make_type(state, "%s", state->AST_type, NULL, 0,'
                % (name, name)
            ),
            1,
        )
    ...
    def visitConstructor(self, cons, name, simple):
        ...
        self.emit(
            (
                'state->%s_type = make_type(state, "%s", state->%s_type, %s, %d,'
                % (cons.name, cons.name, name, fields, len(cons.fields))
            ),
            1,
        )
        self.emit(
            ("%s);" % reflow_c_string(asdl_of(cons.name, cons), 2)), 2, reflow=False
        )
        ...
        if simple:
            self.emit(
                (
                    "state->%s_singleton = PyType_GenericNew((PyTypeObject *)state->%s_type, NULL, NULL);"
                    % (cons.name, cons.name)
                ),
                1,
            )
    ...
    def emit_defaults(self, name, fields, depth):
        for field in fields:
            if field.opt:
                self.emit(
                    (
                        "if (PyObject_SetAttr(state->%s_type, state->%s, Py_None) == -1)"
                        % (name, field.name)
                    ),
                    depth,
                )
...
class ASTModuleVisitor(PickleVisitor):
    ...
    def addObj(self, name):
        self.emit(
            (
                'if (PyModule_AddObjectRef(m, "%s", state->%s_type) < 0) {'
                % (name, name)
            ),
            1,
        )
...
class ObjVisitor(PickleVisitor):
    ...
    def visitSum(self, sum, name):
        ...
        for a in sum.attributes:
            ...
            self.emit(
                ("if (PyObject_SetAttr(result, state->%s, value) < 0)" % a.name), 1
            )
    ...
    def simpleSum(self, sum, name):
        self.emit(
            ("PyObject* ast2obj_%s(struct ast_state *state, %s_ty o)" % (name, name)), 0
        )
        self.emit("{", 0)
    ...
    def set(self, field, value, depth):
        if field.seq:
            if field.type == "cmpop":
                ...
                self.emit(
                    (
                        "PyList_SET_ITEM(value, i, ast2obj_cmpop(state, (cmpop_ty)asdl_seq_GET(%s, i)));"
                        % value
                    ),
                    (depth + 2),
                    reflow=False,
                ...
                self.emit(
                    (
                        "value = ast2obj_list(state, (asdl_seq*)%s, ast2obj_%s);"
                        % (value, field.type)
                    ),
                    depth,
                )
            ...
            self.emit(
                ("value = ast2obj_%s(state, %s);" % (field.type, value)),
                depth,
                reflow=False,
            )
...
def main(
    ...
    if not asdl.check(mod):
        sys.exit(1)
    with c_filename.open("w") as c_file, h_filename.open(
        "w"
    ) as h_file, internal_h_filename.open("w") as internal_h_file:
