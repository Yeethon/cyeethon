...
KIND._TYPE_DECLS_BY_PRIORITY = [KIND.TYPEDEF, KIND.STRUCT, KIND.UNION, KIND.ENUM]
KIND._ALL_DECLS_BY_PRIORITY = [
    *KIND._TYPE_DECLS_BY_PRIORITY,
    KIND.FUNCTION,
    KIND.VARIABLE,
]
KIND._ALL_BY_PRIORITY = [*KIND._ALL_DECLS_BY_PRIORITY, KIND.STATEMENT]
KIND.TYPES = frozenset(KIND._TYPE_DECLS_BY_PRIORITY)
KIND.DECLS = frozenset(KIND._ALL_DECLS_BY_PRIORITY)
KIND._GROUPS = {"type": KIND.TYPES, "decl": KIND.DECLS}
...
class HighlevelParsedItem:
    ...
    def __init__(
        self, file, name, data, parent=None, *, _extra=None, _shortkey=None, _key=None
    ):
        self.file = file
    ...
    def parsed(self):
        try:
            ...
            self._parsed = ParsedItem(
                self.file, self.kind, parent, self.name, self._raw_data()
            )
            return self._parsed
...
class VarType(namedtuple("VarType", "typequal typespec abstract")):
    ...
    def _from_str(cls, text):
        ...
        if text.startswith(("const ", "volatile ")):
            ...
            typequal = None
        m = re.match("^ *'?([a-zA-Z_]\\w*(?:\\s+[a-zA-Z_]\\w*)*)\\s*(.*?)'?\\s*$", text)
        if not m:
...
class Variable(Declaration):
    ...
    def _raw_data(self, data, extra):
        ...
        return {"storage": extra["storage"], "vartype": vartype}
    ...
    def __init__(self, file, name, data, parent=None, storage=None):
        super().__init__(
            ...
            _key=(str(file), f"~{(parent or '')}~", name),
        )
        if storage:
...
class Signature(namedtuple("Signature", "params returntype inline isforward")):
    ...
    def __str__(self):
        ...
        parts.extend(
            [str(self.returntype), self.params, (";" if self.isforward else "{}")]
        )
        return " ".join(parts)
...
class Function(Declaration):
    ...
    def __init__(self, file, name, data, parent=None, storage=None):
        ...
        self._key = (str(file), self._shortkey)
...
class TypeDeclaration(Declaration):
    def __init__(self, file, name, data, parent=None, *, _shortkey=None):
        ...
        super().__init__(
            file, name, data, parent, _shortkey=_shortkey, _key=(str(file), _shortkey)
        )
...
class Statement(HighlevelParsedItem):
    ...
    def __init__(self, file, name, data, parent=None):
        super().__init__(
            file, name, data, parent, _shortkey=(data or ""), _key=(str(file), file.lno)
        )
...
KIND_CLASSES = {
    cls.kind: cls
    for cls in [Variable, Function, TypeDef, Struct, Union, Enum, Statement]
}
