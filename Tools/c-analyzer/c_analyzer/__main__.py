...
from . import analyze as _analyze, datafiles as _datafiles, check_all as _check_all
...
FORMATS = {"raw": fmt_raw, "brief": fmt_brief, "summary": fmt_summary, "full": fmt_full}
...
def _cli_check(parser, checks=None, **kwargs):
    ...
    if checks is False:
        ...
        (len(checks) == 1)
        and (type(checks) is not dict)
        and re.match("^<.*>$", checks[0])
    ):
        check = checks[0][1:(-1)]
    ...
    return [process_checks, process_progress, process_output, process_files]
...
def cmd_check(
    ...
    **kwargs,
    ...
    if not checks:
        ...
        checks = [checks]
    checks = [(_CHECKS[c] if isinstance(c, str) else c) for c in checks]
    printer = Printer(verbosity)
    (handle_failure, handle_after, div) = _get_check_handlers(fmt, printer, verbosity)
...
def _cli_analyze(parser, **kwargs):
    ...
    process_files = add_files_cli(parser, **kwargs)
    return [process_progress, process_output, process_files]
    ...
    **kwargs,
    ...
    if datacmd == "show":
        ...
        if (known is None) or usestdout:
            outfile = io.StringIO()
            _datafiles.write_known(analyzed, outfile, extracolumns, relroot=relroot)
            print(outfile.getvalue())
        else:
            _datafiles.write_known(analyzed, known, extracolumns, relroot=relroot)
    elif datacmd == "check":
...
def parse_args(argv=sys.argv[1:], prog=sys.argv[0], *, subset=None):
    ...
    parser = argparse.ArgumentParser(prog=(prog or get_prog()))
    processors = add_commands_cli(
        parser,
        commands={k: v[1] for (k, v) in COMMANDS.items()},
        commonspecs=[add_verbosity_cli, add_traceback_cli],
        subset=subset,
    )
    ...
    (verbosity, traceback_cm) = process_args_by_key(
        args, argv, processors[cmd], ["verbosity", "traceback_cm"]
    )
