...
CHECKS = dict(c_analyzer.CHECKS, **{"globals": _analyzer.check_globals})
...
def _cli_parse(parser):
    ...
    process_preprocessor = c_parser.add_preprocessor_cli(
        parser, get_preprocessor=_parser.get_preprocessor
    )
    process_files = add_files_cli(parser, **FILES_KWARGS)
    return [process_output, process_kind, process_preprocessor, process_files]
...
def cmd_parse(filenames=None, **kwargs):
    ...
    c_parser.cmd_parse(filenames, relroot=REPO_ROOT, **kwargs)
    ...
    if datacmd == "show":
        ...
        def analyze(files, **kwargs):
            ...
            results = _c_analyzer.analyze_decls(
                decls, known={}, analyze_resolved=_analyzer.analyze_resolved
            )
...
def cmd_capi(
    ...
    **kwargs,
    ...
    lines = render(
        items, groupby=groupby, showempty=showempty, verbose=(verbosity > VERBOSITY)
    )
    print()
    for line in lines:
        print(line)
...
COMMANDS = {
    ...
    "parse": ("parse the CPython source files", [_cli_parse], cmd_parse),
    "data": (
        "check/manage local data (e.g. knwon types, ignored vars, caches)",
        [_cli_data],
        cmd_data,
    ),
    "capi": ("inspect the C-API", [_cli_capi], cmd_capi),
}
