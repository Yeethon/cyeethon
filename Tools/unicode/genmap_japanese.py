...
def loadmap_jisx0213(fo):
    ...
    for line in fo:
        ...
        if len(row[1].split("+")) == 2:
            ...
            uniprefix = eval(("0x" + row[1][2:6]))
            uni = eval(("0x" + row[1][7:11]))
            if level != 3:
...
def main():
    ...
    jisx0213file = open_mapping_file(
    ...
    for (c1, m) in sjisdecmap.items():
        for (c2, code) in m.items():
            ...
            sjisencmap[(code >> 8)][(code & 255)] = (c1 << 8) | c2
    for (c1, m) in cp932decmap.items():
        for (c2, code) in m.items():
            cp932encmap.setdefault((code >> 8), {})
            if (code & 255) not in cp932encmap[(code >> 8)]:
                cp932encmap[(code >> 8)][(code & 255)] = (c1 << 8) | c2
    for (c1, m) in cp932encmap.copy().items():
        for (c2, code) in m.copy().items():
            if (
                (c1 in sjisencmap)
                and (c2 in sjisencmap[c1])
                and (sjisencmap[c1][c2] == code)
            ):
    ...
    for (c1, m) in jisx0208decmap.items():
        for (c2, code) in m.items():
            ...
            jisx0208_0212encmap[(code >> 8)][(code & 255)] = (c1 << 8) | c2
    for (c1, m) in jisx0212decmap.items():
        for (c2, code) in m.items():
            jisx0208_0212encmap.setdefault((code >> 8), {})
            if (code & 255) in jisx0208_0212encmap[(code >> 8)]:
                print("OOPS!!!", code)
            jisx0208_0212encmap[(code >> 8)][(code & 255)] = (32768 | (c1 << 8)) | c2
    jisx0213bmpencmap = {}
    for (c1, m) in jis3decmap.copy().items():
        for (c2, code) in m.copy().items():
            if (c1 in jisx0208decmap) and (c2 in jisx0208decmap[c1]):
                if code in jis3_pairdecmap:
                    jisx0213bmpencmap[(code >> 8)][(code & 255)] = (0,)
                    ...
                    raise ValueError(
                        "Difference between JIS X 0208 and JIS X 0213 Plane 1 is found."
                    )
            else:
                jisx0213bmpencmap.setdefault((code >> 8), {})
                if code not in jis3_pairdecmap:
                    jisx0213bmpencmap[(code >> 8)][(code & 255)] = (c1 << 8) | c2
                else:
                    jisx0213bmpencmap[(code >> 8)][(code & 255)] = (0,)
                    jisx0213pairencmap.append((code, 0, ((c1 << 8) | c2)))
    for (c1, m) in jis4decmap.items():
        for (c2, code) in m.items():
            jisx0213bmpencmap.setdefault((code >> 8), {})
            jisx0213bmpencmap[(code >> 8)][(code & 255)] = (32768 | (c1 << 8)) | c2
    jisx0213empencmap = {}
    for (c1, m) in jis3_2_decmap.items():
        for (c2, code) in m.items():
            jisx0213empencmap.setdefault((code >> 8), {})
            jisx0213empencmap[(code >> 8)][(code & 255)] = (c1 << 8) | c2
    for (c1, m) in jis4_2_decmap.items():
        for (c2, code) in m.items():
            jisx0213empencmap.setdefault((code >> 8), {})
            jisx0213empencmap[(code >> 8)][(code & 255)] = (32768 | (c1 << 8)) | c2
    ...
    with open("mappings_jisx0213_pair.h", "w") as fp:
        print_autogen(fp, os.path.basename(__file__))
        fp.write(
            f"""#define JISX0213_ENCPAIRS {len(jisx0213pairencmap)}
"""
        )
        fp.write(
        ...
        for (body, modifier, jis) in jisx0213pairencmap:
            filler.write(
                "{", ("0x%04x%04x," % (body, modifier)), ("0x%04x" % jis), "},"
            )
        filler.printout(fp)
        fp.write("};\n")
        fp.write("#endif\n")
