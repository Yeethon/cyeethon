...
PUA_1 = range(57344, 63744)
PUA_15 = range(983040, 1048574)
PUA_16 = range(1048576, 1114110)
NAME_ALIASES_START = 983040
NAMED_SEQUENCES_START = 983552
old_versions = ["3.2.0"]
CATEGORY_NAMES = [
    "Cn",
    "Lu",
        ...
        if record:
            if record.decomposition_type:
                ...
                if len(decomp) > 19:
                    raise Exception(
                        (
                            "character %x has a decomposition too large for nfd_nfkd"
                            % char
                        )
                    )
                if decomp[0][0] == "<":
...
def makeunicodename(unicode, trace):
    ...
    with open(FILE, "w") as fp:
        ...
        fprint(
            (
                "static const unsigned int aliases_end = %#x;"
...
def merge_old_version(version, new, old):
    ...
    bidir_changes = [255] * 1114112
    category_changes = [255] * 1114112
    decimal_changes = [255] * 1114112
    mirrored_changes = [255] * 1114112
    east_asian_width_changes = [255] * 1114112
    numeric_changes = [0] * 1114112
    ...
    def __init__(self, version, cjk_check=True):
        ...
        if version != "3.2.0":
            ...
            for (name, chars) in UcdFile(NAMED_SEQUENCES, version):
                ...
                assert all(
                    ((c <= 65535) for c in chars)
                ), "use Py_UCS4 in the NamedSequence struct and in unicodedata_lookup"
...
def splitbins(t, trace=0):
    ...
    if trace:
        ...
        print(
            "Size of original table:", (len(t) * getsize(t)), "bytes", file=sys.stderr
        )
    n = len(t) - 1
    maxshift = 0
    if n > 0:
