...
class EncodeMapWriter:
    ...
    def printmap(self):
        self.fp.write(
            f"""static const {self.elemtype} __{self.prefix}_encmap[{len(self.filler)}] = {{
"""
        )
        self.filler.printout(self.fp)
        self.fp.write("};\n\n")
        self.fp.write(
            f"""static const {self.indextype} {self.prefix}_encmap[256] = {{
"""
        )
        for i in range(256):
...
def print_autogen(fo, source):
    fo.write(
        f"""// AUTO-GENERATED FILE FROM {source}: DO NOT EDIT
"""
    )
