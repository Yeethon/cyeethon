...
def parse_err_h(args):
    ...
    pat = re.compile("#\\s*define\\W+ERR_LIB_(\\w+)\\s+(\\d+)")
    lib2errnum = {}
...
def parse_extra_reasons(args):
    ...
    pat = re.compile("^R\\s+((\\w+)_R_(\\w+))\\s+(\\d+)")
...
def main():
    ...
    if not os.path.isfile(args.errtxt):
        parser.error(
            f"""File {args.errtxt} not found in srcdir
."""
        )
    ...
    lines = [
        f"/* File generated by Tools/ssl/make_ssl_data.py *//* Generated on {datetime.datetime.utcnow().isoformat()} */"
    ]
