...
def check_library(stable_abi_file, library, abi_funcs, dynamic=False):
    ...
    if missing_symbols:
        raise Exception(
            f"""Some symbols from the limited API are missing: {', '.join(missing_symbols)}
...
def generate_limited_api_symbols(args):
    ...
    with open(args.output_file, "w") as output_file:
        output_file.write(
            f"""# File generated by 'make regen-limited-abi'
"""
        )
        output_file.write(
...
def get_limited_api_definitions(headers):
    ...
    stable_functions = set(
        re.findall("__PyAPI_FUNC\\(.*?\\)\\s*(.*?)\\s*\\(", preprocesor_output)
    )
    stable_exported_data = set(
        re.findall("__EXPORT_DATA\\((.*?)\\)", preprocesor_output)
    )
    stable_data = set(
        re.findall("__PyAPI_DATA\\(.*?\\)\\s*\\(?(.*?)\\)?\\s*;", preprocesor_output)
    )
