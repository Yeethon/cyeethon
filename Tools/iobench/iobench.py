...
def get_text_files():
    return (
        (("%s-%s-%s.txt" % (name, TEXT_ENCODING, NEWLINES)), size)
...
def run_all_tests(options):
    ...
    def print_results(size, n, real, cpu):
        ...
        if cpu < (0.9 * real):
            out.write(
                (
                    "   warning: test above used only %d%% CPU, result may be flawed!\n"
                    % ((100.0 * cpu) / real)
                )
            )
    ...
    if ("t" in options) and ("w" in options):
        ...
        run_test_family(
            ...
            "b",
            text_files,
            (lambda fn: text_open(os.devnull, "w")),
            make_test_source,
        )
    if ("b" in options) and ("w" in options):
        print("\n** Binary overwrite **\n")
