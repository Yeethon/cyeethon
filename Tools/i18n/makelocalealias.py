...
def parse_glibc_supported(filename):
    ...
    for line in lines:
        ...
        if modifier and (
            not ((modifier == "euro") and (alias_encoding == "ISO-8859-15"))
        ):
            alias += "@" + modifier
        data[locale] = alias
    return data
...
def check(data):
    ...
    for (k, v) in data.items():
        if locale.normalize(k) != v:
            print(
                ("ERROR: %a -> %a != %a" % (k, locale.normalize(k), v)), file=sys.stderr
            )
            errors += 1
    return errors
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--locale-alias",
        default=LOCALE_ALIAS,
        help=("location of the X11 alias file (default: %a)" % LOCALE_ALIAS),
    )
    parser.add_argument(
        "--glibc-supported",
        default=SUPPORTED,
        help=("location of the glibc SUPPORTED locales file (default: %a)" % SUPPORTED),
    )
