...
class Chooser:
    ...
    def __init__(
        self, master=None, databasefile=None, initfile=None, ignore=None, wantspec=None
    ):
    ...
    def show(self, color, options):
        ...
        if not self.__master:
            ...
            self.__master = Tk()
        if not self.__pw:
            (self.__pw, self.__sb) = Main.build(
                master=self.__master, initfile=self.__initfile, ignore=self.__ignore
            )
        else:
            self.__pw.deiconify()
        if colordb:
