...
class Sheet:
    def __init__(self):
        self.cells = {}
        self.ns = dict(cell=self.cellvalue, cells=self.multicellvalue, sum=sum)
    ...
    def selectcells(self, x1, y1, x2, y2):
        ...
        if y1 > y2:
            (y1, y2) = (y2, y1)
        return [
            (x, y) for (x, y) in self.cells if ((x1 <= x <= x2) and (y1 <= y <= y2))
        ]
    ...
    def end_value(self, text):
        if isinstance(self.value, BaseCell):
            ...
            self.cell = StringCell(
                self.value, (self.fmt or "%s"), (self.alignment or LEFT)
            )
        else:
...
def translate(formula):
    ...
    out = []
    for part in re.split("(\\w+(?::\\w+)?)", formula):
        m = re.match("^([A-Z]+)([1-9][0-9]*)(?::([A-Z]+)([1-9][0-9]*))?$", part)
        if m is None:
            out.append(part)
        else:
