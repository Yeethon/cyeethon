...
def main(file_source, install_target):
    ...
    assert len(files) == len(
        set((make_id(f[1]) for f in files))
    ...
    for dir_parent in sorted(directories):
        ...
        for dir_name in sorted(directories[dir_parent]):
            lines.append(
                '            <Directory Id="{}_{}" Name="{}" />'.format(
                    dir_parent, make_id(dir_name), dir_name
                )
            )
    ...
    for dir_parent in (
        ...
        lines.append('        <DirectoryRef Id="{}">'.format(dir_parent))
        lines.append(
            '            <Directory Id="{}___pycache__" Name="__pycache__" />'.format(
                dir_parent
            )
        )
        lines.append("        </DirectoryRef>")
    ...
    for group in sorted(groups):
        lines.extend(
            ["    <Fragment>", '        <ComponentGroup Id="{}">'.format(group)]
        )
        for (source, target, disk_id, condition) in groups[group]:
            lines.append(
                '            <Component Id="{}" Directory="{}" Guid="*">'.format(
                    make_id(target), make_id(target.parent)
                )
            )
            if condition:
                lines.append(
            ...
            if disk_id:
                lines.append(
                    '                <File Id="{}" Name="{}" Source="{}" DiskId="{}" />'.format(
                        make_id(target), target.name, source, disk_id
                    )
                )
            else:
                lines.append(
                    '                <File Id="{}" Name="{}" Source="{}" />'.format(
                        make_id(target), target.name, source
                    )
                )
        ...
        if create_folders or remove_folders:
            lines.append(
                '            <Component Id="{}__pycache__folders" Directory="TARGETDIR" Guid="{}">'.format(
                    group, uuid1()
                )
            )
            lines.extend(
                (
                    '                <CreateFolder Directory="{}" />'.format(p)
                    for p in create_folders
                )
            )
            lines.extend(
                (
                    '                <RemoveFile Id="Remove_{0}_files" Name="*" On="uninstall" Directory="{0}" />'.format(
                        p
                    )
                    for p in create_folders
                )
            ...
            lines.extend(
                (
                    '                <RemoveFolder Id="Remove_{0}_folder" On="uninstall" Directory="{0}" />'.format(
                        p
                    )
                    for p in (create_folders | remove_folders)
                )
            )
            lines.append("            </Component>")
