...
Body.enum.converters["loweralpha"] = Body.enum.converters[
    "upperalpha"
] = Body.enum.converters["lowerroman"] = Body.enum.converters[
    "upperroman"
] = lambda x: None
...
class Availability(Directive):
    ...
    def run(self):
        ...
        pnode = nodes.paragraph(
            (availability_ref + self.arguments[0]), classes=["availability"]
        )
    ...
    def run(self):
        ...
        if not target:
            target = "audit_event_{}_{}".format(
                re.sub("\\W", "_", name), len(info["source"])
            )
            ids.append(target)
...
class MiscNews(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            return [node]
        content = issue_re.sub(
            "`bpo-\\1 <https://bugs.python.org/issue\\1>`__", content
        )
    ...
    for name in sorted(getattr(env, "all_audit_events", ())):
        ...
        for (i, (doc, label)) in backlinks:
            if isinstance(label, str):
                ...
                try:
                    ref["refuri"] = "{}#{}".format(
                        app.builder.get_relative_uri(fromdocname, doc), label
                    )
                except NoUri:
                    continue
                node += ref
